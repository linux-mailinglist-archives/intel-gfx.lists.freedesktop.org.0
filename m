...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 7061028F6E5
	for <lists+intel-gfx@lfdr.de>; Thu, 15 Oct 2020 18:36:19 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id C1C646ED83;
	Thu, 15 Oct 2020 16:36:17 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga11.intel.com (mga11.intel.com [192.55.52.93])
 by gabe.freedesktop.org (Postfix) with ESMTPS id D6F4A6ED83
 for <intel-gfx@lists.freedesktop.org>; Thu, 15 Oct 2020 16:36:15 +0000 (UTC)
IronPort-SDR: /PUK2aecg5Ga6w2+M7NRbMTaimMtRg6ClfNCRvRnaGc8IbKQm87DxN7q1gnWjKtkKtxR+P/NH+
 TjzCzNVsofvA==
X-IronPort-AV: E=McAfee;i="6000,8403,9775"; a="162927727"
X-IronPort-AV: E=Sophos;i="5.77,379,1596524400"; d="scan'208";a="162927727"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga003.jf.intel.com ([10.7.209.27])
 by fmsmga102.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 15 Oct 2020 09:36:15 -0700
IronPort-SDR: o9yD4FnDufLevTAEZC4g872zSUd0QF+u/Inhn9brfwwVLl4wmNthMwYL1bYUcu9J2LKRutdC+H
 8k1uz7Tw75+w==
X-IronPort-AV: E=Sophos;i="5.77,379,1596524400"; d="scan'208";a="314575692"
Received: from labuser-z97x-ud5h.jf.intel.com (HELO labuser-Z97X-UD5H)
 ([10.165.21.211])
 by orsmga003-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 15 Oct 2020 09:36:15 -0700
Date: Thu, 15 Oct 2020 09:37:47 -0700
From: "Navare, Manasi" <manasi.d.navare@intel.com>
To: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
Message-ID: <20201015163741.GB28888@labuser-Z97X-UD5H>
...
In-Reply-To: <20201015130705.GL6112@intel.com>
...
Well honestly @Maarten wrote this and I believe had several iterations of r=
eviews with you.
So he can explain better.
My understanding so far was that there is no encoder /transcoder for slave =
and thats
...
> > @@ -4529,7 +4562,8 @@ void intel_ddi_get_config(struct intel_encoder *e=
ncoder,
...
> > @@ -7125,6 +7125,45 @@ static void hsw_set_frame_start_delay(const stru=
ct intel_crtc_state *crtc_state)
> >  	intel_de_write(dev_priv, reg, val);
> >  }
> >  =

> > +static void tgl_ddi_bigjoiner_pre_enable(struct intel_atomic_state *st=
ate,
...
> >  {
> > @@ -7138,34 +7177,39 @@ static void hsw_crtc_enable(struct intel_atomic=
_state *state,
...
> > +	if (!new_crtc_state->bigjoiner_slave && !transcoder_is_dsi(cpu_transc=
oder)) {
> > +		if (!transcoder_is_dsi(cpu_transcoder))
> > +			intel_set_transcoder_timings(new_crtc_state);
> >  =
...
> > @@ -7201,6 +7245,11 @@ static void hsw_crtc_enable(struct intel_atomic_=
state *state,
...
> >  	if (psl_clkgate_wa) {
> > @@ -7495,6 +7544,9 @@ static u64 get_crtc_power_domains(struct intel_cr=
tc_state *crtc_state)
...
> > @@ -8113,6 +8165,30 @@ static u32 ilk_pipe_pixel_rate(const struct inte=
...
dev);
> > @@ -9027,20 +9103,22 @@ static void intel_get_pipe_src_size(struct inte=
l_crtc *crtc,
...
> >  }
> > @@ -11207,6 +11285,9 @@ static void hsw_get_ddi_port_state(struct intel=
_crtc *crtc,
> >  	} else {
...
> >  		else
> > @@ -11279,12 +11360,20 @@ static bool hsw_get_pipe_config(struct intel_=
crtc *crtc,
...
> >  	}
> > @@ -11370,8 +11459,11 @@ static bool hsw_get_pipe_config(struct intel_c=
rtc *crtc,
> >  		}
> >  	}
> >  =
...
> > @@ -13385,12 +13477,15 @@ intel_crtc_copy_uapi_to_hw_state(struct intel=
_atomic_state *state,
...
> > @@ -14027,21 +14122,42 @@ intel_pipe_config_compare(const struct intel_=
...
> > @@ -14051,24 +14167,11 @@ intel_pipe_config_compare(const struct intel_=
crtc_state *current_config,
...
> > @@ -14094,7 +14197,8 @@ intel_pipe_config_compare(const struct intel_cr=
...
> > @@ -14115,48 +14219,51 @@ intel_pipe_config_compare(const struct intel_=
crtc_state *current_config,

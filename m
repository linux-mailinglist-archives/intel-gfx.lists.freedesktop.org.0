...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 1F67412BBD1
	for <lists+intel-gfx@lfdr.de>; Sat, 28 Dec 2019 00:47:31 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id EC1A76E0D1;
	Fri, 27 Dec 2019 23:47:26 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga11.intel.com (mga11.intel.com [192.55.52.93])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 117366E0C6
 for <intel-gfx@lists.freedesktop.org>; Fri, 27 Dec 2019 23:47:26 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from fmsmga001.fm.intel.com ([10.253.24.23])
 by fmsmga102.fm.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 27 Dec 2019 15:47:25 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.69,364,1571727600"; d="scan'208";a="223844117"
Received: from labuser-z97x-ud5h.jf.intel.com (HELO intel.com) ([10.54.75.49])
 by fmsmga001.fm.intel.com with ESMTP; 27 Dec 2019 15:47:25 -0800
Date: Fri, 27 Dec 2019 15:49:15 -0800
From: Manasi Navare <manasi.d.navare@intel.com>
To: Matt Roper <matthew.d.roper@intel.com>
Message-ID: <20191227234915.GB27178@intel.com>
...
In-Reply-To: <20191227231707.GN2877816@mdroper-desk1.amr.corp.intel.com>
...
> > > > > > We also need to force modeset for all synced crtcs after fastse=
t check.
> > > > > > =
...
> > > > > I sent a reply to your v4 of this right at the same time you sent=
 out
> > > > > v5, but I'm not sure if my reply went through since it doesn't sh=
ow up
> > > > > in patchwork.  I've included the same feedback I gave on v4 below=
 in
...
> > > > > > @@ -14304,6 +14304,118 @@ static bool intel_cpu_transcoder_need=
s_modeset(struct intel_atomic_state *state,
> > > > > >  	return false;
> > > > > >  }
> > > > > >  =
...
> > > > > Are we guaranteed to have the other CRTC in the state at this poi=
nt?  If
> > > > > the tile information was gone at the beginning of the transaction=
, then
> > > > > it wouldn't have known to bring in the other CRTC.  So if a modes=
et was
> > > > > only submitted on one of the two synchronized CRTCs then I don't =
see
> > > > > where the other one would be added to the transaction before this=
 point?
> > > > >
> > > > =

> > > > Yes basically at this point the new crtc state is not cleared so we=
 use the =

> > > > previously synced crtcs through new crtc state master slave assignm=
ents from previous
> > > > modeset.
> > > =
...
> > > automatically add CRTC-B (slave).  CRTC B's committed state does inde=
ed
...
> > > transaction (the only place where we bring in new CRTCs in this patch=
 is
...
Yes this above example makes complete sense. I think now the DRM patches ma=
ke sure that
if we lose one of the tiles through hotplug it also fallsback the mode on r=
emaining conn
so conn A here to a lower non tiled mode so I see that it also gets added t=
o the state
with mode changed and the other disable due to conn B unplug goes through d=
isable sequence
and disables all associated master/slaves as well.

But from what you are saying is it would be safe to check if any of the con=
ns in state that needs a modeset is involved in port sync
and then loop through all drm connectors and  force modeset on all other co=
nns in the same port sync?
So kind of like the intel_modeset_synced_crtcs() right after modeset_all_ti=
les() ?

But may be for now, add a FIXME for this case and land this patch to make s=
ure that it fixes
...
> > > > > > +
> > > > > > +static void
> > > > > > +intel_atomic_check_synced_crtcs(struct intel_atomic_state *sta=
te)
> > > > > > +{
...
> > > > > There isn't really anything i915-specific in this function and it=
 feels
> > > > > like something other drivers may need as well if they treat tiled
> > > > > monitors in a similar manner.  We may want to consider pulling th=
is out
> > > > > to a DRM core helper, although we can do that in a future patch.
> > > > > =

> > > > > With a bit of extra work, the function below could also potential=
ly be
> > > > > moved to the core too.  Do you know if there's other hardware wit=
h port
> > > > > sync capabilities that could benefit from these?
> > > > >
> > > > =

> > > > Yes definitely can eventually make this a drm helper even though th=
e whole master slave
> > > > thing is i915 specific for now
> > > >  =

> > > > > > +
> > > > > > +static int
> > > > > > +intel_atomic_check_tiled_conns(struct intel_atomic_state *stat=
e)
> > > > > > +{
> > > > > > +	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev=
);
...
> > > > > >   * @dev: drm device
> > > > > > @@ -14331,6 +14443,10 @@ static int intel_atomic_check(struct d=
rm_device *dev,
> > > > > >  	if (ret)
> > > > > >  		goto fail;
> > > > > >  =
...
> > > > > > @@ -14378,6 +14494,13 @@ static int intel_atomic_check(struct d=
rm_device *dev,
> > > > > >  		}
> > > > > >  	}
> > > > > >  =
...
> > > > > explanation for why intel_atomic_check_tiled_conns above didn't a=
lready
> > > > > take care of this.  I.e., if you've plugged in a different monito=
r, the
> > > > > tile information may have already vanished from drm_connector by =
the
> > > > > time we start this atomic transaction, so we still need to deal w=
ith
> > > > > connectors that used to be tiled (and thus were port-synced) but =
no
> > > > > longer are.
> > > > >
> > > > =
...
> > > > > Although I can't think of a reason why it would cause a problem i=
n this
> > > > > case, we do seem to be violating the directions in the big kernel=
doc
> > > > > warning attached to drm_atomic_helper_check_modeset().  I.e. if w=
e set
> > > > > mode_changed in our own check functions, then we're supposed to r=
e-call
> > > > > drm_atomic_helper_check_modeset() to make sure everything is prop=
erly
> > > > > handled.  If we're not going to follow those directions, we should
> > > > > probably be clear why we don't think it's necessary.
> > > > > =

> > > > > Actually, I wonder if some of this tiling handling should eventua=
lly
> > > > > migrate into that core helper function in the future...
> > > > >
> > > > =

> > > > We are setting the mode hanged to true directly in our function so =
yes it makes
> > > > sense to eventually do that as part of the core helper function but=
 i still
> > > > dont see the need for us to call drm_atomic_helper_check_modeset() =
again
> > > > since our function is essentially checking modeset based on the til=
ed conditions
> > > =
...
> > > function in cases like what we're doing here, so if we're intentional=
ly
> > > not following those directions we should explain why for future
> > > reference.  It's also possible that function may expand its role in t=
he
> > > future and start doing something new that we really do need to worry
> > > about, so it would be good to have the history and justification for =
not

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 395991ED2B9
	for <lists+intel-gfx@lfdr.de>; Wed,  3 Jun 2020 16:57:26 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id BEC4F89C82;
	Wed,  3 Jun 2020 14:57:20 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mblankhorst.nl (mblankhorst.nl
 [IPv6:2a02:2308::216:3eff:fe92:dfa3])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 237B689C61
 for <intel-gfx@lists.freedesktop.org>; Wed,  3 Jun 2020 14:57:18 +0000 (UTC)
From: Maarten Lankhorst <maarten.lankhorst@linux.intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Wed,  3 Jun 2020 16:56:56 +0200
Message-Id: <20200603145713.3835124-7-maarten.lankhorst@linux.intel.com>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH 07/24] drm/i915: Use per object locking in
 execbuf, v11.
...
---
 .../gpu/drm/i915/gem/i915_gem_execbuffer.c    | 396 +++++++++++-------
 .../i915/gem/selftests/i915_gem_execbuffer.c  |   2 +-
 drivers/gpu/drm/i915/i915_gem.c               |   6 +
 drivers/gpu/drm/i915/i915_gem.h               |   1 +
 4 files changed, 249 insertions(+), 156 deletions(-)

diff --git a/drivers/gpu/drm/i915/gem/i915_gem_execbuffer.c b/drivers/gpu/drm/i915/gem/i915_gem_execbuffer.c
index 4ef6e48da8f1..5bcc802e0014 100644
...
@@ -984,17 +1004,41 @@ static inline struct i915_ggtt *cache_to_ggtt(struct reloc_cache *cache)
...
+		goto out_pool;
 	...
 	if (IS_ERR(batch)) {
@@ -1023,11 +1067,9 @@ static int reloc_gpu_chain(struct reloc_cache *cache)
...
-		i915_vma_unlock(batch);
 	...
 	if (err)
@@ -1048,7 +1090,11 @@ static int reloc_gpu_chain(struct reloc_cache *cache)
 	cache->rq_vma = batch;
 
 out_pool:
-	intel_gt_buffer_pool_put(pool);
+	/* Remove the old pool from chain */
...
@@ -1057,7 +1103,7 @@ static unsigned int reloc_bb_flags(const struct reloc_cache *cache)
...
+static int reloc_gpu_flush(struct i915_execbuffer *eb, struct reloc_cache *cache)
 {
 	...
 	int err;
@@ -1091,7 +1137,12 @@ static int reloc_gpu_flush(struct reloc_cache *cache)
...
@@ -1110,7 +1161,6 @@ static void reloc_cache_reset(struct reloc_cache *cache)
...
@@ -1145,15 +1195,9 @@ static void *reloc_kmap(struct drm_i915_gem_object *obj,
...
@@ -1192,9 +1236,7 @@ static void *reloc_iomap(struct drm_i915_gem_object *obj,
...
@@ -1283,7 +1325,7 @@ static int reloc_move_to_gpu(struct i915_request *rq, struct i915_vma *vma)
...
@@ -1293,8 +1335,6 @@ static int reloc_move_to_gpu(struct i915_request *rq, struct i915_vma *vma)
 	if (err == 0)
...
@@ -1303,15 +1343,22 @@ static int __reloc_gpu_alloc(struct i915_execbuffer *eb,
 			     unsigned int len)
 {
...
+		goto err_pool;
 	...
 	cmd = i915_gem_object_pin_map(pool->obj,
 				      cache->has_llc ?
@@ -1319,7 +1366,7 @@ static int __reloc_gpu_alloc(struct i915_execbuffer *eb,
 				      I915_MAP_FORCE_WC);
 	if (IS_ERR(cmd)) {
 		err = PTR_ERR(cmd);
-		goto out_pool;
...
@@ -1358,11 +1405,11 @@ static int __reloc_gpu_alloc(struct i915_execbuffer *eb,
 	if (err)
 		goto err_request;
...
@@ -1373,9 +1420,10 @@ static int __reloc_gpu_alloc(struct i915_execbuffer *eb,
 	cache->rq_cmd = cmd;
 	cache->rq_size = 0;
 	cache->rq_vma = batch;
+	cache->pool = pool;
...
@@ -1385,8 +1433,8 @@ static int __reloc_gpu_alloc(struct i915_execbuffer *eb,
 	i915_vma_unpin(batch);
 err_unmap:
...
@@ -1429,7 +1477,7 @@ static u32 *reloc_gpu(struct i915_execbuffer *eb,
...
+		err = reloc_gpu_chain(eb, cache);
 		if (unlikely(err)) {
 			...
 			return ERR_PTR(err);
@@ -1468,7 +1516,7 @@ static unsigned long vma_phys_addr(struct i915_vma *vma, u32 offset)
...
+static int __reloc_entry_gpu(struct i915_execbuffer *eb,
 			      ...
 			      u64 offset,
 			      u64 target_addr)
@@ -1486,7 +1534,9 @@ static bool __reloc_entry_gpu(struct i915_execbuffer *eb,
 		len = 3;
...
@@ -1539,7 +1589,7 @@ static bool __reloc_entry_gpu(struct i915_execbuffer *eb,
 	return true;
 }
...
+static int reloc_entry_gpu(struct i915_execbuffer *eb,
 			    ...
 			    u64 offset,
 			    u64 target_addr)
@@ -1561,8 +1611,12 @@ relocate_entry(struct i915_vma *vma,
 {
...
@@ -1928,6 +1982,10 @@ static noinline int eb_relocate_parse_slow(struct i915_execbuffer *eb)
 		goto out;
 	}
...
+
 	 ...
 	 *
@@ -1950,9 +2008,21 @@ static noinline int eb_relocate_parse_slow(struct i915_execbuffer *eb)
 		cond_resched();
 		err = 0;
 	}
+
...
+
 	list_for_each_entry(ev, &eb->relocs, reloc_link) {
 		if (!have_copy) {
 			pagefault_disable();
@@ -1967,7 +2037,10 @@ static noinline int eb_relocate_parse_slow(struct i915_execbuffer *eb)
 		}
 	}
...
@@ -1990,6 +2063,13 @@ static noinline int eb_relocate_parse_slow(struct i915_execbuffer *eb)
 	 */
...
@@ -2018,15 +2098,12 @@ static int eb_relocate_parse(struct i915_execbuffer *eb)
 {
 	int err;
...
@@ -2039,48 +2116,49 @@ static int eb_relocate_parse(struct i915_execbuffer *eb)
 				break;
 		}
...
-
 	while (i--) {
 		struct eb_vma *ev = &eb->vma[i];
 		struct i915_vma *vma = ev->vma;
@@ -2124,10 +2202,7 @@ static int eb_move_to_gpu(struct i915_execbuffer *eb)
...
-	ww_acquire_fini(&acquire);
 	...
 	if (unlikely(err))
 		goto err_skip;
@@ -2281,10 +2356,6 @@ static int eb_parse_pipeline(struct i915_execbuffer *eb,
 	pw->shadow = shadow;
...
@@ -2299,19 +2370,14 @@ static int eb_parse_pipeline(struct i915_execbuffer *eb,
...
-err_trampoline:
 ...
 err_shadow:
@@ -2326,7 +2392,7 @@ static int eb_parse_pipeline(struct i915_execbuffer *eb,
...
+	struct intel_gt_buffer_pool_node *pool = eb->batch_pool;
 	...
 	unsigned int len;
 	int err;
@@ -2349,9 +2415,16 @@ static int eb_parse(struct i915_execbuffer *eb)
...
+		goto err;
 	...
 	if (IS_ERR(shadow)) {
@@ -2387,7 +2460,6 @@ static int eb_parse(struct i915_execbuffer *eb)
...
-	shadow->private = pool;
 ...
 err_trampoline:
@@ -2396,7 +2468,6 @@ static int eb_parse(struct i915_execbuffer *eb)
 err_shadow:
...
@@ -2883,6 +2954,7 @@ i915_gem_do_execbuffer(struct drm_device *dev,
 	eb.exec = exec;
...
@@ -2942,6 +3014,14 @@ i915_gem_do_execbuffer(struct drm_device *dev,
 	if (unlikely(err))
 		goto err_context;
...
+
 	...
 	if (err) {
 		/*
@@ -2955,6 +3035,8 @@ i915_gem_do_execbuffer(struct drm_device *dev,
 		goto err_vma;
 	}
...
@@ -2975,7 +3057,7 @@ i915_gem_do_execbuffer(struct drm_device *dev,
...
+			goto err_vma;
 		...
 		batch = vma;
@@ -3027,8 +3109,8 @@ i915_gem_do_execbuffer(struct drm_device *dev,
...
@@ -3055,14 +3137,18 @@ i915_gem_do_execbuffer(struct drm_device *dev,
 err_batch_unpin:

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 365F439930C
	for <lists+intel-gfx@lfdr.de>; Wed,  2 Jun 2021 21:00:57 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id F18BD6E536;
	Wed,  2 Jun 2021 19:00:54 +0000 (UTC)
...
Subject: Re: [Intel-gfx] [PATCH v4 16/17] drm/i915/pxp: black pixels on pxp
 disabled
...
> @@ -392,6 +392,11 @@ static int bo_has_valid_encryption(const struct drm_=
i915_gem_object *obj)
...
> +{
> +	return i915_gem_object_is_protected(obj) && !bo_has_valid_encryption(ob=
j);
> +}
> +
...
>  {
> @@ -424,10 +429,14 @@ int intel_plane_atomic_check(struct intel_atomic_st=
ate *state,
>  					  crtc);
>  =

>  	fb =3D new_plane_state->hw.fb;
> -	if (fb)
> +	if (fb) {
>  		new_plane_state->decrypt =3D bo_has_valid_encryption(intel_fb_obj(fb));
> -	else
> +		new_plane_state->force_black =3D pxp_is_borked(intel_fb_obj(fb));
> +
> +	} else {
>  		new_plane_state->decrypt =3D old_plane_state->decrypt;
> +		new_plane_state->force_black =3D old_plane_state->force_black;
> +	}
>  =
...
> @@ -975,6 +975,33 @@ static u32 skl_surf_address(const struct intel_plane=
_state *plane_state,
>  	}
>  }
>  =
...
>  	/*
>  	 * FIXME: pxp session invalidation can hit any time even at time of com=
mit

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 409C65FCCF3
	for <lists+intel-gfx@lfdr.de>; Wed, 12 Oct 2022 23:19:23 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 4305810E09A;
	Wed, 12 Oct 2022 21:19:17 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga18.intel.com (mga18.intel.com [134.134.136.126])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 6127710E09A
 for <intel-gfx@lists.freedesktop.org>; Wed, 12 Oct 2022 21:19:10 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
 d=intel.com; i=@intel.com; q=dns/txt; s=Intel;
 t=1665609550; x=1697145550;
 h=message-id:date:mime-version:subject:to:cc:references:
 from:in-reply-to:content-transfer-encoding;
 bh=Yt2AkGyvtTeU/0FFzATJJXXyDTRj1i3cNNE/S80gXUY=;
 b=dLO4wq/HRohTrXlMfN7dGw/OpuvBW+7ZnZpGxzfDEZ6+dcdr6qpOx/HP
 FfWcdv2icYiJvHWLnM5w4AeIHJcG/lv+rwg2WMcKzexCTXjQGNL7MAQ1l
 +XxcpC5lBsZeaiTMeJSQ6UK6uWVdKcZ7P1heXAjR2JrVLpXjyC0Y+JytD
 2M9xLXvuudacUetvZa51zs3paCnl1Ki2u9HeXmnn/YnbyDVi2EOr1WjLj
 y9sVOAu0jZ1+G1V53Pp8KsPa92YHFqq/UqWFzauf33TNvF6AfRfIjt5Yk
 vOXc+BT4RUbCtZgPBJmvy8nJvRYzZYUk3jGu0VYPu6ZW1G4pKv6ytmf9c w==;
X-IronPort-AV: E=McAfee;i="6500,9779,10498"; a="288178594"
X-IronPort-AV: E=Sophos;i="5.95,180,1661842800"; d="scan'208";a="288178594"
Received: from fmsmga003.fm.intel.com ([10.253.24.29])
 by orsmga106.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 12 Oct 2022 14:19:01 -0700
X-IronPort-AV: E=McAfee;i="6500,9779,10498"; a="716074485"
X-IronPort-AV: E=Sophos;i="5.95,180,1661842800"; d="scan'208";a="716074485"
Received: from ahajda-mobl.ger.corp.intel.com (HELO [10.213.24.42])
 ([10.213.24.42])
 by fmsmga003-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 12 Oct 2022 14:18:59 -0700
Message-ID: <eefd907f-c9e2-811e-8303-ad970ed1d7ae@intel.com>
Date: Wed, 12 Oct 2022 23:18:57 +0200
MIME-Version: 1.0
...
References: <20221012124317.3511810-1-andrzej.hajda@intel.com>
 <a2b468a6-5cc9-6905-3360-8cdb62a5076a@intel.com>
From: Andrzej Hajda <andrzej.hajda@intel.com>
Organization: Intel Technology Poland sp. z o.o. - ul. Slowackiego 173, 80-298
 Gdansk - KRS 101882 - NIP 957-07-52-316
In-Reply-To: <a2b468a6-5cc9-6905-3360-8cdb62a5076a@intel.com>
...
>> +++ b/drivers/gpu/drm/i915/gem/selftests/i915_gem_client_blt.c
>> @@ -457,21 +457,6 @@ static int verify_buffer(const struct 
>> tiled_blits *t,
>>       return ret;
>>   }
...
>> +++ b/drivers/gpu/drm/i915/gem/selftests/i915_gem_coherency.c
>> @@ -239,9 +239,7 @@ static int gpu_set(struct context *ctx, unsigned 
>> long offset, u32 v)
>>       }
...
>>       if (err)
>>           goto skip_request;
>>   @@ -1538,11 +1534,7 @@ static int write_to_scratch(struct 
...
>>       if (err)
>>           goto skip_request;
>>   @@ -1674,11 +1666,7 @@ static int read_from_scratch(struct 
>> i915_gem_context *ctx,
>>           goto err_unpin;
>>       }
>>   -    i915_vma_lock(vma);
...
>> +++ b/drivers/gpu/drm/i915/gem/selftests/i915_gem_mman.c
>> @@ -565,10 +565,7 @@ static int make_obj_busy(struct 
...
>> -                              EXEC_OBJECT_WRITE);
>> +        err = i915_vma_move_to_active_unlocked(vma, rq, 
>> EXEC_OBJECT_WRITE);
>>             i915_request_add(rq);
>>   err_unpin:
>> @@ -1608,9 +1605,7 @@ static int __igt_mmap_gpu(struct 
...
>> +++ b/drivers/gpu/drm/i915/gt/intel_renderstate.c
>> @@ -215,9 +215,7 @@ int intel_renderstate_emit(struct 
...
>> +++ b/drivers/gpu/drm/i915/gt/intel_workarounds.c
>> @@ -3031,9 +3031,7 @@ static int engine_wa_list_verify(struct 
>> intel_context *ce,
>>           goto err_vma;
>>       }
>>   -    err = i915_request_await_object(rq, vma->obj, true);
...
>> +++ b/drivers/gpu/drm/i915/gt/selftest_execlists.c
>> @@ -2764,9 +2764,7 @@ static int create_gang(struct intel_engine_cs 
>> *engine,
>>       i915_request_get(rq);
>>         i915_vma_lock(vma);
>> -    err = i915_request_await_object(rq, vma->obj, false);
...
>>           err = rq->engine->emit_bb_start(rq,
>>                           vma->node.start,
>> @@ -3179,17 +3177,11 @@ create_gpr_client(struct intel_engine_cs 
>> *engine,
>>           goto out_batch;
>>       }
>>   -    i915_vma_lock(vma);
...
>>           err = rq->engine->emit_bb_start(rq,
>>                           batch->node.start,
>> @@ -3519,9 +3511,7 @@ static int smoke_submit(struct preempt_smoke 
>> *smoke,
>>         if (vma) {
>>           i915_vma_lock(vma);
>> -        err = i915_request_await_object(rq, vma->obj, false);
...
>>   {
>> @@ -175,11 +159,11 @@ hang_create_request(struct hang *h, struct 
...
>>       if (err)
>>           goto cancel_rq;
>>   @@ -1510,18 +1494,9 @@ static int __igt_reset_evict_vma(struct 
>> intel_gt *gt,
>>           }
>>       }
...
>> +++ b/drivers/gpu/drm/i915/gt/selftest_lrc.c
>> @@ -452,9 +452,7 @@ static int __live_lrc_state(struct 
>> intel_engine_cs *engine,
...
>> +    err = i915_vma_move_to_active_unlocked(scratch, rq, 
>> EXEC_OBJECT_WRITE);
>>         i915_request_get(rq);
>>       i915_request_add(rq);
>> @@ -601,11 +599,7 @@ __gpr_read(struct intel_context *ce, struct 
>> i915_vma *scratch, u32 *slot)
>>           *cs++ = 0;
>>       }
...
>>         i915_request_get(rq);
>>       i915_request_add(rq);
>> @@ -1053,21 +1047,6 @@ store_context(struct intel_context *ce, struct 
>> i915_vma *scratch)
>>       return batch;
>>   }
...
>>       if (err)
>>           goto err_rq;
>>   @@ -1243,7 +1222,7 @@ static int poison_registers(struct 
>> intel_context *ce, u32 poison, u32 *sema)
>>           goto err_batch;
>>       }
...
>>       if (err)
>>           goto err_req;
>>   @@ -632,16 +628,11 @@ static int check_dirty_whitelist(struct 
>> intel_context *ce)
>>                   goto err_request;
>>           }
>>   -        err = i915_request_await_object(rq, batch->obj, false);
...
>>           if (err)
>>               goto err_request;
>>   @@ -859,11 +850,7 @@ static int read_whitelisted_registers(struct 
>> intel_context *ce,
>>       if (IS_ERR(rq))
>>           return PTR_ERR(rq);
...
>>       if (err)
>>           goto err_req;
>>   @@ -943,11 +930,7 @@ static int scrub_whitelisted_registers(struct 
>> intel_context *ce)
>>               goto err_request;
>>       }
>>   -    i915_vma_lock(batch);
...
>> @@ -60,11 +70,25 @@ int __must_check _i915_vma_move_to_active(struct 
>> i915_vma *vma,
>>                         struct i915_request *rq,
...
>> +i915_vma_move_to_active_unlocked(struct i915_vma *vma, struct 
>> i915_request *rq,
>> +                 unsigned int flags)
>> +{
>> +    return i915_request_await_object(rq, vma->obj, flags & 
>> EXEC_OBJECT_WRITE) ?:
>> +           _i915_vma_move_to_active(vma, rq, &rq->fence, flags);
>
> IMO easier to read if we split over multiple lines instead of ?:

This pattern is already present in many kernel places, ex.:
crypto/hmac.c:79:       return crypto_shash_init(shash) ?:
crypto/hmac.c:80:              crypto_shash_update(shash, ipad, bs) ?:
crypto/hmac.c:81:              crypto_shash_export(shash, ipad) ?:
crypto/hmac.c:82:              crypto_shash_init(shash) ?:
crypto/hmac.c:83:              crypto_shash_update(shash, opad, bs) ?:
crypto/hmac.c-84-              crypto_shash_export(shash, opad);

Command below shows more:
# git grep -A1 -n '?:$'

but no big feelings, I will switch to sth more traditional.
...
Ups, I've messed it up during different naming attempts.

>
> Also IIUC there is no i915_request_await_object() in here, but it 
> looks like some of the callers are converted over to using this, even 
> though they previously had the await? That looks like a functional 
> change? Maybe I'm misreading something here...

Again the effects of my late changes, sorry, it should be this way 
(pseudo c):
i915_vma_move_to_active(vma, rq, flags)
{
     return i915_request_await_object(rq, vma->obj, flags & 
EXEC_OBJECT_WRITE) ?:
            _i915_vma_move_to_active(vma, rq, &rq->fence, flags);
...
i915_vma_move_to_active_unlocked(vma, rq, flags)
{
     int err;
...
And all calls swapped.

I will post fixed version then :(
...
>> @@ -165,11 +149,11 @@ igt_spinner_create_request(struct igt_spinner 
>> *spin,
>>       if (IS_ERR(rq))
>>           return ERR_CAST(rq);
>>   -    err = move_to_active(vma, rq, 0);

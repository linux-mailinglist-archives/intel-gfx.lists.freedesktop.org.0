...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id E23931BF4EE
	for <lists+intel-gfx@lfdr.de>; Thu, 30 Apr 2020 12:08:27 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id C510A6E1CD;
	Thu, 30 Apr 2020 10:08:25 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga01.intel.com (mga01.intel.com [192.55.52.88])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 17C106E1CD
 for <intel-gfx@lists.freedesktop.org>; Thu, 30 Apr 2020 10:08:23 +0000 (UTC)
IronPort-SDR: q1I8lkQPzB64GLkMvMPwxH5y3G6DUSQZHuV49lUwPfYulOYAGRDOhSc7x5KgpocgJ1zX8RjZYZ
 zSgeCbbI+eAA==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga006.jf.intel.com ([10.7.209.51])
 by fmsmga101.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 30 Apr 2020 03:08:23 -0700
IronPort-SDR: Mnzp+iu3B65cGJP7ktpH8HyUd4w5+e1IOGC117eg51nFJeJgthO4u9UDP4qXw5gnfXw5qjx02y
 jcsQUadZ1YKQ==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,334,1583222400"; d="scan'208";a="261727829"
Received: from stinkbox.fi.intel.com (HELO stinkbox) ([10.237.72.174])
 by orsmga006.jf.intel.com with SMTP; 30 Apr 2020 03:08:20 -0700
Received: by stinkbox (sSMTP sendmail emulation);
 Thu, 30 Apr 2020 13:08:20 +0300
Date: Thu, 30 Apr 2020 13:08:20 +0300
From: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
To: "Lisovskiy, Stanislav" <stanislav.lisovskiy@intel.com>
Message-ID: <20200430100820.GQ6112@intel.com>
...
In-Reply-To: <20200430095242.GA2558@intel.com>
...
> > > > What I meant is that we need both. Serialize if sagv state is going=
 to
> > > > change, otherwise lock if the mask changes.
> > > =

> > > As I understand whenever we modify global state but not a real hw, we=
 do
> > > only global state locking - pipe sagv mask is not actually a hw, but =
just
> > > a virtual thing. It affects the QGV points we enable and if it happen=
s to
...
> like intel_lock_or_serialize_state(state, bool global_state_changed, bool=
 hw_state_changed) =
...
> > > > > @@ -3760,34 +3761,75 @@ intel_disable_sagv(struct drm_i915_privat=
e *dev_priv)
> > > > >  void intel_sagv_pre_plane_update(struct intel_atomic_state *stat=
e)
> > > > >  {
...
> > > > > +	 * This is different from situation when we have SAGV but just =
can't
> > > > > +	 * afford it due to DBuf limitation - in case if SAGV is comple=
tely
> > > > > +	 * disabled in a BIOS, we are not even allowed to send a PCode =
request,
...
> > > > >  void intel_sagv_post_plane_update(struct intel_atomic_state *sta=
te)
> > > > >  {
> > > > >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
> > > > > +	const struct intel_bw_state *new_bw_state;
...
> > > > >  static bool intel_crtc_can_enable_sagv(const struct intel_crtc_s=
tate *crtc_state)
> > > > >  {
...
> > > > > @@ -3803,7 +3845,7 @@ static bool intel_crtc_can_enable_sagv(cons=
...
> > > > > @@ -3819,35 +3861,44 @@ static bool intel_crtc_can_enable_sagv(co=
...
> > > > > diff --git a/drivers/gpu/drm/i915/intel_pm.h b/drivers/gpu/drm/i9=
15/intel_pm.h

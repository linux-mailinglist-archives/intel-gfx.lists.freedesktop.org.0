...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id E1E553E284A
	for <lists+intel-gfx@lfdr.de>; Fri,  6 Aug 2021 12:11:09 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id A6E406EB7C;
	Fri,  6 Aug 2021 10:11:06 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga17.intel.com (mga17.intel.com [192.55.52.151])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 94C776EB7C;
 Fri,  6 Aug 2021 10:11:05 +0000 (UTC)
X-IronPort-AV: E=McAfee;i="6200,9189,10067"; a="194614982"
X-IronPort-AV: E=Sophos;i="5.84,300,1620716400"; d="scan'208";a="194614982"
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
 by fmsmga107.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 06 Aug 2021 03:11:01 -0700
X-IronPort-AV: E=Sophos;i="5.84,300,1620716400"; d="scan'208";a="523406754"
Received: from imccann-mobl.ger.corp.intel.com (HELO localhost) ([10.252.9.82])
 by fmsmga002-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 06 Aug 2021 03:10:55 -0700
Content-Type: text/plain; charset="utf-8"
MIME-Version: 1.0
Content-Transfer-Encoding: quoted-printable
In-Reply-To: <YQ0JmYiXhGskNcrI@jlahtine-mobl.ger.corp.intel.com>
References: <YQ0JmYiXhGskNcrI@jlahtine-mobl.ger.corp.intel.com>
...
Message-ID: <162824465321.37441.8423033784821160549@jlahtine-mobl.ger.corp.intel.com>
User-Agent: alot/0.8.1
Date: Fri, 06 Aug 2021 13:10:53 +0300
...
>=20
> As an example is dropping of faster GPU relocation path for older platfor=
ms,
> which should be mitigated by updating to the latest UMD versions to regain
> the perf.
>=20
> This PR drops various bits of uAPI where userspace has dropped the ball a=
fter reviews
> have been completed on both sides (Thanks Jason for doing the due-diligen=
ce!). [1]
> Due to the complexity of tracing back who used what, I think we could do =
better in the
> future to avoid such situations to begin with. See below for my suggestio=
n [2].
>=20
> In addition to the refactoring and uAPI cleanup there is preliminay code =
for
...
>=20
> I915_MMAP_OFFSET_FIXED is also added to to align with the static/fixed ca=
ching
> mode per BO instead of per-mapping mode (for dGFX only). There is GuC fir=
mware
> interface update and backend code major rework that unblock enabling GuC =
on Gen11
> (not on by default). Then there is the addition of the GuCRC power manage=
ment
...
>=20
> I think that is mostly all. I tried to summarize much in the tag descript=
ion so
> it should hopefully not be horribly long...
>=20
> Regards, Joonas
>=20
> [1] Given that Jason is only human and there is no way to automate this a=
nalysis, we
> may have to bring something back as fixes if we find breakage (like with =
the MMAP IOCTL).
>=20
> [2] As we first require to merge the kernel code, should we introduce som=
e further rules
...
patch as possible
> in the final release candidate and then remove it in next release. Though=
ts?
>=20
> ***
>=20
...
>=20
>   Userspace: https://gitlab.freedesktop.org/mesa/mesa/-/merge_requests/11=
888
>=20
> - Reinstate the mmap ioctl for (already released) integrated Gen12 platfo=
rms
>=20
>   Rationale: Otherwise media driver breaks eg. for ADL-P. Long term goal =
is
>   still to sunset the IOCTL even for integrated and require using mmap_of=
fset.
>=20
...
>=20
> - Disallow changing context parameters after first use on Gen12 and earli=
er
> - Require setting context parameters at creation on platforms after Gen12
>=20
>   Rationale (for both): Allow less dynamic changes to the context to simp=
lify
...
> - Probe existence of backing struct pages upon userptr creation (Chris, M=
att A)
>=20
> - Add an intermediate GEM proto-context to delay real context creation (J=
ason)
...
> - Correct the locking and pin pattern for dma-buf (Thomas H, Michael R, J=
ason)
> - Migrate to system at dma-buf attach time (Thomas, Michael R)
>=20
> - MAJOR refactoring of the GuC backend code to allow for enabling on Gen1=
1+
...
>   John, Tvrtko)
> - Use the correct IRQ during resume and eliminate DRM IRQ midlayer (Thoma=
s Z)
...
> - Add i915_sched_engine to abstract a submission queue between backends (=
Matt B)
...
>       drm/i915/userptr: Probe existence of backing struct pages upon crea=
tion
>=20
> Clint Taylor (1):
...
>       drm/i915/gem: Set the watchdog timeout directly in intel_context_se=
t_gem (v2)
...
>       drm/i915/gem: Don't allow changing the engine set on running contex=
ts (v3)
>       drm/i915/selftests: Take a VM in kernel_context()
>       i915/gem/selftests: Assign the VM at context creation in igt_shared=
_ctx_exec
...
>       Revert "drm/i915: Propagate errors on awaiting already signaled fen=
ces"
...
(v2)
>       drm/i915/gem: Call i915_gem_flush_free_objects() in i915_gem_dumb_c=
reate()
...
>=20
> Joonas Lahtinen (1):
>       Merge tag 'topic/i915-ttm-2021-06-11' of git://anongit.freedesktop.=
org/drm/drm-misc into drm-intel-gt-next
>=20
...
>       drm/i915/icl: Drop workarounds that only apply to pre-production st=
eppings
...
>       Merge branch 'topic/xehp-dg2-definitions-2021-07-21' into drm-intel=
-gt-next
...
>       drm/i915/gem: Introduce a selftest for the gem object migrate funct=
ionality
...
>       drm/i915/ttm: Fix incorrect assumptions about ttm_bo_validate() sem=
antics
...
>  delete mode 100644 drivers/gpu/drm/i915/gem/selftests/i915_gem_object_bl=
t.c

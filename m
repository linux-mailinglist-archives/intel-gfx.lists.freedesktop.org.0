...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id CFD356AD7B5
	for <lists+intel-gfx@lfdr.de>; Tue,  7 Mar 2023 07:55:01 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 9C4A810E368;
	Tue,  7 Mar 2023 06:54:54 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga18.intel.com (mga18.intel.com [134.134.136.126])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 3D8C610E368
 for <intel-gfx@lists.freedesktop.org>; Tue,  7 Mar 2023 06:54:53 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
 d=intel.com; i=@intel.com; q=dns/txt; s=Intel;
 t=1678172093; x=1709708093;
 h=from:to:cc:subject:date:message-id:in-reply-to:
 references:mime-version:content-transfer-encoding;
 bh=PGIieI6sZYD+RLV0KLjsDZTjHyZctpOslxZGzcAJdmw=;
 b=UfORpLtNfQXYYMu5fPtkvEK7RLq1/ItVAT+xRkTlXn05C+kZe+W5Egj2
 I+LJkxqPP0l5FBTrxSiAjl/nkOcy4D6OPIvCfuXGpkIlxQefN8cOutxey
 MV+dA1jIn8OvxAqy1CY2K3hpFpbMpjoCt0IrgtxwIEljMmxQmaBxpH8i5
 3JSBXb1prwg8vvgSmR7zm9y5DKBq0UicCRLGblvZqYFswUiVYD2GbLNIv
 XFF+pDK9H1G08AI0qcnq2Q6/0f4MPoe8AEXWArwG4dsj8juKzR3nngAhC
 P6OMDF58s2eLWBiIXlXVrm7qf9DFwbvkeyDM7P6kqZXhHalLNfBs+4d1J g==;
X-IronPort-AV: E=McAfee;i="6500,9779,10641"; a="319609517"
X-IronPort-AV: E=Sophos;i="5.98,240,1673942400"; d="scan'208";a="319609517"
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
 by orsmga106.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 06 Mar 2023 22:54:52 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=McAfee;i="6500,9779,10641"; a="786579582"
X-IronPort-AV: E=Sophos;i="5.98,240,1673942400"; d="scan'208";a="786579582"
Received: from kandpal-x299-ud4-pro.iind.intel.com ([10.190.239.32])
 by fmsmga002.fm.intel.com with ESMTP; 06 Mar 2023 22:54:49 -0800
From: Suraj Kandpal <suraj.kandpal@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Tue,  7 Mar 2023 12:23:52 +0530
Message-Id: <20230307065356.181278-3-suraj.kandpal@intel.com>
...
Subject: [Intel-gfx] [PATCH v11 2/6] drm/i915/hdcp: Use generic names for
...
index 2984d2810e42..ded15cfd2e91 100644
--- a/drivers/gpu/drm/i915/display/intel_hdcp.c
+++ b/drivers/gpu/drm/i915/display/intel_hdcp.c
@@ -1142,7 +1142,7 @@ hdcp2_prepare_ake_init(struct intel_connector *connector,
...
@@ -1153,7 +1153,7 @@ hdcp2_prepare_ake_init(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->initiate_hdcp2_session(comp->hdcp_dev, data, ake_data);
 	if (ret)
 		drm_dbg_kms(&dev_priv->drm, "Prepare_ake_init failed. %d\n",
 			    ret);
@@ -1172,7 +1172,7 @@ hdcp2_verify_rx_cert_prepare_km(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1183,7 +1183,7 @@ hdcp2_verify_rx_cert_prepare_km(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->verify_receiver_cert_prepare_km(comp->hdcp_dev, data,
 							 ...
 							 ek_pub_km, msg_sz);
 	if (ret < 0)
@@ -1200,7 +1200,7 @@ static int hdcp2_verify_hprime(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1211,7 +1211,7 @@ static int hdcp2_verify_hprime(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
@@ -1226,7 +1226,7 @@ hdcp2_store_pairing_info(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1237,7 +1237,7 @@ hdcp2_store_pairing_info(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->store_pairing_info(comp->hdcp_dev, data, pairing_info);
 	if (ret < 0)
 		drm_dbg_kms(&dev_priv->drm, "Store pairing info failed. %d\n",
 			    ret);
@@ -1253,7 +1253,7 @@ hdcp2_prepare_lc_init(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1264,7 +1264,7 @@ hdcp2_prepare_lc_init(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->initiate_locality_check(comp->hdcp_dev, data, lc_init);
 	if (ret < 0)
 		drm_dbg_kms(&dev_priv->drm, "Prepare lc_init failed. %d\n",
 			    ret);
@@ -1280,7 +1280,7 @@ hdcp2_verify_lprime(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1291,7 +1291,7 @@ hdcp2_verify_lprime(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->verify_lprime(comp->hdcp_dev, data, rx_lprime);
 	if (ret < 0)
 		drm_dbg_kms(&dev_priv->drm, "Verify L_Prime failed. %d\n",
 			    ret);
@@ -1306,7 +1306,7 @@ static int hdcp2_prepare_skey(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1317,7 +1317,7 @@ static int hdcp2_prepare_skey(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->get_session_key(comp->hdcp_dev, data, ske_data);
 	if (ret < 0)
 		drm_dbg_kms(&dev_priv->drm, "Get session key failed. %d\n",
 			    ret);
@@ -1335,7 +1335,7 @@ hdcp2_verify_rep_topology_prepare_ack(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1346,7 +1346,7 @@ hdcp2_verify_rep_topology_prepare_ack(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->repeater_check_flow_prepare_ack(comp->hdcp_dev, data,
 							 ...
 							 rep_send_ack);
 	if (ret < 0)
@@ -1364,7 +1364,7 @@ hdcp2_verify_mprime(struct intel_connector *connector,
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1375,7 +1375,7 @@ hdcp2_verify_mprime(struct intel_connector *connector,
 		return -EINVAL;
 	}
...
+	ret = comp->ops->verify_mprime(comp->hdcp_dev, data, stream_ready);
 	...
 	mutex_unlock(&dev_priv->display.hdcp.comp_mutex);
@@ -1388,7 +1388,7 @@ static int hdcp2_authenticate_port(struct intel_connector *connector)
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct hdcp_port_data *data = &dig_port->hdcp_port_data;
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1399,7 +1399,7 @@ static int hdcp2_authenticate_port(struct intel_connector *connector)
 		return -EINVAL;
 	}
...
+	ret = comp->ops->enable_hdcp_authentication(comp->hdcp_dev, data);
 	if (ret < 0)
 		drm_dbg_kms(&dev_priv->drm, "Enable hdcp auth failed. %d\n",
 			    ret);
@@ -1412,7 +1412,7 @@ static int hdcp2_close_mei_session(struct intel_connector *connector)
 {
 	struct intel_digital_port *dig_port = intel_attached_dig_port(connector);
 	struct drm_i915_private *dev_priv = to_i915(connector->base.dev);
-	struct i915_hdcp_comp_master *comp;
+	struct i915_hdcp_master *comp;
 	...
 	mutex_lock(&dev_priv->display.hdcp.comp_mutex);
@@ -1423,7 +1423,7 @@ static int hdcp2_close_mei_session(struct intel_connector *connector)
 		return -EINVAL;
 	}
...
@@ -2142,8 +2142,8 @@ static int i915_hdcp_component_bind(struct device *i915_kdev,
...
+	dev_priv->display.hdcp.master->hdcp_dev = mei_kdev;
 	...
 	return 0;
@@ -2160,30 +2160,30 @@ static void i915_hdcp_component_unbind(struct device *i915_kdev,
 	mutex_unlock(&dev_priv->display.hdcp.comp_mutex);
 }
...
@@ -2197,20 +2197,20 @@ static int initialize_hdcp_port_data(struct intel_connector *connector,
...
@@ -2253,7 +2253,7 @@ void intel_hdcp_component_init(struct drm_i915_private *dev_priv)
...
@@ -2347,7 +2347,8 @@ int intel_hdcp_enable(struct intel_connector *connector,
 	}
...
@@ -2482,7 +2483,7 @@ void intel_hdcp_component_fini(struct drm_i915_private *dev_priv)

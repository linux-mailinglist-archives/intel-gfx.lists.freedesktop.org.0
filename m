...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 619FA129BCB
	for <lists+intel-gfx@lfdr.de>; Tue, 24 Dec 2019 00:23:37 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id B04CC89ECD;
	Mon, 23 Dec 2019 23:23:35 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga04.intel.com (mga04.intel.com [192.55.52.120])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 4988F89ECD
 for <intel-gfx@lists.freedesktop.org>; Mon, 23 Dec 2019 23:23:34 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from orsmga004.jf.intel.com ([10.7.209.38])
 by fmsmga104.fm.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 23 Dec 2019 15:23:33 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.69,349,1571727600"; d="scan'208";a="367180994"
Received: from mdroper-desk1.fm.intel.com (HELO
 mdroper-desk1.amr.corp.intel.com) ([10.1.27.64])
 by orsmga004.jf.intel.com with ESMTP; 23 Dec 2019 15:23:33 -0800
Date: Mon, 23 Dec 2019 15:23:33 -0800
From: Matt Roper <matthew.d.roper@intel.com>
To: Lucas De Marchi <lucas.demarchi@intel.com>
Message-ID: <20191223232333.GU2877816@mdroper-desk1.amr.corp.intel.com>
References: <20191223173244.30742-1-lucas.demarchi@intel.com>
 <20191223173244.30742-8-lucas.demarchi@intel.com>
 <20191223231330.GP2877816@mdroper-desk1.amr.corp.intel.com>
 <20191223232255.j63s2xllbu5yh5so@ldmartin-desk1>
MIME-Version: 1.0
Content-Disposition: inline
In-Reply-To: <20191223232255.j63s2xllbu5yh5so@ldmartin-desk1>
...
> > > diff --git a/drivers/gpu/drm/i915/display/intel_crt.c b/drivers/gpu/d=
rm/i915/display/intel_crt.c
> > > index b2b1336ecdb6..cbe5978e7fb5 100644
> > > --- a/drivers/gpu/drm/i915/display/intel_crt.c
> > > +++ b/drivers/gpu/drm/i915/display/intel_crt.c
> > > @@ -247,7 +247,7 @@ static void hsw_post_disable_crt(struct intel_enc=
oder *encoder,
> > > =
...
> > > @@ -351,7 +351,7 @@ intel_crt_mode_valid(struct drm_connector *connec=
tor,
> > > =
...
> > > @@ -427,7 +427,7 @@ static int hsw_crt_compute_config(struct intel_en=
coder *encoder,
> > >  	return 0;
> > >  }
> > > =
...
> > > @@ -535,7 +535,7 @@ static bool intel_crt_detect_hotplug(struct drm_c=
onnector *connector)
> > >  	int i, tries =3D 0;
> > > =
...
> > > @@ -3898,7 +3898,7 @@ static void intel_ddi_post_disable(struct intel=
_encoder *encoder,
...
> > > @@ -157,7 +157,7 @@ static void intel_cpu_transcoder_set_m_n(const st=
ruct intel_crtc_state *crtc_sta
> > >  					 const struct intel_link_m_n *m_n,
> > >  					 const struct intel_link_m_n *m2_n2);
> > >  static void i9xx_set_pipeconf(const struct intel_crtc_state *crtc_st=
ate);
> > > -static void ironlake_set_pipeconf(const struct intel_crtc_state *crt=
c_state);
> > > +static void ilk_set_pipeconf(const struct intel_crtc_state *crtc_sta=
te);
> > >  static void hsw_set_pipeconf(const struct intel_crtc_state *crtc_sta=
te);
> > >  static void bdw_set_pipemisc(const struct intel_crtc_state *crtc_sta=
te);
> > >  static void vlv_prepare_pll(struct intel_crtc *crtc,
> > > @@ -165,7 +165,7 @@ static void vlv_prepare_pll(struct intel_crtc *cr=
tc,
> > >  static void chv_prepare_pll(struct intel_crtc *crtc,
> > >  			    const struct intel_crtc_state *pipe_config);
> > >  static void skl_pfit_enable(const struct intel_crtc_state *crtc_stat=
e);
> > > -static void ironlake_pfit_enable(const struct intel_crtc_state *crtc=
_state);
> > > +static void ilk_pfit_enable(const struct intel_crtc_state *crtc_stat=
e);
> > >  static void intel_modeset_setup_hw_state(struct drm_device *dev,
> > >  					 struct drm_modeset_acquire_ctx *ctx);
> > >  static struct intel_crtc_state *intel_crtc_state_alloc(struct intel_=
crtc *crtc);
...
> > > @@ -415,7 +415,7 @@ static const struct intel_limit intel_limits_iron=
lake_dac =3D {
...
> > > @@ -428,7 +428,7 @@ static const struct intel_limit intel_limits_iron=
lake_single_lvds =3D {
...
> > >  	.vco =3D { .min =3D 1760000, .max =3D 3510000 },
> > >  	.n =3D { .min =3D 1, .max =3D 3 },
> > > @@ -442,7 +442,7 @@ static const struct intel_limit intel_limits_iron=
lake_dual_lvds =3D {
> > >  };
> > > =

> > >  /* LVDS 100mhz refclk limits. */
> > > -static const struct intel_limit intel_limits_ironlake_single_lvds_10=
0m =3D {
> > > +static const struct intel_limit ilk_limits_single_lvds_100m =3D {
> > >  	.dot =3D { .min =3D 25000, .max =3D 350000 },
> > >  	.vco =3D { .min =3D 1760000, .max =3D 3510000 },
> > >  	.n =3D { .min =3D 1, .max =3D 2 },
> > > @@ -455,7 +455,7 @@ static const struct intel_limit intel_limits_iron=
lake_single_lvds_100m =3D {
> > >  		.p2_slow =3D 14, .p2_fast =3D 14 },
> > >  };
> > > =
...
> > >  	.vco =3D { .min =3D 1760000, .max =3D 3510000 },
> > >  	.n =3D { .min =3D 1, .max =3D 3 },
> > > @@ -1637,7 +1637,7 @@ void vlv_wait_port_ready(struct drm_i915_privat=
e *dev_priv,
> > >  		     I915_READ(dpll_reg) & port_mask, expected_mask);
> > >  }
> > > =

> > > -static void ironlake_enable_pch_transcoder(const struct intel_crtc_s=
tate *crtc_state)
...
> > > @@ -1735,8 +1735,8 @@ static void lpt_enable_pch_transcoder(struct dr=
m_i915_private *dev_priv,
> > >  		DRM_ERROR("Failed to enable PCH transcoder\n");
> > >  }
> > > =

> > > -static void ironlake_disable_pch_transcoder(struct drm_i915_private =
*dev_priv,
> > > -					    enum pipe pipe)
> > > +static void ilk_disable_pch_transcoder(struct drm_i915_private *dev_=
priv,
> > > +				       enum pipe pipe)
> > >  {
> > >  	i915_reg_t reg;
> > >  	u32 val;
> > > @@ -4869,8 +4869,8 @@ static void intel_fdi_normal_train(struct intel=
_crtc *crtc)
> > >  }
> > > =
...
> > > @@ -5222,7 +5222,7 @@ static void ivb_manual_fdi_link_train(struct in=
tel_crtc *crtc,
> > >  	DRM_DEBUG_KMS("FDI train done.\n");
> > >  }
> > > =

> > > -static void ironlake_fdi_pll_enable(const struct intel_crtc_state *c=
rtc_state)
> > > +static void ilk_fdi_pll_enable(const struct intel_crtc_state *crtc_s=
tate)
> > >  {
> > >  	struct intel_crtc *intel_crtc =3D to_intel_crtc(crtc_state->uapi.cr=
tc);
> > >  	struct drm_i915_private *dev_priv =3D to_i915(intel_crtc->base.dev);
> > > @@ -5259,7 +5259,7 @@ static void ironlake_fdi_pll_enable(const struc=
...
> > >  	struct drm_i915_private *dev_priv =3D to_i915(dev);
> > > @@ -5289,7 +5289,7 @@ static void ironlake_fdi_pll_disable(struct int=
el_crtc *intel_crtc)
> > >  	udelay(100);
> > >  }
> > > =
...
> > > @@ -5496,8 +5496,8 @@ int lpt_get_iclkip(struct drm_i915_private *dev=
_priv)
> > >  				 desired_divisor << auxdiv);
> > >  }
> > > =

> > > -static void ironlake_pch_transcoder_set_timings(const struct intel_c=
rtc_state *crtc_state,
> > > -						enum pipe pch_transcoder)
> > > +static void ilk_pch_transcoder_set_timings(const struct intel_crtc_s=
tate *crtc_state,
> > > +					   enum pipe pch_transcoder)
> > >  {
> > >  	struct intel_crtc *crtc =3D to_intel_crtc(crtc_state->uapi.crtc);
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -5601,8 +5601,8 @@ intel_get_crtc_new_encoder(const struct intel_a=
tomic_state *state,
> > >   *   - DP transcoding bits
> > >   *   - transcoder
> > >   */
> > > -static void ironlake_pch_enable(const struct intel_atomic_state *sta=
te,
> > > -				const struct intel_crtc_state *crtc_state)
...
> > >  	struct intel_crtc *crtc =3D to_intel_crtc(crtc_state->uapi.crtc);
> > >  	struct drm_device *dev =3D crtc->base.dev;
> > > @@ -5650,7 +5650,7 @@ static void ironlake_pch_enable(const struct in=
...
> > > @@ -5682,7 +5682,7 @@ static void ironlake_pch_enable(const struct in=
tel_atomic_state *state,
> > >  		I915_WRITE(reg, temp);
> > >  	}
> > > =
...
> > > @@ -5697,7 +5697,7 @@ static void lpt_pch_enable(const struct intel_a=
tomic_state *state,
...
> > >  }
> > > @@ -6047,7 +6047,7 @@ static void skl_pfit_enable(const struct intel_=
...
> > >  	struct intel_crtc *crtc =3D to_intel_crtc(crtc_state->uapi.crtc);
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -6643,8 +6643,8 @@ static void intel_disable_primary_plane(const s=
truct intel_crtc_state *crtc_stat
...
> > > @@ -6680,7 +6680,7 @@ static void ironlake_crtc_enable(struct intel_a=
tomic_state *state,
...
> > > @@ -6690,13 +6690,13 @@ static void ironlake_crtc_enable(struct intel=
_atomic_state *state,
...
> > > @@ -6712,7 +6712,7 @@ static void ironlake_crtc_enable(struct intel_a=
tomic_state *state,
...
> > > @@ -6846,7 +6846,7 @@ static void hsw_crtc_enable(struct intel_atomic=
_state *state,
> > >  	if (INTEL_GEN(dev_priv) >=3D 9)
...
> > >  	/*
> > >  	 * On ILK+ LUT must be loaded before the pipe is running but with
> > > @@ -6895,7 +6895,7 @@ static void hsw_crtc_enable(struct intel_atomic=
_state *state,
> > >  	}
> > >  }
> > > =

> > > -void ironlake_pfit_disable(const struct intel_crtc_state *old_crtc_s=
tate)
...
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -6910,8 +6910,8 @@ void ironlake_pfit_disable(const struct intel_c=
...
> > > @@ -6932,15 +6932,15 @@ static void ironlake_crtc_disable(struct inte=
...
> > >  			i915_reg_t reg;
> > > @@ -6960,7 +6960,7 @@ static void ironlake_crtc_disable(struct intel_=
atomic_state *state,
...
> > > @@ -7505,8 +7505,8 @@ static int pipe_required_fdi_lanes(struct intel=
_crtc_state *crtc_state)
> > >  	return 0;
> > >  }
> > > =

> > > -static int ironlake_check_fdi_lanes(struct drm_device *dev, enum pip=
e pipe,
> > > -				     struct intel_crtc_state *pipe_config)
> > > +static int ilk_check_fdi_lanes(struct drm_device *dev, enum pipe pip=
e,
> > > +			       struct intel_crtc_state *pipe_config)
> > >  {
> > >  	struct drm_i915_private *dev_priv =3D to_i915(dev);
> > >  	struct drm_atomic_state *state =3D pipe_config->uapi.state;
> > > @@ -7578,8 +7578,8 @@ static int ironlake_check_fdi_lanes(struct drm_=
device *dev, enum pipe pipe,
> > >  }
> > > =
...
djusted_mode;
> > > @@ -7598,15 +7598,15 @@ static int ironlake_fdi_compute_config(struct=
...
> > > @@ -7812,7 +7812,7 @@ static int intel_crtc_compute_config(struct int=
el_crtc *crtc,
...
> > >  	return 0;
> > >  }
> > > @@ -9224,7 +9224,7 @@ static bool i9xx_get_pipe_config(struct intel_c=
rtc *crtc,
> > >  	return ret;
> > >  }
> > > =

> > > -static void ironlake_init_pch_refclk(struct drm_i915_private *dev_pr=
iv)
> > > +static void ilk_init_pch_refclk(struct drm_i915_private *dev_priv)
> > >  {
> > >  	struct intel_encoder *encoder;
> > >  	int i;
> > > @@ -9722,12 +9722,12 @@ static void lpt_init_pch_refclk(struct drm_i9=
15_private *dev_priv)
...
> > >  	struct intel_crtc *crtc =3D to_intel_crtc(crtc_state->uapi.crtc);
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -9871,7 +9871,7 @@ int bdw_get_pipemisc_bpp(struct intel_crtc *crt=
c)
> > >  	}
> > >  }
> > > =

> > > -int ironlake_get_lanes_required(int target_clock, int link_bw, int b=
pp)
> > > +int ilk_get_lanes_required(int target_clock, int link_bw, int bpp)
> > >  {
> > >  	/*
> > >  	 * Account for spread spectrum to avoid
> > > @@ -9882,14 +9882,14 @@ int ironlake_get_lanes_required(int target_cl=
ock, int link_bw, int bpp)
...
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > >  	u32 dpll, fp, fp2;
> > > @@ -9909,7 +9909,7 @@ static void ironlake_compute_dpll(struct intel_=
crtc *crtc,
> > > =
...
> > >  	if (reduced_clock) {
> > > @@ -9989,8 +9989,8 @@ static void ironlake_compute_dpll(struct intel_=
crtc *crtc,
...
> > > @@ -10014,17 +10014,17 @@ static int ironlake_crtc_compute_clock(stru=
ct intel_crtc *crtc,
> > > =
...
> > > @@ -10034,7 +10034,7 @@ static int ironlake_crtc_compute_clock(struct=
 intel_crtc *crtc,
> > >  		return -EINVAL;
> > >  	}
> > > =
...
> > > @@ -10276,8 +10276,8 @@ skl_get_initial_plane_config(struct intel_crt=
c *crtc,
> > >  	kfree(intel_fb);
> > >  }
> > > =
...
> > >  	struct drm_device *dev =3D crtc->base.dev;
> > >  	struct drm_i915_private *dev_priv =3D to_i915(dev);
> > > @@ -10300,8 +10300,8 @@ static void ironlake_get_pfit_config(struct i=
ntel_crtc *crtc,
> > >  	}
> > >  }
> > > =
...
> > >  	struct drm_device *dev =3D crtc->base.dev;
> > >  	struct drm_i915_private *dev_priv =3D to_i915(dev);
> > > @@ -10372,7 +10372,7 @@ static bool ironlake_get_pipe_config(struct i=
ntel_crtc *crtc,
...
> > >  			/*
> > > @@ -10400,7 +10400,7 @@ static bool ironlake_get_pipe_config(struct i=
ntel_crtc *crtc,
...
> > >  	}
> > > @@ -10408,7 +10408,7 @@ static bool ironlake_get_pipe_config(struct i=
ntel_crtc *crtc,
...
> > > @@ -10769,7 +10769,7 @@ static void hsw_get_ddi_port_state(struct int=
el_crtc *crtc,
> > >  		pipe_config->fdi_lanes =3D ((FDI_DP_PORT_WIDTH_MASK & tmp) >>
> > >  					  FDI_DP_PORT_WIDTH_SHIFT) + 1;
> > > =
...
> > > @@ -10921,7 +10921,7 @@ static bool hsw_get_pipe_config(struct intel_=
crtc *crtc,
> > >  		if (INTEL_GEN(dev_priv) >=3D 9)
...
> > > @@ -14472,9 +14472,9 @@ static void intel_pipe_fastset(const struct i=
...
> > > @@ -16826,13 +16826,13 @@ void intel_init_display_hooks(struct drm_i9=
15_private *dev_priv)
...
> > >  		dev_priv->display.get_initial_plane_config =3D
> > > @@ -16878,7 +16878,7 @@ void intel_init_display_hooks(struct drm_i915=
_private *dev_priv)
> > >  	}
> > > =
...
> > > @@ -521,7 +521,7 @@ int intel_get_pipe_from_crtc_id_ioctl(struct drm_=
...
> > > @@ -579,7 +579,7 @@ void intel_crtc_arm_fifo_underrun(struct intel_cr=
tc *crtc,
> > >  u16 skl_scaler_calc_phase(int sub, int scale, bool chroma_center);
> > >  int skl_update_scaler_crtc(struct intel_crtc_state *crtc_state);
> > >  void skl_scaler_disable(const struct intel_crtc_state *old_crtc_stat=
e);
> > > -void ironlake_pfit_disable(const struct intel_crtc_state *old_crtc_s=
tate);
> > > +void ilk_pfit_disable(const struct intel_crtc_state *old_crtc_state);
...
> > >  u32 glk_plane_color_ctl_crtc(const struct intel_crtc_state *crtc_sta=
te);
...
> > > @@ -2509,7 +2509,7 @@ static void intel_dp_prepare(struct intel_encod=
er *encoder,
> > >  	 *
...
> > > @@ -2653,7 +2653,7 @@ static void edp_wait_backlight_off(struct intel=
_dp *intel_dp)
> > >   * is locked
> > >   */
> > > =
...
> > >  	u32 control;
> > > @@ -2703,7 +2703,7 @@ static bool edp_panel_vdd_on(struct intel_dp *i=
ntel_dp)
...
> > > @@ -2768,7 +2768,7 @@ static void edp_panel_vdd_off_sync(struct intel=
_dp *intel_dp)
...
> > > @@ -2864,7 +2864,7 @@ static void edp_panel_on(struct intel_dp *intel=
_dp)
> > >  	wait_panel_power_cycle(intel_dp);
> > > =
...
> > > @@ -2919,7 +2919,7 @@ static void edp_panel_off(struct intel_dp *inte=
l_dp)
> > >  	WARN(!intel_dp->want_panel_vdd, "Need [ENCODER:%d:%s] VDD to turn o=
ff panel\n",
...
> > > @@ -2968,7 +2968,7 @@ static void _intel_edp_backlight_on(struct inte=
l_dp *intel_dp)
...
> > > @@ -3004,7 +3004,7 @@ static void _intel_edp_backlight_off(struct int=
el_dp *intel_dp)
> > >  		i915_reg_t pp_ctrl_reg =3D _pp_ctrl_reg(intel_dp);
> > >  		u32 pp;
> > > =
...
> > >  		I915_WRITE(pp_ctrl_reg, pp);
> > > @@ -3042,7 +3042,7 @@ static void intel_edp_backlight_power(struct in=
...
> > > @@ -3079,8 +3079,8 @@ static void assert_edp_pll(struct drm_i915_priv=
ate *dev_priv, bool state)
...
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -3119,8 +3119,8 @@ static void ironlake_edp_pll_on(struct intel_dp=
 *intel_dp,
> > >  	udelay(200);
> > >  }
> > > =
...
> > >  	struct drm_i915_private *dev_priv =3D to_i915(crtc->base.dev);
> > > @@ -3410,7 +3410,7 @@ static void g4x_post_disable_dp(struct intel_en=
coder *encoder,
> > > =
...
> > > @@ -3615,7 +3615,7 @@ static void g4x_pre_enable_dp(struct intel_enco=
der *encoder,
> > > =
...
> > > @@ -6693,7 +6693,7 @@ intel_pps_readout_hw_state(struct intel_dp *int=
el_dp, struct edp_power_seq *seq)
> > > =
...
> > >  	if (!HAS_DDI(dev_priv))
> > > @@ -6863,7 +6863,7 @@ intel_dp_init_panel_power_sequencer_registers(s=
truct intel_dp *intel_dp,
...
> > > @@ -250,7 +250,7 @@ static void intel_mst_post_disable_dp(struct inte=
l_encoder *encoder,
> > >  	if (INTEL_GEN(dev_priv) >=3D 9)
> > >  		skl_scaler_disable(old_crtc_state);
> > >  	else
...
> > > @@ -126,8 +126,8 @@ static void i9xx_set_fifo_underrun_reporting(stru=
ct drm_device *dev,
> > >  	}
> > >  }
> > > =

> > > -static void ironlake_set_fifo_underrun_reporting(struct drm_device *=
dev,
...
> > > @@ -264,7 +264,7 @@ static bool __intel_set_cpu_fifo_underrun_reporti=
ng(struct drm_device *dev,
...
> > > -			     unsigned int retry)
> > > +static int ilk_do_reset(struct intel_gt *gt, intel_engine_mask_t eng=
ine_mask,
> > > +			unsigned int retry)
> > >  {
> > >  	struct intel_uncore *uncore =3D gt->uncore;
> > >  	int ret;
> > > @@ -597,7 +596,7 @@ static reset_func intel_get_gpu_reset(const struc=
t intel_gt *gt)
...
> > > @@ -1001,7 +1001,7 @@ static int i915_frequency_info(struct seq_file =
*m, void *unused)
> > >  	return ret;
> > >  }
> > > =
...
> > > @@ -1209,7 +1209,7 @@ static int i915_drpc_info(struct seq_file *m, v=
oid *unused)
...
> > > @@ -2031,7 +2031,7 @@ static void ivb_display_irq_handler(struct drm_=
...
> > > @@ -2742,7 +2742,7 @@ static void vlv_display_irq_postinstall(struct =
...
> > > @@ -3225,7 +3225,7 @@ static void ibx_irq_postinstall(struct drm_i915=
_private *dev_priv)
> > >  		spt_hpd_detection_setup(dev_priv);
> > >  }
> > > =

> > > -static void ironlake_irq_postinstall(struct drm_i915_private *dev_pr=
iv)
> > > +static void ilk_irq_postinstall(struct drm_i915_private *dev_priv)
> > >  {
> > >  	struct intel_uncore *uncore =3D &dev_priv->uncore;
> > >  	u32 display_mask, extra_mask;
> > > @@ -3980,7 +3980,7 @@ static irq_handler_t intel_irq_handler(struct d=
...
> > > @@ -4003,7 +4003,7 @@ static void intel_irq_reset(struct drm_i915_pri=
vate *dev_priv)
> > >  		else if (INTEL_GEN(dev_priv) >=3D 8)
...
> > > @@ -4026,7 +4026,7 @@ static void intel_irq_postinstall(struct drm_i9=
15_private *dev_priv)
> > >  		else if (INTEL_GEN(dev_priv) >=3D 8)
...
> > > @@ -178,7 +178,7 @@ static void pnv_get_mem_freq(struct drm_i915_priv=
ate *dev_priv)
> > >  	dev_priv->is_ddr3 =3D (tmp & CSHRDDR3CTL_DDR3) ? 1 : 0;
> > >  }
> > > =

> > > -static void i915_ironlake_get_mem_freq(struct drm_i915_private *dev_=
priv)
...
> > > @@ -7182,7 +7182,7 @@ void intel_init_pm(struct drm_i915_private *dev=
_priv)

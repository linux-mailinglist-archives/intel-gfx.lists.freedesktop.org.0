...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 739215BD775
	for <lists+intel-gfx@lfdr.de>; Tue, 20 Sep 2022 00:34:27 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 60DE410E1C1;
	Mon, 19 Sep 2022 22:33:26 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga17.intel.com (mga17.intel.com [192.55.52.151])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 554EE10E19B;
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
 ...
 bh=P4u5hHAzwDHJGEgeNDvFLH0YXL5u2q5oQNSOaahaeLY=;
 b=gBXLDR8akt7AII41dQBs4I4xZylL8SCc2FF9sMoKPdqC+kZDr0iSFxNp
 Ty9RHdFgDGFULeY1H63ZPd9Y7sOqmCuk7z+hFxexLiEfVkoJ0vmfhHD/i
 7AbVsOt09AGkpbMuocjfXv/a/5TfYfpxpdWDqDVDeT94QCJpHCJdstQIS
 6M42qqMUupfc65MfB/836KQrElpeqjXft/goWU3oMnitsrtPDRwxvU+70
 9dd914mut5zH+uqSAbwcH3bulqspBmq2hmYrGI7bcKV7Jm49PL8ZC9yUn
 u89vU8iBPcRpTvaMHoEr7NegLzs9I9VkCsh7MSn6bBOkbRxsWqaYxemnk Q==;
X-IronPort-AV: E=McAfee;i="6500,9779,10475"; a="279920539"
X-IronPort-AV: E=Sophos;i="5.93,329,1654585200"; d="scan'208";a="279920539"
Received: from orsmga005.jf.intel.com ([10.7.209.41])
 by fmsmga107.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 19 Sep 2022 15:33:09 -0700
X-IronPort-AV: E=Sophos;i="5.93,329,1654585200"; d="scan'208";a="794030782"
Received: from mdroper-desk1.fm.intel.com ([10.1.27.134])
 by orsmga005-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 19 Sep 2022 15:33:08 -0700
From: Matt Roper <matthew.d.roper@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Mon, 19 Sep 2022 15:32:56 -0700
Message-Id: <20220919223259.263525-10-matthew.d.roper@intel.com>
...
Subject: [Intel-gfx] [PATCH 09/12] drm/i915/gt: Add MCR-specific workaround
 initializers
...
@@ -1628,14 +1680,25 @@ wa_list_apply(struct intel_gt *gt, const struct i915_wa_list *wal)
 		u32 val, old = 0;
...
@@ -1664,8 +1727,9 @@ static bool wa_list_verify(struct intel_gt *gt,
...
+				intel_uncore_read_fw(uncore, wa->reg),
 	...
 	intel_uncore_forcewake_put__locked(uncore, fw);
@@ -1711,12 +1775,36 @@ whitelist_reg_ext(struct i915_wa_list *wal, i915_reg_t reg, u32 flags)
 	_wa_add(wal, &wa);
 }
...
@@ -1742,7 +1830,7 @@ static void skl_whitelist_build(struct intel_engine_cs *engine)
...
@@ -1763,7 +1851,7 @@ static void kbl_whitelist_build(struct intel_engine_cs *engine)
 	gen9_whitelist_build(w);
...
@@ -1828,10 +1916,10 @@ static void icl_whitelist_build(struct intel_engine_cs *engine)
 	switch (engine->class) {
 	case RENDER_CLASS:
...
@@ -2107,24 +2195,24 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2133,13 +2221,13 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 		 * Wa_22012826095:dg2
 		 * Wa_22013059131:dg2
 		 */
...
@@ -2152,19 +2240,19 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 	if (IS_DG2_GRAPHICS_STEP(i915, G10, STEP_B0, STEP_FOREVER) ||
 	    IS_DG2_G11(i915) || IS_DG2_G12(i915)) {
 		/* Wa_22013037850:dg2 */
...
+				  LSC_L1_FLUSH_CTL_3D_DATAPORT_FLUSH_EVENTS_MASK);
 	...
 	if (IS_DG2_GRAPHICS_STEP(i915, G10, STEP_A0, STEP_B0)) {
@@ -2172,7 +2260,7 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+		wa_mcr_masked_en(wal, GEN8_ROW_CHICKEN,
 		...
 		/*
@@ -2181,31 +2269,31 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2216,17 +2304,17 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 	if (IS_DG2_GRAPHICS_STEP(engine->i915, G10, STEP_A0, STEP_B0) ||
 	    IS_DG2_GRAPHICS_STEP(engine->i915, G11, STEP_A0, STEP_B0)) {
 		/* Wa_14012362059:dg2 */
...
@@ -2253,7 +2341,7 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+		wa_mcr_masked_en(wal, GEN8_ROW_CHICKEN2, GEN12_DISABLE_EARLY_READ);
 		...
 		/*
 		 * Wa_1407928979:tgl A*
@@ -2282,14 +2370,14 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+		wa_mcr_masked_en(wal, GEN9_ROW_CHICKEN4, GEN12_DISABLE_TDL_PUSH);
 	...
 	if (IS_DG1_GRAPHICS_STEP(i915, STEP_A0, STEP_B0) ||
@@ -2313,9 +2401,9 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2349,9 +2437,9 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 		 * Wa_1405733216:icl
...
+				GEN11_LQSC_CLEAN_EVICT_DISABLE);
 		...
 		/* Wa_1606682166:icl */
 		wa_write_or(wal,
@@ -2359,10 +2447,10 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2420,30 +2508,30 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+				     EVICTION_PERF_FIX_ENABLE, 0);
 	...
 	if (IS_HASWELL(i915)) {
@@ -2657,7 +2745,7 @@ ccs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 {
...
@@ -2683,8 +2771,8 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
 	}
...
@@ -2695,10 +2783,10 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
...
+			   true);
 	...
 	/*
@@ -2707,8 +2795,8 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
 	 * platforms.
 	 */
...
+					THREAD_EX_ARB_MODE_RR_AFTER_DEP);
 ...
 /*
@@ -2734,30 +2822,30 @@ general_render_compute_wa_init(struct intel_engine_cs *engine, struct i915_wa_li
...
@@ -2766,19 +2854,19 @@ general_render_compute_wa_init(struct intel_engine_cs *engine, struct i915_wa_li

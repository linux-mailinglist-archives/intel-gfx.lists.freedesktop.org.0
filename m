...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id AAA6E11EC45
	for <lists+intel-gfx@lfdr.de>; Fri, 13 Dec 2019 21:56:21 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 0F7256EDBD;
	Fri, 13 Dec 2019 20:56:20 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga04.intel.com (mga04.intel.com [192.55.52.120])
 by gabe.freedesktop.org (Postfix) with ESMTPS id C8C2B6EDBD
 for <intel-gfx@lists.freedesktop.org>; Fri, 13 Dec 2019 20:56:18 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
 by fmsmga104.fm.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 13 Dec 2019 12:56:18 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.69,311,1571727600"; d="scan'208";a="246258265"
Received: from stinkbox.fi.intel.com (HELO stinkbox) ([10.237.72.174])
 by fmsmga002.fm.intel.com with SMTP; 13 Dec 2019 12:56:16 -0800
Received: by stinkbox (sSMTP sendmail emulation);
 Fri, 13 Dec 2019 22:56:15 +0200
Date: Fri, 13 Dec 2019 22:56:15 +0200
From: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
To: =?iso-8859-1?Q?Jos=E9?= Roberto de Souza <jose.souza@intel.com>
Message-ID: <20191213205615.GI1208@intel.com>
...
> > @@ -206,6 +206,20 @@ intel_digital_connector_needs_modeset(struct intel=
_atomic_state *state,
> >  	       (new_crtc && drm_atomic_crtc_needs_modeset(&new_crtc_state->ua=
pi));
> >  }
> >  =

> > +struct intel_digital_connector_state *
> > +intel_atomic_get_digital_connector_state(struct intel_atomic_state *st=
ate,
...
> >  struct drm_crtc_state *intel_crtc_duplicate_state(struct drm_crtc *crt=
c);
...
> > @@ -1903,8 +1903,13 @@ intel_ddi_transcoder_func_reg_val_get(const stru=
...
> > @@ -11663,6 +11664,7 @@ static void intel_crtc_state_reset(struct intel=
...
> > @@ -12510,6 +12512,9 @@ static void intel_dump_pipe_config(const struct=
...
> >  	if (!state)
> >  		return;
> > @@ -12651,6 +12656,7 @@ intel_crtc_prepare_cleared_state(struct intel_c=
...
> > @@ -13290,6 +13296,8 @@ intel_pipe_config_compare(const struct intel_cr=
...
> > @@ -14374,7 +14382,7 @@ static void intel_commit_modeset_disables(struc=
...
> > @@ -14388,7 +14396,8 @@ static void intel_commit_modeset_disables(struc=
t intel_atomic_state *state)
...
> > @@ -87,6 +87,49 @@ static int intel_dp_mst_compute_link_config(struct i=
...
> > +/*
> > + * Iterate over all connectors and set mst_master_transcoder to the sm=
allest
...
->uapi.state);
> > +	struct intel_connector *connector =3D to_intel_connector(connector_st=
ate->connector);
...
> > +			continue;
> > +
> > +		if (iter_crtc_state->cpu_transcoder < crtc_state->mst_master_transco=
der)
> > +			crtc_state->mst_master_transcoder =3D iter_crtc_state->cpu_transcod=
er;
...
> > @@ -154,24 +197,88 @@ static int intel_dp_mst_compute_config(struct int=
el_encoder *encoder,
> >  =
...
> > +
> > +/*
> > + * If one of the connectors in a MST stream needs a modeset, mark all =
CRTCs
> > + * that have a connector in the same MST stream as mode changed,
> > + * intel_modeset_pipe_config()+intel_crtc_check_fastset() will take to=
 do a
> > + * fastset when possible.
> > + */
> > +static int
> > +intel_dp_mst_atomic_master_trans_check(struct intel_connector *connect=
or,
...
> > @@ -182,12 +289,9 @@ intel_dp_mst_atomic_check(struct drm_connector *co=
nnector,
> >  	 * connector
> >  	 */
> >  	if (new_crtc) {
...
> > @@ -196,7 +300,7 @@ intel_dp_mst_atomic_check(struct drm_connector *con=
nector,
> >  	}
> >  =

> >  	mgr =3D &enc_to_mst(old_conn_state->best_encoder)->primary->dp.mst_mg=
r;
...
> >  	return ret;
> > @@ -241,6 +345,9 @@ static void intel_mst_post_disable_dp(struct intel_=
encoder *encoder,
...
> > @@ -321,6 +428,9 @@ static void intel_mst_pre_enable_dp(struct intel_en=
coder *encoder,
...
> >  	if (first_mst_stream)
> > @@ -726,3 +836,14 @@ intel_dp_mst_encoder_cleanup(struct intel_digital_=
port *intel_dig_port)
...
state)
> > +{
> > +	return crtc_state->mst_master_transcoder =3D=3D crtc_state->cpu_trans=
coder;
> > +}
> > +
...
> >  int intel_dp_mst_encoder_init(struct intel_digital_port *intel_dig_por=
t, int conn_id);
> >  void intel_dp_mst_encoder_cleanup(struct intel_digital_port *intel_dig=
_port);

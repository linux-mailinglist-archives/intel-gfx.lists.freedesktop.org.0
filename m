...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 11C2D1D8B84
	for <lists+intel-gfx@lfdr.de>; Tue, 19 May 2020 01:14:21 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 686D46E4A7;
	Mon, 18 May 2020 23:14:19 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from fireflyinternet.com (mail.fireflyinternet.com [109.228.58.192])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 432466E4A7
 for <intel-gfx@lists.freedesktop.org>; Mon, 18 May 2020 23:14:17 +0000 (UTC)
X-Default-Received-SPF: pass (skip=forwardok (res=PASS))
 x-ip-name=78.156.65.138; 
Received: from build.alporthouse.com (unverified [78.156.65.138]) 
 by fireflyinternet.com (Firefly Internet (M1)) with ESMTP id 21236097-1500050 
 for multiple; Tue, 19 May 2020 00:14:09 +0100
From: Chris Wilson <chris@chris-wilson.co.uk>
To: intel-gfx@lists.freedesktop.org
Date: Tue, 19 May 2020 00:14:06 +0100
Message-Id: <20200518231407.6837-3-chris@chris-wilson.co.uk>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH 3/4] drm/i915/selftests: Restore to default
 heartbeat
...
index f128744ab09b..3cf7c0b13ab4 100644
...
@@ -4338,7 +4329,6 @@ static int reset_virtual_engine(struct intel_gt *gt,
 {
...
-	unsigned long *heartbeat;
 	...
 	struct i915_request *rq;
 	unsigned int n;
@@ -4350,15 +4340,9 @@ static int reset_virtual_engine(struct intel_gt *gt,
...
-
 	...
 	if (IS_ERR(ve)) {
 		err = PTR_ERR(ve);
@@ -4366,7 +4350,7 @@ static int reset_virtual_engine(struct intel_gt *gt,
 	}
...
+		engine_heartbeat_disable(siblings[n]);
 	...
 	if (IS_ERR(rq)) {
@@ -4437,13 +4421,11 @@ static int reset_virtual_engine(struct intel_gt *gt,
 	i915_request_put(rq);
 out_heartbeat:
 	for (n = 0; n < nsibling; n++)
...
@@ -5112,9 +5094,7 @@ static int live_lrc_gpr(void *arg)
...
+		engine_heartbeat_disable(engine);
 		...
 		if (err)
@@ -5125,7 +5105,7 @@ static int live_lrc_gpr(void *arg)
 			goto err;
...
+		engine_heartbeat_enable(engine);
 		if (igt_flush_test(gt->i915))
 			err = -EIO;
 		if (err)
@@ -5272,10 +5252,9 @@ static int live_lrc_timestamp(void *arg)
 	 */
...
+		engine_heartbeat_disable(data.engine);
 		...
 		for (i = 0; i < ARRAY_SIZE(data.ce); i++) {
 			struct intel_context *tmp;
@@ -5308,7 +5287,7 @@ static int live_lrc_timestamp(void *arg)
 		}

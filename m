...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 51A9B1536EB
	for <lists+intel-gfx@lfdr.de>; Wed,  5 Feb 2020 18:46:07 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 4334E6F8FC;
	Wed,  5 Feb 2020 17:46:05 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga02.intel.com (mga02.intel.com [134.134.136.20])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 395706F902;
 Wed,  5 Feb 2020 17:46:04 +0000 (UTC)
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga001.fm.intel.com ([10.253.24.23])
 by orsmga101.jf.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 05 Feb 2020 09:45:53 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.70,406,1574150400"; d="scan'208";a="343803653"
Received: from plaxmina-desktop.iind.intel.com ([10.145.162.62])
 by fmsmga001.fm.intel.com with ESMTP; 05 Feb 2020 09:45:48 -0800
...
Date: Wed,  5 Feb 2020 23:04:15 +0530
Message-Id: <20200205173419.1695-4-pankaj.laxminarayan.bharadiya@intel.com>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH v6 3/7] drm/i915/display/display: Make WARN* drm
...
+				  FDI_RX_CTL(PIPE_B)) & FDI_RX_ENABLE);
+	drm_WARN_ON(&dev_priv->drm,
+		    intel_de_read(dev_priv,
+				  FDI_RX_CTL(PIPE_C)) & FDI_RX_ENABLE);
...
+		drm_WARN_ON(dev, *power_domain_mask & BIT_ULL(power_domain));
 		...
 		wf = intel_display_power_get_if_enabled(dev_priv, power_domain);
 		if (!wf)
@@ -11032,8 +11049,8 @@ static void hsw_get_ddi_port_state(struct intel_crtc *crtc,
 
 	pll = pipe_config->shared_dpll;
 	if (pll) {
-		WARN_ON(!pll->info->funcs->get_hw_state(dev_priv, pll,
-						&pipe_config->dpll_hw_state));
+		drm_WARN_ON(&dev_priv->drm, !pll->info->funcs->get_hw_state(dev_priv, pll,
+									    &pipe_config->dpll_hw_state));
 	}
 
 	/*
@@ -11103,8 +11120,9 @@ static void icl_get_trans_port_sync_config(struct intel_crtc_state *crtc_state)
...
@@ -11135,7 +11153,7 @@ static bool hsw_get_pipe_config(struct intel_crtc *crtc,
...
@@ -11202,7 +11220,7 @@ static bool hsw_get_pipe_config(struct intel_crtc *crtc,
...
+	drm_WARN_ON(&dev_priv->drm, power_domain_mask & BIT_ULL(power_domain));
 	...
 	wf = intel_display_power_get_if_enabled(dev_priv, power_domain);
 	if (wf) {
@@ -11449,8 +11467,8 @@ static int i845_check_cursor(struct intel_crtc_state *crtc_state,
 		return -EINVAL;
 	}
...
+		    plane_state->color_plane[0].stride != fb->pitches[0]);
 	...
 	switch (fb->pitches[0]) {
 	case 256:
@@ -11666,8 +11684,8 @@ static int i9xx_check_cursor(struct intel_crtc_state *crtc_state,
 		return -EINVAL;
 	}
...
@@ -11886,7 +11904,7 @@ int intel_get_load_detect_pipe(struct drm_connector *connector,
...
@@ -12330,7 +12348,7 @@ int intel_plane_atomic_calc_changes(const struct intel_crtc_state *old_crtc_stat
...
+	if (!was_crtc_enabled && drm_WARN_ON(&dev_priv->drm, was_visible))
 	...
 	/*
@@ -12796,7 +12814,7 @@ static int intel_crtc_atomic_check(struct intel_atomic_state *state,
...
+	    !drm_WARN_ON(&dev_priv->drm, crtc_state->shared_dpll)) {
 		...
 		if (ret)
 			return ret;
@@ -12826,7 +12844,8 @@ static int intel_crtc_atomic_check(struct intel_atomic_state *state,
 	}
...
+				!dev_priv->display.compute_pipe_wm))
 		...
 		/*
@@ -13259,12 +13278,12 @@ static bool check_digital_port_conflicts(struct intel_atomic_state *state)
...
@@ -13520,7 +13539,8 @@ intel_modeset_pipe_config(struct intel_crtc_state *pipe_config)
 	}
...
@@ -14051,9 +14071,10 @@ static void intel_pipe_config_sanity_check(struct drm_i915_private *dev_priv,
...
@@ -15243,7 +15264,8 @@ static struct intel_crtc *intel_get_slave_crtc(const struct intel_crtc_state *ne
...
@@ -15411,8 +15433,8 @@ static void intel_update_trans_port_sync_crtcs(struct intel_crtc *crtc,
...
@@ -15533,8 +15555,8 @@ static void skl_commit_modeset_enables(struct intel_atomic_state *state)
...
@@ -15568,8 +15590,8 @@ static void skl_commit_modeset_enables(struct intel_atomic_state *state)
...
+									entries, num_pipes, pipe));
 		...
 		entries[pipe] = new_crtc_state->wm.skl.ddb;
 		modeset_pipes &= ~BIT(pipe);
@@ -15577,7 +15599,7 @@ static void skl_commit_modeset_enables(struct intel_atomic_state *state)
...
@@ -16826,7 +16848,7 @@ static int intel_crtc_init(struct drm_i915_private *dev_priv, enum pipe pipe)
...
@@ -17693,7 +17715,7 @@ static void sanitize_watermarks(struct drm_i915_private *dev_priv)
 		return;
...
@@ -17745,7 +17767,8 @@ static void sanitize_watermarks(struct drm_i915_private *dev_priv)
...
@@ -18022,7 +18045,8 @@ void i830_enable_pipe(struct drm_i915_private *dev_priv, enum pipe pipe)
 	u32 dpll, fp;
 	int i;
...
@@ -18087,11 +18111,19 @@ void i830_disable_pipe(struct drm_i915_private *dev_priv, enum pipe pipe)
...
@@ -18595,7 +18627,7 @@ static void intel_modeset_readout_hw_state(struct drm_device *dev)
...
@@ -18809,7 +18841,7 @@ intel_modeset_setup_hw_state(struct drm_device *dev,
 		u64 put_domains;
...
@@ -18875,7 +18907,7 @@ void intel_modeset_driver_remove(struct drm_i915_private *i915)

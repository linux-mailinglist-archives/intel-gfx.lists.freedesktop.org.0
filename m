...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id BDFA114A95E
	for <lists+intel-gfx@lfdr.de>; Mon, 27 Jan 2020 19:00:57 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 270B36E092;
	Mon, 27 Jan 2020 18:00:56 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga09.intel.com (mga09.intel.com [134.134.136.24])
 by gabe.freedesktop.org (Postfix) with ESMTPS id A72B56E092
 for <intel-gfx@lists.freedesktop.org>; Mon, 27 Jan 2020 18:00:54 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from orsmga008.jf.intel.com ([10.7.209.65])
 by orsmga102.jf.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 27 Jan 2020 09:55:09 -0800
X-IronPort-AV: E=Sophos;i="5.70,370,1574150400"; d="scan'208";a="221804463"
Received: from ideak-desk.fi.intel.com ([10.237.72.183])
 by orsmga008-auth.jf.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 27 Jan 2020 09:55:07 -0800
Date: Mon, 27 Jan 2020 19:54:58 +0200
From: Imre Deak <imre.deak@intel.com>
To: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
Message-ID: <20200127175458.GC5006@ideak-desk.fi.intel.com>
References: <20200120174728.21095-18-ville.syrjala@linux.intel.com>
 <20200121140353.25997-1-ville.syrjala@linux.intel.com>
 <20200127170301.GA5006@ideak-desk.fi.intel.com>
 <20200127171506.GF13686@intel.com>
MIME-Version: 1.0
Content-Disposition: inline
In-Reply-To: <20200127171506.GF13686@intel.com>
...
On Mon, Jan 27, 2020 at 07:15:06PM +0200, Ville Syrj=E4l=E4 wrote:
...
> > > @@ -518,7 +518,6 @@ void intel_atomic_state_clear(struct drm_atomic_s=
tate *s)
...
> > > +		return 0;
> > > +
> > > +	DRM_DEBUG_KMS("[PLANE:%d:%s] min cdclk (%d kHz) > [CRTC:%d:%s] min =
cdclk (%d kHz)\n",
> > > +		      plane->base.base.id, plane->base.name,
...
> > >  static void intel_plane_clear_hw_state(struct intel_plane_state *pla=
ne_state)
...
> > > @@ -46,7 +46,8 @@ int intel_plane_atomic_calc_changes(const struct in=
...
> > > @@ -800,6 +801,34 @@ void intel_init_audio_hooks(struct drm_i915_priv=
ate *dev_priv)
> > >  	}
> > >  }
> > >  =

> > > +static int glk_force_audio_cdclk_commit(struct intel_atomic_state *s=
tate,
> > > +					bool enable)
> > > +{
...
> > >  				  bool enable)
> > >  {
> > > @@ -815,15 +844,7 @@ static void glk_force_audio_cdclk(struct drm_i91=
5_private *dev_priv,
> > >  	state->acquire_ctx =3D &ctx;
> > >  =

> > >  retry:
> > > -	to_intel_atomic_state(state)->cdclk_state.force_min_cdclk_changed =
=3D true;
...
> > > -
> > > +	ret =3D glk_force_audio_cdclk_commit(to_intel_atomic_state(state), =
enable);
...
> > > @@ -1807,37 +1807,6 @@ static bool intel_cdclk_changed(const struct i=
ntel_cdclk_config *a,
...
> > > - * @state: atomic state
> > > - *
> > > - * This is the CDCLK version of drm_atomic_helper_swap_state() since=
 the
> > > - * helper does not handle driver-specific global state.
> > > - *
> > > - * Similarly to the atomic helpers this function does a complete swa=
p,
> > > - * i.e. it also puts the old state into @state. This is used by the =
commit
> > > - * code to determine how CDCLK has changed (for instance did it incr=
ease or
> > > - * decrease).
> > > - */
...
> > > -}
> > > -
> > >  void intel_dump_cdclk_config(const struct intel_cdclk_config *cdclk_=
config,
> > >  			     const char *context)
> > >  {
...
> > > -	const struct intel_cdclk_state *old_cdclk_state =3D &state->cdclk_s=
tate;
> > > -	const struct intel_cdclk_state *new_cdclk_state =3D &dev_priv->cdcl=
k_state;
...
> > > -	    old_cdclk_state->actual.cdclk <=3D new_cdclk_state->actual.cdcl=
k)
> > > +	    old_cdclk_state->actual.cdclk <=3D new_cdclk_state->actual.cdcl=
k) {
...
crtc_state)
> > > @@ -2042,10 +2027,10 @@ int intel_crtc_compute_min_cdclk(const struct=
 intel_crtc_state *crtc_state)
> > >  	return min_cdclk;
> > >  }
> > >  =
...
> > > +static int intel_compute_min_cdclk(struct intel_cdclk_state *cdclk_s=
tate)
> > >  {
...
> > >  	struct intel_crtc_state *crtc_state;
> > >  	int min_cdclk, i;
> > > @@ -2063,7 +2048,7 @@ static int intel_compute_min_cdclk(struct intel=
_atomic_state *state)
> > >  =
...
> > >  			return ret;
> > >  	}
> > > @@ -2088,10 +2073,10 @@ static int intel_compute_min_cdclk(struct int=
...
state)
> > > +static int bxt_compute_min_voltage_level(struct intel_cdclk_state *c=
dclk_state)
> > >  {
> > > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> > >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > > @@ -2111,7 +2096,7 @@ static int bxt_compute_min_voltage_level(struct=
...
> > >  			return ret;
> > >  	}
> > > @@ -2124,13 +2109,13 @@ static int bxt_compute_min_voltage_level(stru=
...
> > > +static int vlv_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_st=
ate)
> > >  {
> > > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> > >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > > @@ -2153,12 +2138,12 @@ static int vlv_modeset_calc_cdclk(struct inte=
...
> > > +static int bdw_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_st=
ate)
> > >  {
> > > -	struct intel_cdclk_state *cdclk_state =3D &state->cdclk_state;
> > > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
...
> > > @@ -2185,10 +2170,10 @@ static int bdw_modeset_calc_cdclk(struct inte=
l_atomic_state *state)
> > >  	return 0;
> > >  }
> > >  =
...
> > >  	struct intel_crtc_state *crtc_state;
> > >  	int vco, i;
> > > @@ -2222,16 +2207,16 @@ static int skl_dpll0_vco(struct intel_atomic_=
state *state)
> > >  	return vco;
> > >  }
> > >  =
...
> > > +static int skl_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_st=
ate)
> > >  {
> > > -	struct intel_cdclk_state *cdclk_state =3D &state->cdclk_state;
> > > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
...
> > > @@ -2258,17 +2243,17 @@ static int skl_modeset_calc_cdclk(struct inte=
l_atomic_state *state)
> > >  	return 0;
> > >  }
> > >  =
...
> > > +static int bxt_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_st=
ate)
> > >  {
> > > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> > >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > > @@ -2335,7 +2320,7 @@ static int intel_modeset_all_pipes(struct intel=
_atomic_state *state)
> > >  	return 0;
> > >  }
> > >  =
...
> > > +static int fixed_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_=
state)
> > >  {
> > >  	int min_cdclk;
> > >  =

> > > @@ -2344,54 +2329,95 @@ static int fixed_modeset_calc_cdclk(struct in=
...
> > > +static struct intel_global_state *intel_cdclk_duplicate_state(struct=
 intel_global_obj *obj)
> > > +{
> > > +	struct intel_cdclk_state *cdclk_state;
> > > +
> > > +	cdclk_state =3D kmemdup(obj->state, sizeof(*cdclk_state), GFP_KERNE=
L);
> > > +	if (!cdclk_state)
> > > +		return NULL;
> > > +
...
> > > +
> > > +	cdclk_state =3D intel_atomic_get_global_obj_state(state, &dev_priv-=
>cdclk.obj);
> > > +	if (IS_ERR(cdclk_state))
...
> > >  {
> > >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
> > > -	const struct intel_cdclk_state *old_cdclk_state =3D &dev_priv->cdcl=
k_state;
...
> > > -	memcpy(new_cdclk_state->min_voltage_level, old_cdclk_state->min_vol=
tage_level,
...
> > > -		new_cdclk_state->force_min_cdclk =3D old_cdclk_state->force_min_cd=
clk;
...
> > >  			return ret;
> > >  	} else {
> > > diff --git a/drivers/gpu/drm/i915/display/intel_cdclk.h b/drivers/gpu=
/drm/i915/display/intel_cdclk.h
...
> > > +};
> > > +
> > >  int intel_crtc_compute_min_cdclk(const struct intel_crtc_state *crtc=
_state);
> > >  void intel_cdclk_init_hw(struct drm_i915_private *i915);
> > >  void intel_cdclk_uninit_hw(struct drm_i915_private *i915);
> > > @@ -31,12 +61,21 @@ void intel_update_cdclk(struct drm_i915_private *=
dev_priv);
...
> > > -void intel_cdclk_clear_state(struct intel_atomic_state *state);
> > > -void intel_cdclk_swap_state(struct intel_atomic_state *state);
> > >  void intel_set_cdclk_pre_plane_update(struct intel_atomic_state *sta=
te);
> > >  void intel_set_cdclk_post_plane_update(struct intel_atomic_state *st=
ate);
> > >  void intel_dump_cdclk_config(const struct intel_cdclk_config *cdclk_=
config,
> > >  			     const char *context);
...
> > > +intel_atomic_get_cdclk_state(struct intel_atomic_state *state);
> > > +
> > > +#define to_intel_cdclk_state(x) container_of((x), struct intel_cdclk=
_state, base)
...
> > > @@ -7482,7 +7482,7 @@ static void intel_crtc_disable_noatomic(struct =
intel_crtc *crtc,
...
> > > @@ -7780,17 +7780,17 @@ bool hsw_crtc_state_ips_capable(const struct =
intel_crtc_state *crtc_state)
> > >  	return true;
> > >  }
> > >  =

> > > -static bool hsw_compute_ips_config(struct intel_crtc_state *crtc_sta=
te)
> > > +static int hsw_compute_ips_config(struct intel_crtc_state *crtc_stat=
e)
> > >  {
...
> > > @@ -7799,18 +7799,27 @@ static bool hsw_compute_ips_config(struct int=
...
> > >  static bool intel_crtc_supports_double_wide(const struct intel_crtc =
*crtc)
> > > @@ -12543,14 +12552,11 @@ static u16 hsw_linetime_wm(const struct int=
el_crtc_state *crtc_state)
...
> > >  		return 0;
> > > @@ -12580,6 +12586,32 @@ static u16 skl_linetime_wm(const struct inte=
...
> > >  				   struct intel_crtc *crtc)
> > >  {
> > > @@ -12615,7 +12647,6 @@ static int intel_crtc_atomic_check(struct int=
el_atomic_state *state,
> > >  			return ret;
> > >  	}
> > >  =
...
> > >  		if (ret) {
> > > @@ -12646,20 +12677,25 @@ static int intel_crtc_atomic_check(struct i=
...
> > >  static void intel_modeset_update_connector_atomic_state(struct drm_d=
evice *dev)
> > > @@ -14478,7 +14514,7 @@ static bool active_planes_affects_min_cdclk(s=
truct drm_i915_private *dev_priv)
> > >  }
> > >  =

> > >  static int intel_atomic_check_planes(struct intel_atomic_state *stat=
e,
> > > -				     bool *need_modeset)
> > > +				     bool *need_cdclk_calc)
> > >  {
...
> > > @@ -14532,8 +14568,11 @@ static int intel_atomic_check_planes(struct =
intel_atomic_state *state,
...
> > >  	return 0;
> > >  }
> > > @@ -14654,6 +14693,7 @@ static int intel_atomic_check(struct drm_devi=
ce *dev,
...
> > > @@ -14766,18 +14806,18 @@ static int intel_atomic_check(struct drm_de=
vice *dev,
> > >  	if (ret)
> > >  		goto fail;
> > >  =
...
> > > @@ -15676,8 +15716,6 @@ static int intel_atomic_commit(struct drm_dev=
ice *dev,
...
> > > @@ -17356,7 +17394,7 @@ void intel_init_display_hooks(struct drm_i915=
_private *dev_priv)
...
> > > @@ -17587,6 +17625,10 @@ int intel_modeset_init(struct drm_i915_priva=
te *i915)
> > >  =
...
> > >  	if (ret)
> > >  		return ret;
> > > @@ -18071,6 +18113,8 @@ static void readout_plane_state(struct drm_i9=
15_private *dev_priv)
...
> > > @@ -18190,8 +18234,6 @@ static void intel_modeset_readout_hw_state(st=
ruct drm_device *dev)

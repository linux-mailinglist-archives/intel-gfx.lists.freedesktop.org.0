...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 60C4B29A288
	for <lists+intel-gfx@lfdr.de>; Tue, 27 Oct 2020 03:10:15 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id C86D76E06D;
	Tue, 27 Oct 2020 02:10:12 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga06.intel.com (mga06.intel.com [134.134.136.31])
 by gabe.freedesktop.org (Postfix) with ESMTPS id D277E6E06D;
 Tue, 27 Oct 2020 02:10:11 +0000 (UTC)
IronPort-SDR: yzWOv1svSI+1CVBXw8SmoqvHzBMcFKJ3PRVnHlbbyJQqMSFDuAZx9uchO492Iye77WYksWxiQy
 wziJYoQ/w6yQ==
X-IronPort-AV: E=McAfee;i="6000,8403,9786"; a="229651923"
X-IronPort-AV: E=Sophos;i="5.77,422,1596524400"; 
 d="asc'?scan'208";a="229651923"
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from fmsmga003.fm.intel.com ([10.253.24.29])
 by orsmga104.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 26 Oct 2020 19:10:11 -0700
IronPort-SDR: 997+0I3tpXz8vLYkWKt4zRLdbkBMvJnL3LwAz2yBNeeQK2yKT+yH47t0mhDemnBUSuB4ruXX1G
 of9H7UD0N3Rw==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.77,422,1596524400"; 
 d="asc'?scan'208";a="360583193"
Received: from zhen-hp.sh.intel.com (HELO zhen-hp) ([10.239.160.147])
 by FMSMGA003.fm.intel.com with ESMTP; 26 Oct 2020 19:10:09 -0700
Date: Tue, 27 Oct 2020 09:49:06 +0800
From: Zhenyu Wang <zhenyuw@linux.intel.com>
To: Colin Xu <Colin.Xu@intel.com>
Message-ID: <20201027014906.GX27141@zhen-hp.sh.intel.com>
...
MIME-Version: 1.0
In-Reply-To: <62d47b5a-142b-32dc-b144-15bc56d70933@intel.com>
User-Agent: Mutt/1.10.0 (2018-05-17)
Subject: Re: [Intel-gfx] [PATCH v6 1/2] drm/i915/gvt: Save/restore HW status
 to support GVT suspend/resume
...
Content-Type: multipart/mixed; boundary="===============1836532206=="
...
Content-Type: multipart/signed; micalg=pgp-sha1;
	protocol="application/pgp-signature"; boundary="m3EZO/+NOdcYnaJL"
...
> > > @@ -636,9 +636,20 @@ static void ggtt_set_host_entry(struct intel_vgp=
u_mm *mm,
> > >   		struct intel_gvt_gtt_entry *entry, unsigned long index)
> > >   {
> > >   	struct intel_gvt_gtt_pte_ops *pte_ops =3D mm->vgpu->gvt->gtt.pte_o=
ps;
...
> > > +	} else if (vgpu_gmadr_is_hidden(mm->vgpu, index << I915_GTT_PAGE_SH=
IFT)) {
...
> > > +	}
> > Looks 'dirty' flag is not needed at all, as what you need is to restore=
 all present
...
> other PTE fields are still valid, this is "present". For dirty it's purpo=
se
> is to record whether or not host has update this entry, regardless the pa=
ge
> presence. If only check the present bit, there will be a case that previo=
us
> PTE points to region that shouldn't be walked by the new PDE, but new
> PDE/PTE update only updates PDE w/ present, but PTE w/o present. That cou=
ld
...
>=20
> And actually, when vGPU created, all GGTT pages are updates so in practic=
e,
> all entries are dirty. So from real practice, it's true that dirty check =
can
> be removed.

You mean guest would update all entries normally? That's ok which we just f=
ollow
guest's behavior. And we've also seen guest partial update of GGTT entry wh=
ich
could be point for suspend/resume and we reset present bit at that time so =
can be
...
> > >   }
> > > @@ -1928,6 +1939,7 @@ static struct intel_vgpu_mm *intel_vgpu_create_=
ggtt_mm(struct intel_vgpu *vgpu)
> > >   {
...
> > >   	if (!mm)
> > > @@ -1944,6 +1956,25 @@ static struct intel_vgpu_mm *intel_vgpu_create=
_ggtt_mm(struct intel_vgpu *vgpu)
...
> look clean. Since kernel has 80 columns as the 'strong preferred limit', =
if
...
> > >   	}
> > >   	vgpu_free_mm(mm);
> > > @@ -2852,3 +2885,45 @@ void intel_vgpu_reset_gtt(struct intel_vgpu *v=
gpu)
...
> > >   		} ggtt_mm;
> > >   	};
> > > @@ -280,5 +288,6 @@ int intel_vgpu_emulate_ggtt_mmio_write(struct int=
el_vgpu *vgpu,
...
> > > @@ -405,6 +405,14 @@ int intel_gvt_init_device(struct drm_i915_privat=
e *i915)
> > >   	return ret;
> > >   }
> > > +int
...
> > > @@ -685,6 +687,7 @@ void intel_gvt_debugfs_remove_vgpu(struct intel_v=
gpu *vgpu);
...
> > > @@ -3120,9 +3120,10 @@ static int init_skl_mmio_info(struct intel_gvt=
 *gvt)
...
> > > @@ -3661,3 +3662,35 @@ int intel_vgpu_mmio_reg_rw(struct intel_vgpu *=
...
> > > @@ -147,3 +148,17 @@ void intel_gvt_driver_remove(struct drm_i915_pri=
vate *dev_priv)
...
> > > @@ -33,6 +33,7 @@ int intel_gvt_init_device(struct drm_i915_private *=
dev_priv);
...
> > >   {
> > > @@ -46,6 +47,10 @@ static inline void intel_gvt_driver_remove(struct =
...
iF0EARECAB0WIQTXuabgHDW6LPt9CICxBBozTXgYJwUCX5d8kgAKCRCxBBozTXgY
JyFOAJ0YFLTCVItVB/hqnUK8ikYXOM59vQCfRxrEtIbSz6HtZgZVyzs+o/cfWLY=
=LWrq
-----END PGP SIGNATURE-----

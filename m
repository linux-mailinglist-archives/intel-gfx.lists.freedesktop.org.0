...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 120E814A8D5
	for <lists+intel-gfx@lfdr.de>; Mon, 27 Jan 2020 18:19:24 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id B2B956E32E;
	Mon, 27 Jan 2020 17:19:21 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga09.intel.com (mga09.intel.com [134.134.136.24])
 by gabe.freedesktop.org (Postfix) with ESMTPS id D7CFA6E32E
 for <intel-gfx@lists.freedesktop.org>; Mon, 27 Jan 2020 17:19:20 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from orsmga007.jf.intel.com ([10.7.209.58])
 by orsmga102.jf.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 27 Jan 2020 09:15:09 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.70,370,1574150400"; d="scan'208";a="217335332"
Received: from stinkbox.fi.intel.com (HELO stinkbox) ([10.237.72.174])
 by orsmga007.jf.intel.com with SMTP; 27 Jan 2020 09:15:07 -0800
Received: by stinkbox (sSMTP sendmail emulation);
 Mon, 27 Jan 2020 19:15:06 +0200
Date: Mon, 27 Jan 2020 19:15:06 +0200
From: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
To: Imre Deak <imre.deak@intel.com>
Message-ID: <20200127171506.GF13686@intel.com>
...
In-Reply-To: <20200127170301.GA5006@ideak-desk.fi.intel.com>
...
> > @@ -518,7 +518,6 @@ void intel_atomic_state_clear(struct drm_atomic_sta=
te *s)
...
> > @@ -155,44 +156,61 @@ unsigned int intel_plane_data_rate(const struct i=
...
> > -		DRM_DEBUG_KMS("[PLANE:%d:%s] min_cdclk (%d kHz) > logical cdclk (%d =
kHz)\n",
...
> > +		return 0;
> > +
> > +	DRM_DEBUG_KMS("[PLANE:%d:%s] min cdclk (%d kHz) > [CRTC:%d:%s] min cd=
clk (%d kHz)\n",
> > +		      plane->base.base.id, plane->base.name,
...
> >  static void intel_plane_clear_hw_state(struct intel_plane_state *plane=
_state)
...
> > @@ -46,7 +46,8 @@ int intel_plane_atomic_calc_changes(const struct inte=
l_crtc_state *old_crtc_stat
> >  				    struct intel_crtc_state *crtc_state,
...
> > @@ -800,6 +801,34 @@ void intel_init_audio_hooks(struct drm_i915_privat=
e *dev_priv)
> >  	}
> >  }
> >  =

> > +static int glk_force_audio_cdclk_commit(struct intel_atomic_state *sta=
te,
> > +					bool enable)
> > +{
...
> >  				  bool enable)
> >  {
> > @@ -815,15 +844,7 @@ static void glk_force_audio_cdclk(struct drm_i915_=
private *dev_priv,
> >  	state->acquire_ctx =3D &ctx;
> >  =

> >  retry:
> > -	to_intel_atomic_state(state)->cdclk_state.force_min_cdclk_changed =3D=
 true;
...
> > -
> > +	ret =3D glk_force_audio_cdclk_commit(to_intel_atomic_state(state), en=
able);
...
> > @@ -1807,37 +1807,6 @@ static bool intel_cdclk_changed(const struct int=
el_cdclk_config *a,
...
> > - * @state: atomic state
> > - *
> > - * This is the CDCLK version of drm_atomic_helper_swap_state() since t=
he
> > - * helper does not handle driver-specific global state.
> > - *
> > - * Similarly to the atomic helpers this function does a complete swap,
> > - * i.e. it also puts the old state into @state. This is used by the co=
mmit
> > - * code to determine how CDCLK has changed (for instance did it increa=
se or
> > - * decrease).
> > - */
...
> > -}
> > -
> >  void intel_dump_cdclk_config(const struct intel_cdclk_config *cdclk_co=
nfig,
> >  			     const char *context)
> >  {
...
> > -	const struct intel_cdclk_state *old_cdclk_state =3D &state->cdclk_sta=
te;
> > -	const struct intel_cdclk_state *new_cdclk_state =3D &dev_priv->cdclk_=
state;
...
> >  static int intel_pixel_rate_to_cdclk(const struct intel_crtc_state *cr=
tc_state)
> > @@ -2042,10 +2027,10 @@ int intel_crtc_compute_min_cdclk(const struct i=
ntel_crtc_state *crtc_state)
> >  	return min_cdclk;
> >  }
> >  =
...
> > +static int intel_compute_min_cdclk(struct intel_cdclk_state *cdclk_sta=
te)
> >  {
...
> >  	struct intel_crtc_state *crtc_state;
> >  	int min_cdclk, i;
> > @@ -2063,7 +2048,7 @@ static int intel_compute_min_cdclk(struct intel_a=
tomic_state *state)
> >  =
...
> >  			return ret;
> >  	}
> > @@ -2088,10 +2073,10 @@ static int intel_compute_min_cdclk(struct intel=
_atomic_state *state)
> >   * future platforms this code will need to be
> >   * adjusted.
> >   */
> > -static int bxt_compute_min_voltage_level(struct intel_atomic_state *st=
ate)
> > +static int bxt_compute_min_voltage_level(struct intel_cdclk_state *cdc=
lk_state)
> >  {
> > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > @@ -2111,7 +2096,7 @@ static int bxt_compute_min_voltage_level(struct i=
...
> >  			return ret;
> >  	}
> > @@ -2124,13 +2109,13 @@ static int bxt_compute_min_voltage_level(struct=
...
> > +static int vlv_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_stat=
e)
> >  {
> > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > @@ -2153,12 +2138,12 @@ static int vlv_modeset_calc_cdclk(struct intel_=
atomic_state *state)
> >  	return 0;
> >  }
> >  =
...
> > +static int bdw_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_stat=
e)
> >  {
> > -	struct intel_cdclk_state *cdclk_state =3D &state->cdclk_state;
> > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
...
> > @@ -2185,10 +2170,10 @@ static int bdw_modeset_calc_cdclk(struct intel_=
atomic_state *state)
> >  	return 0;
> >  }
> >  =
...
> >  	struct intel_crtc_state *crtc_state;
> >  	int vco, i;
> > @@ -2222,16 +2207,16 @@ static int skl_dpll0_vco(struct intel_atomic_st=
ate *state)
> >  	return vco;
> >  }
> >  =
...
> > +static int skl_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_stat=
e)
> >  {
> > -	struct intel_cdclk_state *cdclk_state =3D &state->cdclk_state;
> > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
...
> > @@ -2258,17 +2243,17 @@ static int skl_modeset_calc_cdclk(struct intel_=
atomic_state *state)
> >  	return 0;
> >  }
> >  =
...
> > +static int bxt_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_stat=
e)
> >  {
> > +	struct intel_atomic_state *state =3D cdclk_state->base.state;
> >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
...
> > @@ -2335,7 +2320,7 @@ static int intel_modeset_all_pipes(struct intel_a=
tomic_state *state)
> >  	return 0;
> >  }
> >  =
...
> > +static int fixed_modeset_calc_cdclk(struct intel_cdclk_state *cdclk_st=
ate)
> >  {
> >  	int min_cdclk;
> >  =

> > @@ -2344,54 +2329,95 @@ static int fixed_modeset_calc_cdclk(struct inte=
...
> > +static struct intel_global_state *intel_cdclk_duplicate_state(struct i=
ntel_global_obj *obj)
> > +{
> > +	struct intel_cdclk_state *cdclk_state;
> > +
...
> > +
> > +	cdclk_state =3D intel_atomic_get_global_obj_state(state, &dev_priv->c=
dclk.obj);
> > +	if (IS_ERR(cdclk_state))
...
> >  {
> >  	struct drm_i915_private *dev_priv =3D to_i915(state->base.dev);
> > -	const struct intel_cdclk_state *old_cdclk_state =3D &dev_priv->cdclk_=
state;
...
> > -	memcpy(new_cdclk_state->min_voltage_level, old_cdclk_state->min_volta=
ge_level,
...
> > -		new_cdclk_state->force_min_cdclk =3D old_cdclk_state->force_min_cdcl=
k;
...
> >  			return ret;
> >  	} else {
> > diff --git a/drivers/gpu/drm/i915/display/intel_cdclk.h b/drivers/gpu/d=
rm/i915/display/intel_cdclk.h
...
> > @@ -31,12 +61,21 @@ void intel_update_cdclk(struct drm_i915_private *de=
v_priv);
...
> >  void intel_set_cdclk_post_plane_update(struct intel_atomic_state *stat=
e);
> >  void intel_dump_cdclk_config(const struct intel_cdclk_config *cdclk_co=
nfig,
> >  			     const char *context);
...
> > +intel_atomic_get_cdclk_state(struct intel_atomic_state *state);
> > +
> > +#define to_intel_cdclk_state(x) container_of((x), struct intel_cdclk_s=
tate, base)
> > +#define intel_atomic_get_old_cdclk_state(state) \
> > +	to_intel_cdclk_state(intel_atomic_get_old_global_obj_state(state, &to=
_i915(state->base.dev)->cdclk.obj))
> > +#define intel_atomic_get_new_cdclk_state(state) \
> > +	to_intel_cdclk_state(intel_atomic_get_new_global_obj_state(state, &to=
_i915(state->base.dev)->cdclk.obj))
> > +
> > +int intel_cdclk_init(struct drm_i915_private *dev_priv);
> > +
...
> > @@ -7482,7 +7482,7 @@ static void intel_crtc_disable_noatomic(struct in=
tel_crtc *crtc,
...
> > @@ -7780,17 +7780,17 @@ bool hsw_crtc_state_ips_capable(const struct in=
...
> > @@ -7799,18 +7799,27 @@ static bool hsw_compute_ips_config(struct intel=
...
> >  static bool intel_crtc_supports_double_wide(const struct intel_crtc *c=
rtc)
> > @@ -12543,14 +12552,11 @@ static u16 hsw_linetime_wm(const struct intel=
_crtc_state *crtc_state)
...
> >  		return 0;
> > @@ -12580,6 +12586,32 @@ static u16 skl_linetime_wm(const struct intel_=
...
> >  				   struct intel_crtc *crtc)
> >  {
> > @@ -12615,7 +12647,6 @@ static int intel_crtc_atomic_check(struct intel=
_atomic_state *state,
> >  			return ret;
> >  	}
> >  =
...
> >  		if (ret) {
> > @@ -12646,20 +12677,25 @@ static int intel_crtc_atomic_check(struct int=
...
> >  static void intel_modeset_update_connector_atomic_state(struct drm_dev=
ice *dev)
> > @@ -14478,7 +14514,7 @@ static bool active_planes_affects_min_cdclk(str=
...
> > @@ -14532,8 +14568,11 @@ static int intel_atomic_check_planes(struct in=
...
> > @@ -14766,18 +14806,18 @@ static int intel_atomic_check(struct drm_devi=
ce *dev,
> >  	if (ret)
> >  		goto fail;
> >  =
...
> > @@ -15676,8 +15716,6 @@ static int intel_atomic_commit(struct drm_devic=
e *dev,
...
> > @@ -17356,7 +17394,7 @@ void intel_init_display_hooks(struct drm_i915_p=
rivate *dev_priv)
...
> >  	if (ret)
> >  		return ret;
> > @@ -18071,6 +18113,8 @@ static void readout_plane_state(struct drm_i915=
_private *dev_priv)
...
> > @@ -18190,8 +18234,6 @@ static void intel_modeset_readout_hw_state(stru=
ct drm_device *dev)

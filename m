...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 32AE16EF50E
	for <lists+intel-gfx@lfdr.de>; Wed, 26 Apr 2023 15:07:01 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 84C7F10E999;
	Wed, 26 Apr 2023 13:06:59 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga02.intel.com (mga02.intel.com [134.134.136.20])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 1D11B10E996
 for <intel-gfx@lists.freedesktop.org>; Wed, 26 Apr 2023 13:06:55 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
 d=intel.com; i=@intel.com; q=dns/txt; s=Intel;
 t=1682514415; x=1714050415;
 h=from:to:cc:subject:date:message-id:in-reply-to:
 references:mime-version:content-transfer-encoding;
 bh=j95M3GcOwIjFCoPRVXjUC0A0mWOFo5nuQH4g6Hixogk=;
 b=cFNipXnbZ8aIhO//c+ltxRFXdV0m5L6WJn+50CDGmwHIPg6IcfJjcX5Y
 IwC2R4KlYOnUnVZGBxE8ITDxblLheUsUCKItH6ypiQTXOEsvWTfWEyab7
 2wWMGiNjh1zv++4TJc6+mIGgA/g+xuzPT5N4gb4qiJZQu3r8wkhCaFXuu
 lBHyzm5mvy49Ay6pLlr+lekwdH+5eaLWQ6USW63XeZcFQNFI33kkAqSoF
 AOE7ZfpCaXQPxLGvNaIEXBx/6TDcu0jBSbHdNGSXwHHkbvT70PSpEY1q6
 HDUYjufL4GspTU2NamxbScB8ersIvBbCU/rxt3lWZapq5g/JHcfIx+NVM Q==;
X-IronPort-AV: E=McAfee;i="6600,9927,10691"; a="336014033"
X-IronPort-AV: E=Sophos;i="5.99,228,1677571200"; d="scan'208";a="336014033"
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
 by orsmga101.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 26 Apr 2023 06:06:54 -0700
X-ExtLoop1: 1
X-IronPort-AV: E=McAfee;i="6600,9927,10691"; a="1023612248"
X-IronPort-AV: E=Sophos;i="5.99,228,1677571200"; d="scan'208";a="1023612248"
Received: from aeherrer-mobl2.amr.corp.intel.com (HELO
 vgovind2-mobl3.intel.com) ([10.249.37.76])
 by fmsmga005-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 26 Apr 2023 06:06:51 -0700
From: Vinod Govindapillai <vinod.govindapillai@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Wed, 26 Apr 2023 16:06:18 +0300
Message-Id: <20230426130619.1227635-8-vinod.govindapillai@intel.com>
...
Subject: [Intel-gfx] [PATCH v3 7/8] drm/i915/mtl: Add support for PM DEMAND
...
index 000000000000..99536b4c7324
--- /dev/null
...
+				 u8 dbuf_slices)
+{
+	u32 dbufs = dbuf_slices ? min_t(u32, hweight8(dbuf_slices) - 1, 3) : 0;
+
...
+	*reg1 |= XELPDP_PMDEMAND_QCLK_GV_BW(GET_PARAM_VAL(qclk_gv_bw));
+
+	*reg1 &= ~XELPDP_PMDEMAND_VOLTAGE_INDEX_MASK;
+	*reg1 |= XELPDP_PMDEMAND_VOLTAGE_INDEX(GET_PARAM_VAL(voltage_index));
+
+	/* pcode calculates index. Requirement is to set this to 0*/
+	*reg1 &= ~XELPDP_PMDEMAND_QCLK_GV_INDEX_MASK;
+	*reg1 |= XELPDP_PMDEMAND_QCLK_GV_INDEX(GET_PARAM_VAL(qclk_gv_index));
+
+	*reg1 &= ~XELPDP_PMDEMAND_PIPES_MASK;
+	*reg1 |= XELPDP_PMDEMAND_PIPES(GET_PARAM_VAL(active_pipes));
+
...
+	*reg2 |= XELPDP_PMDEMAND_CDCLK_FREQ(GET_PARAM_VAL(cdclk_freq_mhz));
+
+	*reg2 &= ~XELPDP_PMDEMAND_DDICLK_FREQ_MASK;
+	*reg2 |= XELPDP_PMDEMAND_DDICLK_FREQ(GET_PARAM_VAL(ddiclk_freq_mhz));
+
+	/* Hard code scalers to 7*/
+	*reg2 &= ~XELPDP_PMDEMAND_SCALERS_MASK;
+	*reg2 |= XELPDP_PMDEMAND_SCALERS(GET_PARAM_VAL(scalers));
+
+	/*

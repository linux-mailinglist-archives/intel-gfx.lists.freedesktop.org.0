...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id CE690224B4C
	for <lists+intel-gfx@lfdr.de>; Sat, 18 Jul 2020 15:08:34 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id CE21F6E21A;
	Sat, 18 Jul 2020 13:08:27 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from asavdk4.altibox.net (asavdk4.altibox.net [109.247.116.15])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 9E5736E087;
 Sat, 18 Jul 2020 13:08:24 +0000 (UTC)
Received: from ravnborg.org (unknown [188.228.123.71])
 (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
 (No client certificate requested)
 by asavdk4.altibox.net (Postfix) with ESMTPS id 9C4D280513;
 Sat, 18 Jul 2020 15:08:21 +0200 (CEST)
Date: Sat, 18 Jul 2020 15:08:19 +0200
From: Sam Ravnborg <sam@ravnborg.org>
To: Anitha Chrisanthus <anitha.chrisanthus@intel.com>
Message-ID: <20200718130819.GA2990383@ravnborg.org>
...
X-CMAE-Analysis: v=2.3 cv=aP3eV41m c=1 sm=1 tr=0
 ...
 a=8nJEP1OIZ-IA:10 a=QyXUC8HyAAAA:8 a=e5mUnYsNAAAA:8
 a=NbUQyiW4ozcTRDWNEbYA:9 a=7Zwj6sZBwVKJAoWSPKxL6X1jA+E=:19
 a=A-cRaIRomaI41WbL:21 a=f6OysZVNCLMOZr4t:21 a=KCvXBQXxo3Az3Huz:21
...
> +{
> +	struct kmb_crtc *l_crtc =3D to_kmb_crtc(crtc);
> +
...
> +	struct kmb_drm_private *dev_p =3D to_kmb(dev);
bikeshedding...
general comment.
...
> +	val =3D kmb_read_lcd(dev_p, LCD_INT_ENABLE);
...
> +
> +	ret =3D drm_crtc_init_with_planes(drm, &lcd->crtc, &primary->base_plane,
...
> +	return 0;
> +}
> diff --git a/drivers/gpu/drm/kmb/kmb_crtc.h b/drivers/gpu/drm/kmb/kmb_crt=
c.h
> new file mode 100644
...
> +#include "kmb_drv.h"
> +
> +#define to_kmb_crtc_state(x) container_of(x, struct kmb_crtc_state, crtc=
_base)
...
> +	int ret =3D 0;
> +
> +	ret =3D clk_prepare_enable(clk_lcd);
> +	if (ret) {
> +		DRM_ERROR("Failed to enable LCD clock: %d\n", ret);
> +		return ret;
> +	}
> +
> +	ret =3D clk_prepare_enable(clk_mipi);
> +	if (ret) {
> +		DRM_ERROR("Failed to enable MIPI clock: %d\n", ret);
> +		return ret;
> +	}
> +
> +	ret =3D clk_prepare_enable(clk_mipi_ecfg);
> +	if (ret) {
> +		DRM_ERROR("Failed to enable MIPI_ECFG clock: %d\n", ret);
> +		return ret;
> +	}
> +
> +	ret =3D clk_prepare_enable(clk_mipi_cfg);
> +	if (ret) {
...
> +}
> +
> +static void __iomem *kmb_map_mmio(struct platform_device *pdev, char *na=
me)
> +{
> +	struct resource *res;
> +	u32 size;
...
> +{
> +	struct kmb_drm_private *dev_p =3D to_kmb(drm);
...
> +	dev_p->mipi_mmio =3D kmb_map_mmio(pdev, "mipi_regs");
...
> +	dev_p->lcd_mmio =3D kmb_map_mmio(pdev, "lcd_regs");
...
> +	 */
> +	dev_p->msscam_mmio =3D kmb_map_mmio(pdev, "msscam_regs");
...
> +	}
> +#ifdef ICAM_LCD_QOS
> +	dev_p->icamlcd_mmio =3D ioremap_nocache(ICAM_MMIO, ICAM_MMIO_SIZE);
...
> +	clk_lcd =3D clk_get(&pdev->dev, "clk_lcd");
> +	if (IS_ERR(clk_lcd)) {
> +		drm_err(&dev_p->drm, "clk_get() failed clk_lcd\n");
> +		goto setup_fail;
> +	}
> +
> +	clk_mipi =3D clk_get(&pdev->dev, "clk_mipi");
> +	if (IS_ERR(clk_mipi)) {
> +		drm_err(&dev_p->drm, "clk_get() failed clk_mipi\n");
> +		goto setup_fail;
> +	}
> +
> +	clk_mipi_ecfg =3D clk_get(&pdev->dev, "clk_mipi_ecfg");
> +	if (IS_ERR(clk_mipi_ecfg)) {
> +		drm_err(&dev_p->drm, "clk_get() failed clk_mipi_ecfg\n");
> +		goto setup_fail;
> +	}
> +
> +	clk_mipi_cfg =3D clk_get(&pdev->dev, "clk_mipi_cfg");
> +	if (IS_ERR(clk_mipi_cfg)) {
> +		drm_err(&dev_p->drm, "clk_get() failed clk_mipi_cfg\n");
> +		goto setup_fail;
> +	}
> +	vpu_dev =3D of_find_node_by_path("/soc/vpu-ipc");
> +	clk_pll0 =3D of_clk_get_by_name(vpu_dev, "pll_0_out_0");
> +	if (IS_ERR(clk_pll0)) {
> +		drm_err(&dev_p->drm, "clk_get() failed clk_pll0 ");
> +		goto setup_fail;
> +	}
> +	dev_p->sys_clk_mhz =3D clk_get_rate(clk_pll0)/1000000;
> +	drm_info(&dev_p->drm, "system clk =3D %d Mhz", dev_p->sys_clk_mhz);
> +
> +	/* Set LCD clock to 200 Mhz */
> +	ret =3D clk_set_rate(clk_lcd, KMB_LCD_DEFAULT_CLK);
> +	if (clk_get_rate(clk_lcd) !=3D KMB_LCD_DEFAULT_CLK) {
> +		drm_err(&dev_p->drm, "failed to set to clk_lcd to %d\n",
> +			  KMB_LCD_DEFAULT_CLK);
> +	}
> +	drm_info(&dev_p->drm, "clk_lcd =3D %ld\n", clk_get_rate(clk_lcd));
> +
> +	/* Set MIPI clock to 24 Mhz */
> +	ret =3D clk_set_rate(clk_mipi, KMB_MIPI_DEFAULT_CLK);
> +	if (clk_get_rate(clk_mipi) !=3D KMB_MIPI_DEFAULT_CLK) {
> +		drm_err(&dev_p->drm, "failed to set to clk_mipi to %d\n",
> +			  KMB_MIPI_DEFAULT_CLK);
> +		goto setup_fail;
> +	}
> +	drm_info(&dev_p->drm, "clk_mipi =3D %ld\n", clk_get_rate(clk_mipi));
> +
> +	clk =3D clk_get_rate(clk_mipi_ecfg);
> +	if (clk !=3D KMB_MIPI_DEFAULT_CFG_CLK) {
> +		/* Set MIPI_ECFG clock to 24 Mhz */
> +		ret =3D clk_set_rate(clk_mipi_ecfg, KMB_MIPI_DEFAULT_CFG_CLK);
> +		clk =3D clk_get_rate(clk_mipi_ecfg);
> +		if (clk !=3D KMB_MIPI_DEFAULT_CFG_CLK) {
> +			drm_err(&dev_p->drm,
...
> +		}
> +
> +		drm_info(&dev_p->drm, "clk_mipi_ecfg after set =3D %ld\n", clk);
> +	}
> +
> +	clk =3D clk_get_rate(clk_mipi_cfg);
> +	if (clk !=3D KMB_MIPI_DEFAULT_CFG_CLK) {
> +		/* Set MIPI_CFG clock to 24 Mhz */
> +		ret =3D clk_set_rate(clk_mipi_cfg, 24000000);
> +		clk =3D clk_get_rate(clk_mipi_cfg);
> +		if (clk !=3D KMB_MIPI_DEFAULT_CFG_CLK) {
> +			drm_err(&dev_p->drm,
...
> +	}
> +
> +	dev_p->irq_lcd =3D irq_lcd;
> +
> +	/* icam tests */
> +#ifdef ICAM_LCD_QOS
> +	/*generator mode =3D 0 fixed mode=3D1 limiter */
> +	writel(1, (dev_p->icamlcd_mmio + ICAM_LCD_OFFSET + LCD_QOS_MODE));
> +	/* b/w */
> +	writel(0x60, (dev_p->icamlcd_mmio + ICAM_LCD_OFFSET + LCD_QOS_BW));
> +
> +	/* set priority.p1 */
> +	val =3D readl(dev_p->icamlcd_mmio + ICAM_LCD_OFFSET + LCD_QOS_PRORITY);
> +	val &=3D ~(0x700);
> +	writel(val | 0x100,
> +	       (dev_p->icamlcd_mmio + ICAM_LCD_OFFSET + LCD_QOS_PRORITY));
> +
> +	drm_info(&dev_p->drm,
> +			"ICAM mode =3D 0x%x, priority =3D 0x%x bandwidth=3D0x%x",
...
> +	struct kmb_drm_private *dev_p =3D to_kmb(dev);
> +
> +	status =3D kmb_read_lcd(dev_p, LCD_INT_STATUS);
> +
...
> +				val =3D kmb_read_lcd(dev_p, dma0_state)
> +				    & LCD_DMA_STATE_ACTIVE;
> +				val1 =3D kmb_read_lcd(dev_p, dma1_state)
> +				    & LCD_DMA_STATE_ACTIVE;
> +			} while ((val || val1));
> +			/*disable dma */
...
> +		/* Read VSTATUS */
> +		val =3D kmb_read_lcd(dev_p, LCD_VSTATUS);
...
> +
> +	/* Create DRM device */
> +	dev_p =3D devm_drm_dev_alloc(dev, &kmb_driver,
...
> +
> +	dev_p->drm.dev_private =3D dev_p;
...
> +
> +	/* Load driver */
> +	dev_p->n_layers =3D KMB_MAX_PLANES;
> +	ret =3D kmb_load(&dev_p->drm, 0);
...
> +	dev_p->crtc.port =3D of_graph_get_port_by_id(dev->of_node, 0);
> +	ret =3D drm_vblank_init(&dev_p->drm, dev_p->drm.mode_config.num_crtc);
> +	drm_info(&dev_p->drm, "mode_config.num_crtc=3D%d\n",
...
> +	ret =3D drm_dev_register(&dev_p->drm, 0);
> +
> +	if (ret)
> +		goto err_register;
> +
...
> +{
> +	struct drm_device *drm =3D dev_get_drvdata(dev);
> +	struct kmb_drm_private *lcd =3D drm ? drm->dev_private : NULL;
...
> +
> +	lcd->state =3D drm_atomic_helper_suspend(drm);
> +	if (IS_ERR(lcd->state)) {
...
> +}
> +
> +static void mipi_tx_fg_cfg_regs(struct kmb_drm_private *dev_p, u8 frame_=
gen,
...
> +	 */
> +	sysclk =3D dev_p->sys_clk_mhz - 50;
> +
...
> +
> +	drm_dbg(&dev_p->drm, "ppl_llp_ratio=3D%d\n", ppl_llp_ratio);
> +	drm_dbg(&dev_p->drm, "bpp=3D%d sysclk=3D%d lane-rate=3D%d activ-lanes=
=3D%d\n",
...
> +
> +	drm_dbg(&dev_p->drm, "sync_cfg=3D%d fg_en=3D%d\n", sync_cfg, fg_en);
> +
> +	/* MIPI_TX_HS_CTRL */
> +
> +	/* type:DSI, source:LCD */
...
> +			if (frame->sections[sect] =3D=3D NULL)
> +				continue;
> +
> +			ret =3D mipi_tx_fg_section_cfg(dev_p, frame_id, sect,
> +						     frame->sections[sect],
> +						     &bits_per_pclk,
> +						     &word_count);
> +			if (ret)
...
> +#endif
> +
> +	drm_dbg(&dev_p->drm, "IRQ_STATUS =3D 0x%x\n",
...
> +
> +	if ((dphy_sel >> 0 & 0xf) > 0)
> +		reg_rd_data =3D kmb_read_mipi(dev_p, DPHY_TEST_DOUT0_3);
> +	if ((dphy_sel >> 4 & 0xf) > 0)
> +		reg_rd_data =3D kmb_read_mipi(dev_p, DPHY_TEST_DOUT4_7);
> +	if ((dphy_sel >> 8 & 0x3) > 0)
> +		reg_rd_data =3D kmb_read_mipi(dev_p, DPHY_TEST_DOUT8_9);
> +
> +	if (((dphy_sel >> 0 & 0x1) =3D=3D 1) | ((dphy_sel >> 4 & 0x1) =3D=3D 1)=
 |
> +	    ((dphy_sel >> 8 & 0x1) =3D=3D 1))
...
> +	drm_dbg(&dev_p->drm, "m =3D %d n =3D %d\n", best_m, best_n);
> +
...
> +
> +	drm_dbg(&dev_p->drm, "dphy %d fsm_state =3D 0%x\n", dphy_no,
...
> +		 active_lanes);
> +	drm_dbg(&dev_p->drm, "MIPI_DPHY_STAT0_4_7 =3D 0x%x)\n",
...
> +	val =3D kmb_read_mipi(dev_p, DPHY_INIT_CTRL0);
> +
> +	drm_dbg(&dev_p->drm, "DPHY_INIT_CTRL0 =3D 0x%x\n", val);
> +
> +	/* Init D-PHY_n
...
> +	ndelay(15);
> +
> +	drm_dbg(&dev_p->drm, "DPHY_TEST_CTRL0=3D0x%x\n",
...
> +
> +	drm_dbg(&dev_p->drm, "DPHY_FREQ =3D 0x%x\n",
...
> +
> +	drm_dbg(&dev_p->drm, "DPHY_CFG_CLK_EN =3D 0x%x\n",
...
> +
> +		drm_dbg(&dev_p->drm, "DPHY_INIT_CTRL1 =3D 0x%x\n",
...
> +
> +	drm_dbg(&dev_p->drm, "DPHY_INIT_CTRL2 =3D 0x%x\n",
...
> +		test_mode_send(dev_p, dphy_no, TEST_CODE_FSM_CONTROL, 0x80);
> +
> +		val =3D GET_TEST_DOUT4_7(dev_p, dphy_no);
> +		i++;
> +		if (i > TIMEOUT) {
> +			status =3D 0;
> +			break;
...
> +
> +	drm_dbg(&dev_p->drm, "%s: dphy %d val =3D %x\n", __func__, dphy_no, val=
);
...
> +	int status =3D 1;
> +
> +	drm_dbg(&dev_p->drm, "dphy=3D%d active_lanes=3D%d data_lanes=3D%d\n", d=
phy_no,
> +		 active_lanes, data_lanes);
> +
> +	do {
> +		val =3D kmb_read_mipi(dev_p, MIPI_DPHY_STAT4_7);
> +		stopstatedata =3D GET_STOPSTATE_DATA(dev_p, dphy_no) & data_lanes;
> +
...
> +		 *http://dub30.ir.intel.com/bugzilla/show_bug.cgi?id=3D27785#c12:
> +		 *a. start init PHY1
...
> +		    1000000;
> +
> +		drm_info(&dev_p->drm, "htotal=3D%d vtotal=3D%d refresh=3D%d\n",
> +			 mode->crtc_htotal, mode->crtc_vtotal,
> +			 drm_mode_vrefresh(mode));
> +		drm_info(&dev_p->drm, "data_rate=3D%u active_lanes=3D%d\n",
...
> +		drm_dbg(&dev_p->drm,
> +		     "vfp=3D %d vbp=3D %d vsyc_len=3D%d hfp=3D%d hbp=3D%d hsync_len=3D=
%d lane-rate=3D%d",
...
> +
> +	drm_dbg(&dev_p->drm, "MIPI_TXm_HS_CTRL =3D 0x%x\n",
> +		 kmb_read_mipi(dev_p, MIPI_TXm_HS_CTRL(6)));
> +	drm_dbg(&dev_p->drm, "MIPI LOOP BACK =3D %x\n",
> +		 kmb_read_mipi(dev_p, MIPI_CTRL_DIG_LOOPBACK));
> +	drm_info(&dev_p->drm, "mipi hw_initialized =3D %d\n", hw_initialized);
> +
> +	return 0;
> +}
> +
...
> +	drm_dbg(&dev_p->drm, "dma_len=3D%d ", dma_len);
...
> +	drm_dbg(&dev_p->drm, "dma_cfg=3D0x%x LCD_DMA_CFG=3D0x%x\n", dma_cfg,
...
> +		plane =3D devm_kzalloc(drm->dev, sizeof(*plane), GFP_KERNEL);
...
> diff --git a/drivers/gpu/drm/kmb/kmb_regs.h b/drivers/gpu/drm/kmb/kmb_reg=
s.h
> new file mode 100644
...
> +#define LCD_LAYERn_TRANS_COLOUR_LS(N)	(LCD_LAYER0_TRANS_COLOUR_LS + (0x4=
00*N))
> +#define LCD_LAYER0_TRANS_COLOUR_MS	(0x4 * 0x10a)
> +#define LCD_LAYERn_TRANS_COLOUR_MS(N)	(LCD_LAYER0_TRANS_COLOUR_MS + (0x4=
00*N))
...
> +#define LCD_LAYERn_DMA_START_CB_SHADOW(N)	(LCD_LAYER0_DMA_START_CB_SHADO=
W\
> +						+ (0x20*N))
...
> +#define LCD_LAYERn_DMA_CB_LINE_VSTRIDE(N)	(LCD_LAYER0_DMA_CB_LINE_VSTRID=
E\
> +						+ (0x20*N))
...
> +#define LCD_LAYERn_DMA_CR_LINE_VSTRIDE(N)	(LCD_LAYER0_DMA_CR_LINE_VSTRID=
E\
> +						+ (0x20*N))

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 8FEA81BF69D
	for <lists+intel-gfx@lfdr.de>; Thu, 30 Apr 2020 13:22:09 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id E5EB46EB91;
	Thu, 30 Apr 2020 11:22:07 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga12.intel.com (mga12.intel.com [192.55.52.136])
 by gabe.freedesktop.org (Postfix) with ESMTPS id F2CC76EB91
 for <intel-gfx@lists.freedesktop.org>; Thu, 30 Apr 2020 11:22:05 +0000 (UTC)
IronPort-SDR: gFVfcusUYQIV9d4eS+7oitwfJ1NbjkKzsYH6ySU6iOm77eLkjzQaKL8FclH2T7064Sy4T60TvQ
 bTDa9yl1QAoA==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
 by fmsmga106.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 30 Apr 2020 04:22:05 -0700
IronPort-SDR: 6dxXZAZ315RGBJVpN32Urpo5DNFbOnN70TaV7xHyWt8EA/uCV60X97V1B4wXkXNZL+stj5VHmD
 8+shH7xjyodQ==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,334,1583222400"; d="scan'208";a="293529613"
Received: from stinkbox.fi.intel.com (HELO stinkbox) ([10.237.72.174])
 by fmsmga002.fm.intel.com with SMTP; 30 Apr 2020 04:22:02 -0700
Received: by stinkbox (sSMTP sendmail emulation);
 Thu, 30 Apr 2020 14:22:02 +0300
Date: Thu, 30 Apr 2020 14:22:02 +0300
From: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
To: "Lisovskiy, Stanislav" <stanislav.lisovskiy@intel.com>
Message-ID: <20200430112202.GU6112@intel.com>
...
In-Reply-To: <20200430110702.GA22144@intel.com>
...
> > > > On Thu, Apr 30, 2020 at 01:05:15PM +0300, Lisovskiy, Stanislav wrot=
e:
> > > > > On Thu, Apr 30, 2020 at 12:21:04PM +0300, Ville Syrj=E4l=E4 wrote:
> > > > > > On Thu, Apr 23, 2020 at 10:58:56AM +0300, Stanislav Lisovskiy w=
rote:
...
> > > > > > >  #define to_intel_bw_state(x) container_of((x), struct intel_=
bw_state, base)
...
> > > > > > > @@ -3874,6 +3874,7 @@ static int intel_compute_sagv_mask(stru=
...
> > > > > > >  					 new_crtc_state, i) {
> > > > > > > @@ -3883,6 +3884,12 @@ static int intel_compute_sagv_mask(str=
...
> > > > > Well, that was my question actually here as well. I understand th=
at changing
> > > > > state->active_pipes here feels like some unneeded side effect, ho=
wever having
> > > > > state->active_pipes and bw_state->active_pipes going out of sync =
doesn't sound
> > > > > very attractive to me either. That is why I don't like this idea =
of duplication
> > > > > at all - having constant need to sync those state, bw_state, cdcl=
k_state, because
> > > > > they all might have different active_pipes now.
> > > > =

> > > > Having an out of date active_pipes anywhere would be a bug in that
> > > > specific code. Also state->active_pipes is definitely going the way=
 of
> > > > the dodo soon.
> > > > =
...
> > > > > > > +				intel_calc_active_pipes(state, old_bw_state->active_pipe=
s);
> > > > > > > +			active_pipes_calculated =3D true;
> > > > > > > +		}
> > > > > > =

> > > > > > I'd do this after the loop so we don't need this extra boolean.=
 As far
> > > > > > as the active_pipes check in intel_crtc_can_enable_sagv(), I th=
ink we
> > > > > > can pull it out into intel_compute_sagv_mask() so that we do th=
e check
> > > > > > after computing the mask. And of course change it to use
> > > > > > bw_state->active_pipes instead.
> > > > > =

> > > > > intel_crtc_can_enable_sagv is called per crtc - so can't just pul=
l it out, =

> > > > > will have to have to cycles then - one will compute bw_state->act=
ive_pipes,
> > > > > and another pipe_sagv_mask.
> > > > =

> > > > Hmm. Actually I think what we should probably do is keep the
> > > > active_pipes check in intel_can_enable_sagv(). Ie something like th=
is:
> > > > =
...
> > > I need active_pipes check here for skl code only, as it disables SAGV=
 for multipipe
> > > scenarios. Adding this here would generalize it for other platforms a=
nd we
...
> > > In fact that is the only reason I need active pipes here - otherwise =
I think
> > > it was even your comment that we actually don't need those here at al=
l,
> > > as we just iterate through crtcs in state - pretty clearly remember w=
e discussed
...
> > > > We can then make the check conditional on pre-icl (or whatever we w=
ant)
> > > > in a later patch. And finally we can remove it altogether in a sepa=
rate
...
> > > > > > We're also going to need to lock_global_state() if bw_state->ac=
tive_pipes
> > > > > > mask changes.
> > > > > =
...
> > > > > > >  		else
> > > > > > > @@ -5911,11 +5918,9 @@ skl_compute_wm(struct intel_atomic_sta=
te *state)
> > > > > > >  	if (ret)
> > > > > > >  		return ret;
> > > > > > >  =

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id D24601BBAC8
	for <lists+intel-gfx@lfdr.de>; Tue, 28 Apr 2020 12:08:36 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 50B226E293;
	Tue, 28 Apr 2020 10:08:34 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga02.intel.com (mga02.intel.com [134.134.136.20])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 1E3316E271
 for <intel-gfx@lists.freedesktop.org>; Tue, 28 Apr 2020 10:08:31 +0000 (UTC)
IronPort-SDR: mA5LlEnt+pSuQAnF5zRczSDfEZuWJHezCJyVYGr6k/3lXFdRCA2btylC8rVh/dWSlgyIkLIjzL
 rrNfaFt5YfPQ==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga005.jf.intel.com ([10.7.209.41])
 by orsmga101.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 28 Apr 2020 03:08:30 -0700
IronPort-SDR: gHR7EdYlOWkgbTNcJG4zbWrrCkpJTg+0R6+A87FnlXR3faKF05JIyQlP6bXK6UBz7o0/MM3/ZA
 +29hS57PChSg==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,327,1583222400"; d="scan'208";a="432128941"
Received: from pbooyens-mobl1.ger.corp.intel.com (HELO
 delly.ger.corp.intel.com) ([10.252.37.143])
 by orsmga005.jf.intel.com with ESMTP; 28 Apr 2020 03:08:28 -0700
From: Lionel Landwerlin <lionel.g.landwerlin@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Tue, 28 Apr 2020 13:08:15 +0300
Message-Id: <20200428100816.951014-4-lionel.g.landwerlin@intel.com>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH v7 3/4] drm/i915/perf: prepare driver to receive
...
---
 drivers/gpu/drm/i915/i915_perf.c       | 553 +++++++++++++++----------
 drivers/gpu/drm/i915/i915_perf_types.h |  37 +-
 2 files changed, 357 insertions(+), 233 deletions(-)

diff --git a/drivers/gpu/drm/i915/i915_perf.c b/drivers/gpu/drm/i915/i915_perf.c
index dad596f478ff..79f68efd7d5b 100644
...
+	return err;
 ...
 static void
@@ -1405,8 +1469,7 @@ static void i915_oa_stream_destroy(struct i915_perf_stream *stream)
...
@@ -1498,7 +1561,7 @@ static void gen8_init_oa_buffer(struct i915_perf_stream *stream)
...
@@ -1552,7 +1615,7 @@ static void gen12_init_oa_buffer(struct i915_perf_stream *stream)
 	 * reports we will forward to userspace while filtering for a single
 	 * context.
 	 */
-	stream->oa_buffer.last_ctx_id = INVALID_CTX_ID;
...
@@ -2275,11 +2338,10 @@ static int gen8_configure_context(struct i915_perf_stream *stream,
 	return err;
 }
...
+	int i;
 	...
 	struct flex regs_context[] = {
 		{
@@ -2300,7 +2362,7 @@ static int gen12_configure_oar_context(struct i915_perf_stream *stream,
...
+			RING_CONTEXT_CONTROL(stream->engine->mmio_base),
 			...
 			_MASKED_FIELD(GEN12_CTX_CTRL_OAR_CONTEXT_ENABLE,
 				      active ?
@@ -2309,18 +2371,28 @@ static int gen12_configure_oar_context(struct i915_perf_stream *stream,
 		},
 	};
...
+	return 0;
 ...
 /*
@@ -2576,11 +2648,9 @@ gen12_enable_metric_set(struct i915_perf_stream *stream,
...
@@ -2621,8 +2691,7 @@ static void gen12_disable_metric_set(struct i915_perf_stream *stream,
...
@@ -2631,8 +2700,7 @@ static void gen12_disable_metric_set(struct i915_perf_stream *stream,
...
@@ -2654,7 +2722,7 @@ static void gen7_oa_enable(struct i915_perf_stream *stream)
...
@@ -2871,7 +2939,7 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	}
...
@@ -2919,12 +2987,10 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	if (stream->periodic)
...
@@ -3023,8 +3089,7 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	free_noa_wait(stream);
...
@@ -3217,8 +3282,12 @@ static void i915_perf_enable_locked(struct i915_perf_stream *stream)
...
+	}
 ...
 /**
@@ -3243,8 +3312,12 @@ static void i915_perf_disable_locked(struct i915_perf_stream *stream)
...
@@ -3262,7 +3335,7 @@ static long i915_perf_config_locked(struct i915_perf_stream *stream,
 		return -EINVAL;
...
+		int i;
 		...
 		if (!active) {
@@ -3270,28 +3343,29 @@ static long i915_perf_config_locked(struct i915_perf_stream *stream,
 			goto err_config;
 		}
...
@@ -3372,9 +3446,10 @@ static void i915_perf_destroy_locked(struct i915_perf_stream *stream)
...
@@ -3449,25 +3524,12 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
...
@@ -3488,17 +3550,16 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
...
@@ -3519,23 +3580,43 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
 	if (privileged_op &&
...
@@ -3570,12 +3651,11 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
 err_flags:
 	if (stream->ops->destroy)
 		stream->ops->destroy(stream);
-err_alloc:
...
@@ -3607,7 +3687,7 @@ static int read_properties_unlocked(struct i915_perf *perf,
 {
...
@@ -3641,23 +3721,36 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+			props->n_ctx_handles = 1;
 		...
 		case DRM_I915_PERF_PROP_SAMPLE_OA:
 			if (value)
@@ -3666,7 +3759,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			break;
@@ -3674,12 +3768,14 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			break;
@@ -3687,7 +3783,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
@@ -3716,7 +3813,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
@@ -3732,13 +3830,14 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			props->has_sseu = true;
 			break;
@@ -3747,19 +3846,25 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+	return err;
 ...
 /**
@@ -3819,6 +3924,8 @@ int i915_perf_open_ioctl(struct drm_device *dev, void *data,

...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 1A4353C6037
	for <lists+intel-gfx@lfdr.de>; Mon, 12 Jul 2021 18:12:54 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id EF61389CE2;
	Mon, 12 Jul 2021 16:12:46 +0000 (UTC)
X-Original-To: Intel-gfx@lists.freedesktop.org
Delivered-To: Intel-gfx@lists.freedesktop.org
Received: from mail-wm1-x32a.google.com (mail-wm1-x32a.google.com
 [IPv6:2a00:1450:4864:20::32a])
 by gabe.freedesktop.org (Postfix) with ESMTPS id ABF8889CDD
 for <Intel-gfx@lists.freedesktop.org>; Mon, 12 Jul 2021 16:12:45 +0000 (UTC)
Received: by mail-wm1-x32a.google.com with SMTP id
 l17-20020a05600c1d11b029021f84fcaf75so4993125wms.1
 for <Intel-gfx@lists.freedesktop.org>; Mon, 12 Jul 2021 09:12:45 -0700 (PDT)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 ...
 :in-reply-to;
 bh=7ZFGM47a/9pzzSS4fNHCkJ5x5r3OeKDf04+7MDXeN7Y=;
 b=
X-Gm-Message-State: AOAM532wwzKjju4WbKai4iUg7Msh8w3fahRUgGMNzm6ChAMbKhOWKOGe
 E3zY9x8D6yO8U76jUniNLV4+tg==
...
> > > consolidating between multiple file descriptors owned by the same pro=
cess.
> > > =
...
> then needed to be freed via RCU due sysfs access to it!), but then the id=
ea
...
> ctx->pid to go via ctx->client (process name as well). Those are async en=
try
...
> file_priv could remain being immediately freed on file close. Or perhaps =
we
...
> given error capture and debugfs can be async to file_priv lifetime that's=
 a
> benefit for me.
...
> actually the opposite from what you suggest. Whereas with the sysfs appro=
ach
...
> Also note that even from userspace semantics of getting the pid from proc=
fs
...
> /proc/<pid>/fd/<fd> - to filter out non drm fds avoiding text parsing abo=
ve
> file
...
do
> the rest via openat, I am hoping at least that's safe againt pid swaps. F=
or
...
> > > +}
> > > +
> > > +static void __i915_drm_client_unregister(struct i915_drm_client *cli=
ent)
> > > +{
...
> > > +i915_drm_client_add(struct i915_drm_clients *clients, struct task_st=
ruct *task)
> > > +{
...
> > > +void __i915_drm_client_free(struct kref *kref);
> > > +
> > > +static inline void i915_drm_client_put(struct i915_drm_client *clien=
t)
> > > +{
...
> > >   #include "i915_irq.h"
> > > @@ -343,6 +344,8 @@ static int i915_driver_early_probe(struct drm_i91=
5_private *dev_priv)
...
> > > @@ -362,6 +365,7 @@ static int i915_driver_early_probe(struct drm_i91=
5_private *dev_priv)
> > >   err_gem:
...
> > >   err_ttm:
> > > @@ -381,6 +385,7 @@ static void i915_driver_late_release(struct drm_i=
...
> > > @@ -996,6 +1001,7 @@ static void i915_driver_postclose(struct drm_dev=
ice *dev, struct drm_file *file)
...
> > > @@ -1179,25 +1179,40 @@ void i915_gem_cleanup_early(struct drm_i915_p=
rivate *dev_priv)
> > >   int i915_gem_open(struct drm_i915_private *i915, struct drm_file *f=
ile)
> > >   {

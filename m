...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id B503C2ED3F4
	for <lists+intel-gfx@lfdr.de>; Thu,  7 Jan 2021 17:10:33 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 2D1676E4AE;
	Thu,  7 Jan 2021 16:10:32 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga04.intel.com (mga04.intel.com [192.55.52.120])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 94F5C6E4AE
 for <intel-gfx@lists.freedesktop.org>; Thu,  7 Jan 2021 16:10:30 +0000 (UTC)
IronPort-SDR: ZwN/67pucBWHG1jhtNDv6G4lMeirrO3fs8+0wggIjQz2heS4QhxXHuQyItm3pHSx47km0Cg8sz
 P6rTjOQFA4Vw==
X-IronPort-AV: E=McAfee;i="6000,8403,9857"; a="174868212"
X-IronPort-AV: E=Sophos;i="5.79,329,1602572400"; d="scan'208";a="174868212"
Received: from orsmga008.jf.intel.com ([10.7.209.65])
 by fmsmga104.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 07 Jan 2021 08:10:29 -0800
IronPort-SDR: puvZmWQ85nvTRI+cND0lQ+opA/R9wH042BSxqBStorNQw8nJoid5NJXSc2mCngrMeELOx0DTWC
 rbXnJhdF8zNA==
X-IronPort-AV: E=Sophos;i="5.79,329,1602572400"; d="scan'208";a="379763776"
Received: from unknown (HELO sdutt-i7) ([10.165.21.147])
 by orsmga008-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 07 Jan 2021 08:10:28 -0800
Date: Thu, 7 Jan 2021 08:05:07 -0800
From: Matthew Brost <matthew.brost@intel.com>
To: Chris Wilson <chris@chris-wilson.co.uk>
Message-ID: <20210107160507.GA31817@sdutt-i7>
References: <20201229120145.26045-1-chris@chris-wilson.co.uk>
 <20201229120145.26045-36-chris@chris-wilson.co.uk>
MIME-Version: 1.0
Content-Disposition: inline
In-Reply-To: <20201229120145.26045-36-chris@chris-wilson.co.uk>
...
>  drivers/gpu/drm/i915/i915_scheduler.c         | 431 +++++++++++++-----
...
>  16 files changed, 634 insertions(+), 262 deletions(-)
> 
> diff --git a/drivers/gpu/drm/i915/gt/intel_engine_cs.c b/drivers/gpu/drm/i915/gt/intel_engine_cs.c
> index eb429a90518e..4854a8d15ef4 100644
...
> index a8820ef0282a..3aac8dac5518 100644
...
>  
>  	/*
> @@ -258,25 +258,12 @@ ring_set_paused(const struct intel_engine_cs *engine, int state)
>  
...
> @@ -295,61 +282,61 @@ static struct i915_request *first_request(struct i915_sched_engine *se)
>  	return NULL;
>  }
>  
...
> @@ -361,8 +348,31 @@ static inline bool need_preempt(struct intel_engine_cs *engine,
...
> @@ -379,7 +389,7 @@ assert_priority_queue(const struct i915_request *prev,
...
>  
>  static inline void
> @@ -568,9 +578,12 @@ static inline void __execlists_schedule_out(struct i915_request *rq)
...
> @@ -684,14 +697,14 @@ dump_port(char *buf, int buflen, const char *prefix, struct i915_request *rq)
>  	if (!rq)
>  		return "";
>  
...
>  	return buf;
>  }
> @@ -1199,11 +1212,11 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
>  
...
>  
>  			/*
> @@ -1225,11 +1238,11 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
>  			last = NULL;
...
> @@ -1285,7 +1298,7 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
...
>  			break;
>  		}
> @@ -1299,16 +1312,15 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
>  		}
>  
>  		ENGINE_TRACE(engine,
...
> @@ -1399,6 +1411,10 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
...
> @@ -1435,24 +1451,6 @@ static void execlists_dequeue(struct intel_engine_cs *engine)
>  	}
>  done:
...
>  
>  	/*
> @@ -2646,10 +2644,6 @@ static void execlists_reset_rewind(struct intel_engine_cs *engine, bool stalled)
>  
...
> @@ -2716,16 +2710,12 @@ static void execlists_reset_cancel(struct intel_engine_cs *engine)
>  			rq->engine = engine;
...
>  
> @@ -3125,7 +3115,8 @@ static const struct intel_context_ops virtual_context_ops = {
...
> @@ -3142,9 +3133,11 @@ static intel_engine_mask_t virtual_submission_mask(struct virtual_engine *ve)
...
>  	return mask;
>  }
> @@ -3152,12 +3145,12 @@ static intel_engine_mask_t virtual_submission_mask(struct virtual_engine *ve)
...
>  	if (unlikely(!mask))
>  		return;
> @@ -3190,7 +3183,8 @@ static void virtual_submission_tasklet(unsigned long data)
>  			 */
...
> @@ -3204,7 +3198,7 @@ static void virtual_submission_tasklet(unsigned long data)
>  
>  			rb = *parent;
...
>  			} else {
>  				parent = &rb->rb_right;
> @@ -3219,8 +3213,8 @@ static void virtual_submission_tasklet(unsigned long data)
>  
>  submit_engine:
...
>  
>  unlock_engine:
> @@ -3256,7 +3250,9 @@ static void virtual_submit_request(struct i915_request *rq)
...
> @@ -3359,7 +3355,6 @@ intel_execlists_create_virtual(struct intel_engine_cs **siblings,
...
>  		     (unsigned long)ve);
> @@ -3543,10 +3538,6 @@ void intel_execlists_show_requests(struct intel_engine_cs *engine,
...
> index 600e681e4894..dd315455874f 100644
> --- a/drivers/gpu/drm/i915/i915_request.c
> +++ b/drivers/gpu/drm/i915/i915_request.c
> @@ -519,7 +519,7 @@ bool __i915_request_submit(struct i915_request *request)
...
> @@ -708,6 +708,7 @@ semaphore_notify(struct i915_sw_fence *fence, enum i915_sw_fence_notify state)
>  
>  	switch (state) {
>  	case FENCE_COMPLETE:
...
>  
>  	case FENCE_FREE:
> @@ -1868,14 +1869,15 @@ long i915_request_wait(struct i915_request *rq,
>  	return timeout;
>  }
>  
...
>  	return x;
>  }
> @@ -1955,7 +1957,7 @@ void i915_request_show(struct drm_printer *m,
>  	 *      from the lists
>  	 */
>  
...
> index afb4860868d1..18058267040e 100644
...
> @@ -249,7 +265,7 @@ static inline unsigned int random_level(struct i915_priolist_root *root)
>  }
>  
...
> @@ -257,12 +273,13 @@ lookup_priolist(struct intel_engine_cs *engine, int prio)
...
> @@ -272,14 +289,14 @@ lookup_priolist(struct intel_engine_cs *engine, int prio)
>  	pl = &root->sentinel;
>  	lvl = pl->level;
>  	do {
...
> @@ -303,7 +320,7 @@ lookup_priolist(struct intel_engine_cs *engine, int prio)
>  		}
>  	}
>  
> -	pl->priority = prio;
...
> @@ -325,7 +342,7 @@ lookup_priolist(struct intel_engine_cs *engine, int prio)
>  		chk = &root->sentinel;
>  		lvl = chk->level;
>  		do {
...
>  		} while (--lvl >= 0);
>  
> @@ -345,7 +362,7 @@ static void remove_priolist(struct intel_engine_cs *engine,
>  	struct i915_priolist *pl, *tmp;
...
>  
>  	lockdep_assert_held(&se->lock);
> @@ -356,7 +373,7 @@ static void remove_priolist(struct intel_engine_cs *engine,
>  
...
> @@ -375,7 +392,7 @@ void i915_priolist_advance(struct i915_priolist_root *root,
...
> @@ -410,53 +427,245 @@ stack_pop(struct i915_request *rq,
>  	return rq;
>  }
>  
...
> +{
> +	return virtual_deadline(signal_deadline(rq), adj_prio(rq));
> +}
> +
...
> @@ -471,13 +680,11 @@ static void ipi_priority(struct i915_request *rq, int prio)
>  	__ipi_add(rq);
>  }
>  
...
> @@ -499,6 +706,8 @@ static void __i915_request_set_priority(struct i915_request *rq, int prio)
>  	 */
>  	rq->sched.dfs.next = NULL;
>  	do {
...
>  			struct i915_dependency *p =
>  				list_entry(pos, typeof(*p), signal_link);
> @@ -524,6 +733,8 @@ static void __i915_request_set_priority(struct i915_request *rq, int prio)
...
>  		 * Once the request is ready, it will be placed into the
>  		 * priority lists and then onto the HW runlist. Before the
> @@ -532,21 +743,15 @@ static void __i915_request_set_priority(struct i915_request *rq, int prio)
>  		 * any preemption required, be dealt with upon submission.
>  		 * See engine->submit_request()
>  		 */
...
> @@ -599,7 +804,8 @@ void i915_request_set_priority(struct i915_request *rq, int prio)
>  		goto unlock;
>  
>  	rcu_read_lock();
...
>  
> @@ -607,18 +813,27 @@ void i915_request_set_priority(struct i915_request *rq, int prio)
>  	spin_unlock_irqrestore(&engine->active.lock, flags);
>  }
>  
...
> @@ -645,30 +860,32 @@ void __intel_engine_defer_request(struct intel_engine_cs *engine,
...
> @@ -678,33 +895,13 @@ void __intel_engine_defer_request(struct intel_engine_cs *engine,
...
>  {
>  	struct i915_dependency *p;
> @@ -742,6 +939,7 @@ void i915_request_enqueue(struct i915_request *rq)
>  {
>  	struct intel_engine_cs *engine = rq->engine;
...
>  	bool kick = false;
>  
> @@ -754,11 +952,11 @@ void i915_request_enqueue(struct i915_request *rq)
...
>  
>  	GEM_BUG_ON(list_empty(&rq->sched.link));
> @@ -771,8 +969,8 @@ struct i915_request *
...
>  	lockdep_assert_held(&engine->active.lock);
>  
> @@ -786,13 +984,20 @@ __intel_engine_rewind_requests(struct intel_engine_cs *engine)
>  
...
>  		set_bit(I915_FENCE_FLAG_PQUEUE, &rq->fence.flags);
>  
> @@ -872,14 +1077,10 @@ void __intel_engine_unhold_request(struct intel_engine_cs *engine,
>  				   struct i915_request *rq)
>  {
>  	LIST_HEAD(list);
> +	bool submit = false;
...
> @@ -893,9 +1094,9 @@ void __intel_engine_unhold_request(struct intel_engine_cs *engine,
...
> @@ -925,6 +1126,18 @@ void __intel_engine_unhold_request(struct intel_engine_cs *engine,
>  
...
> @@ -943,10 +1156,12 @@ void i915_sched_node_init(struct i915_sched_node *node)
...
> index 4432aab230b6..df0f1993b3b8 100644
> --- a/drivers/gpu/drm/i915/i915_scheduler.h
> +++ b/drivers/gpu/drm/i915/i915_scheduler.h
> @@ -36,9 +36,16 @@ void i915_sched_park_engine(struct i915_sched_engine *se);
...
>  __intel_engine_rewind_requests(struct intel_engine_cs *engine);
>  void __intel_engine_defer_request(struct intel_engine_cs *engine,
> @@ -49,11 +56,14 @@ bool __intel_engine_hold_request(struct intel_engine_cs *engine,
...
> index a851a156d460..d28ce320760f 100644

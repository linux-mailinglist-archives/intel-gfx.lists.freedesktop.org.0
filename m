...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id A855F12BBE6
	for <lists+intel-gfx@lfdr.de>; Sat, 28 Dec 2019 01:23:38 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 02C0E6E0F1;
	Sat, 28 Dec 2019 00:23:35 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga05.intel.com (mga05.intel.com [192.55.52.43])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 2B3D06E0E0
 for <intel-gfx@lists.freedesktop.org>; Sat, 28 Dec 2019 00:23:33 +0000 (UTC)
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from fmsmga004.fm.intel.com ([10.253.24.48])
 by fmsmga105.fm.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 27 Dec 2019 16:23:32 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.69,364,1571727600"; d="scan'208";a="243365221"
Received: from mdroper-desk1.fm.intel.com (HELO
 mdroper-desk1.amr.corp.intel.com) ([10.1.27.64])
 by fmsmga004.fm.intel.com with ESMTP; 27 Dec 2019 16:23:32 -0800
Date: Fri, 27 Dec 2019 16:23:32 -0800
From: Matt Roper <matthew.d.roper@intel.com>
To: Manasi Navare <manasi.d.navare@intel.com>
Message-ID: <20191228002332.GQ2877816@mdroper-desk1.amr.corp.intel.com>
References: <20191223224429.5151-1-manasi.d.navare@intel.com>
 ...
 <20191227231707.GN2877816@mdroper-desk1.amr.corp.intel.com>
 <20191227234915.GB27178@intel.com>
MIME-Version: 1.0
Content-Disposition: inline
In-Reply-To: <20191227234915.GB27178@intel.com>
...
> > > > > > > We also need to force modeset for all synced crtcs after fast=
set check.
> > > > > > > =
...
> > > > > > I sent a reply to your v4 of this right at the same time you se=
nt out
> > > > > > v5, but I'm not sure if my reply went through since it doesn't =
show up
> > > > > > in patchwork.  I've included the same feedback I gave on v4 bel=
ow in
...
> > > > > > > @@ -14304,6 +14304,118 @@ static bool intel_cpu_transcoder_ne=
eds_modeset(struct intel_atomic_state *state,
> > > > > > >  	return false;
> > > > > > >  }
> > > > > > >  =
...
> > > > > > Are we guaranteed to have the other CRTC in the state at this p=
oint?  If
> > > > > > the tile information was gone at the beginning of the transacti=
on, then
> > > > > > it wouldn't have known to bring in the other CRTC.  So if a mod=
eset was
> > > > > > only submitted on one of the two synchronized CRTCs then I don'=
t see
> > > > > > where the other one would be added to the transaction before th=
is point?
> > > > > >
> > > > > =

> > > > > Yes basically at this point the new crtc state is not cleared so =
we use the =

> > > > > previously synced crtcs through new crtc state master slave assig=
nments from previous
> > > > > modeset.
> > > > =
...
> > > > specific transaction.  If userspace requests a modeset on CRTC-A (e=
.g.,
> > > > master), that will add CRTC-A to the drm_atomic_state, but won't
> > > > automatically add CRTC-B (slave).  CRTC B's committed state does in=
deed
> > > > indicate that it's part of a port-sync pair, but it won't get itera=
ted
> > > > over in the loop above because it still hasn't been added with an
> > > > explicit drm_atomic_get_crtc_state().  So we'll never set mode_chan=
ged
> > > > on CRTC-B as far as I can see because nothing ever brought it into =
the
> > > > transaction (the only place where we bring in new CRTCs in this pat=
ch is
...
> >        already in the state, so it only processes connector-B.  It eith=
er
...
> Yes this above example makes complete sense. I think now the DRM patches =
make sure that
> if we lose one of the tiles through hotplug it also fallsback the mode on=
 remaining conn
> so conn A here to a lower non tiled mode so I see that it also gets added=
 to the state
> with mode changed and the other disable due to conn B unplug goes through=
 disable sequence
...
> But from what you are saying is it would be safe to check if any of the c=
onns in state that needs a modeset is involved in port sync
> and then loop through all drm connectors and  force modeset on all other =
conns in the same port sync?
> So kind of like the intel_modeset_synced_crtcs() right after modeset_all_=
tiles() ?
> =

> But may be for now, add a FIXME for this case and land this patch to make=
 sure that it fixes
...
> > > > > > > +
> > > > > > > +static void
> > > > > > > +intel_atomic_check_synced_crtcs(struct intel_atomic_state *s=
tate)
> > > > > > > +{
...
> > > > > > > +
> > > > > > > +static int
> > > > > > > +intel_modeset_all_tiles(struct intel_atomic_state *state, in=
t tile_grp_id)
> > > > > > > +{
> > > > > > > +	struct drm_i915_private *dev_priv =3D to_i915(state->base.d=
ev);
...
> > > > > > There isn't really anything i915-specific in this function and =
it feels
> > > > > > like something other drivers may need as well if they treat til=
ed
> > > > > > monitors in a similar manner.  We may want to consider pulling =
this out
> > > > > > to a DRM core helper, although we can do that in a future patch.
> > > > > > =

> > > > > > With a bit of extra work, the function below could also potenti=
ally be
> > > > > > moved to the core too.  Do you know if there's other hardware w=
ith port
> > > > > > sync capabilities that could benefit from these?
> > > > > >
> > > > > =

> > > > > Yes definitely can eventually make this a drm helper even though =
the whole master slave
> > > > > thing is i915 specific for now
> > > > >  =

> > > > > > > +
> > > > > > > +static int
> > > > > > > +intel_atomic_check_tiled_conns(struct intel_atomic_state *st=
ate)
> > > > > > > +{
> > > > > > > +	struct drm_i915_private *dev_priv =3D to_i915(state->base.d=
ev);
...
> > > > > > > +			continue;
> > > > > > > +
> > > > > > > +		ret =3D intel_modeset_all_tiles(state, connector->tile_gro=
up->id);
> > > > > > > +		if (ret)
> > > > > > > +			return ret;
> > > > > > > +	}
...
> > > > > > >   * @dev: drm device
> > > > > > > @@ -14331,6 +14443,10 @@ static int intel_atomic_check(struct=
 drm_device *dev,
> > > > > > >  	if (ret)
> > > > > > >  		goto fail;
> > > > > > >  =
...
> > > > > > > @@ -14378,6 +14494,13 @@ static int intel_atomic_check(struct=
 drm_device *dev,
> > > > > > >  		}
> > > > > > >  	}
> > > > > > >  =
...
> > > > > > explanation for why intel_atomic_check_tiled_conns above didn't=
 already
> > > > > > take care of this.  I.e., if you've plugged in a different moni=
tor, the
> > > > > > tile information may have already vanished from drm_connector b=
y the
> > > > > > time we start this atomic transaction, so we still need to deal=
 with
> > > > > > connectors that used to be tiled (and thus were port-synced) bu=
t no
> > > > > > longer are.
> > > > > >
> > > > > =
...
> > > > > > Although I can't think of a reason why it would cause a problem=
 in this
> > > > > > case, we do seem to be violating the directions in the big kern=
eldoc
> > > > > > warning attached to drm_atomic_helper_check_modeset().  I.e. if=
 we set
> > > > > > mode_changed in our own check functions, then we're supposed to=
 re-call
> > > > > > drm_atomic_helper_check_modeset() to make sure everything is pr=
operly
> > > > > > handled.  If we're not going to follow those directions, we sho=
uld
> > > > > > probably be clear why we don't think it's necessary.
> > > > > > =

> > > > > > Actually, I wonder if some of this tiling handling should event=
ually
> > > > > > migrate into that core helper function in the future...
> > > > > >
> > > > > =

> > > > > We are setting the mode hanged to true directly in our function s=
o yes it makes
> > > > > sense to eventually do that as part of the core helper function b=
ut i still
> > > > > dont see the need for us to call drm_atomic_helper_check_modeset(=
) again
> > > > > since our function is essentially checking modeset based on the t=
iled conditions
> > > > =
...
> > > > function in cases like what we're doing here, so if we're intention=
ally
> > > > not following those directions we should explain why for future
> > > > reference.  It's also possible that function may expand its role in=
 the
> > > > future and start doing something new that we really do need to worry
> > > > about, so it would be good to have the history and justification fo=
r not

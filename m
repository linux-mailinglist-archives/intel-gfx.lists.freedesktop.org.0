...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 4F4D33BDD1F
	for <lists+intel-gfx@lfdr.de>; Tue,  6 Jul 2021 20:24:03 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id C705E6E580;
...
> @@ -79,7 +79,8 @@ static_assert(sizeof(struct guc_ct_buffer_desc) =3D=3D =
64);
>   *  +---+-------+-------------------------------------------------------=
-------
> +
...
> @@ -74,7 +74,14 @@ static inline struct intel_guc *log_to_guc(struct inte=
l_guc_l
> og *log)
>  static
...
> +
> +static
> +inline int intel_guc_send_nb(struct intel_guc *guc, const u32 *action, u=
32 len)
> +{
> +       return intel_guc_ct_send(&guc->ct, action, len, NULL, 0,
...
> +       hxg =3D (flags & INTEL_GUC_CT_SEND_NB) ?
...
>      type =3D SEND_NB ? TYPE_EVENT : TYPE REQUEST;
>      hxg =3D PREP(type) | ACTION ...;
> =
...
> @@ -500,6 +506,48 @@ static int wait_for_ct_request_update(struct ct_requ=
est *re
> q, u32 *status)
>         return err;
>  }
...
> +static inline bool h2g_has_room(struct intel_guc_ct_buffer *ctb, u32 len=
_dw)
> +{
> +       struct guc_ct_buffer_desc *desc =3D ctb->desc;
...
>   */
>  int intel_guc_ct_send(struct intel_guc_ct *ct, const u32 *action, u32 le=
n,
> -                     u32 *response_buf, u32 response_buf_size)
...
>         int ret;
> @@ -581,6 +647,9 @@ int intel_guc_ct_send(struct intel_guc_ct *ct, const =
u32 *ac
> tion, u32 len,
>                 return -ENODEV;
>         }
...
> ;
>         if (unlikely(ret < 0)) {
>                 CT_ERROR(ct, "Sending action %#x failed (err=3D%d status=
=3D%#X)\n",
...
>   *
>   * Includes a pair of CT buffers for bi-directional communication and tr=
acking
> @@ -87,8 +86,9 @@ static inline bool intel_guc_ct_enabled(struct intel_gu=
c_ct *c
> t)
>         return ct->enabled;
>  }

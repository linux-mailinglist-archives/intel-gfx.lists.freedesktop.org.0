...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id AE86E337B54
	for <lists+intel-gfx@lfdr.de>; Thu, 11 Mar 2021 18:47:34 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 5B7866E435;
	Thu, 11 Mar 2021 17:47:32 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga01.intel.com (mga01.intel.com [192.55.52.88])
 by gabe.freedesktop.org (Postfix) with ESMTPS id A8F866E435
 for <intel-gfx@lists.freedesktop.org>; Thu, 11 Mar 2021 17:47:31 +0000 (UTC)
IronPort-SDR: 2LM98HnX2HjXByGsdm3gjEM0rXSUx87SD+mF2UYRivaV4uaondpkU4pLz0qlYWWyxdDEytTy0o
 0f0s+bXnr7gQ==
X-IronPort-AV: E=McAfee;i="6000,8403,9920"; a="208535676"
X-IronPort-AV: E=Sophos;i="5.81,241,1610438400"; d="scan'208";a="208535676"
Received: from orsmga008.jf.intel.com ([10.7.209.65])
 by fmsmga101.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 11 Mar 2021 09:47:30 -0800
IronPort-SDR: NExTpbuwhPW1S2AbV4MlAAzK0OHZETrz915jUAuc3SMI0OpZwLwcWxx3I3vxbKD8GvQi9/xLou
 X29r6rBK6JPQ==
X-IronPort-AV: E=Sophos;i="5.81,241,1610438400"; d="scan'208";a="410688889"
Received: from ideak-desk.fi.intel.com ([10.237.68.141])
 by orsmga008-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 11 Mar 2021 09:47:28 -0800
Date: Thu, 11 Mar 2021 19:47:25 +0200
From: Imre Deak <imre.deak@intel.com>
To: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
Message-ID: <20210311174725.GG2970909@ideak-desk.fi.intel.com>
...
> > > > remapping, add a helper to get the tile pitch and rows for an FB pl=
ane,
> > > > so these helpers can be used both by the normal size calculation an=
d the
> > > > remapping code.
> > > > =

> > > > Also add a new fb_plane_view_dims struct in which we can pass aroun=
d the
> > > > view (either FB plane or plane source) and tile dimensions convenie=
ntly
...
> > > > @@ -634,6 +634,59 @@ static u32 setup_fb_rotation(int plane, const =
...
> > > > +};
> > > > +
> > > > +static void init_plane_view_dims(const struct intel_framebuffer *f=
b, int color_plane,
...
> > > > +
> > > > +	intel_tile_dims(&fb->base, color_plane, &dims->tile_width, &dims-=
>tile_height);
> > > > +}
> > > > +
> > > > +static unsigned int plane_view_stride_tiles(const struct intel_fra=
mebuffer *fb, int color_plane,
...
> > > > +}
> > > > +
> > > > +static unsigned int plane_view_tile_rows(const struct intel_frameb=
...
> > > > +}
> > > > +
> > > >  int intel_fill_fb_info(struct drm_i915_private *i915, struct drm_f=
ramebuffer *fb)
> > > >  {
> > > >  	struct intel_framebuffer *intel_fb =3D to_intel_framebuffer(fb);
> > > > @@ -644,6 +697,7 @@ int intel_fill_fb_info(struct drm_i915_private =
...
> > > >  		u32 offset;
> > > > @@ -669,6 +723,8 @@ int intel_fill_fb_info(struct drm_i915_private =
*i915, struct drm_framebuffer *fb
> > > >  		if (ret)
> > > >  			return ret;
> > > >  =
...
> > > > @@ -680,38 +736,22 @@ int intel_fill_fb_info(struct drm_i915_privat=
...
> > > > +			plane_info.stride =3D plane_view_stride_tiles(intel_fb, i, &vie=
w_dims);
> > > > +			plane_info.width =3D DIV_ROUND_UP(x + width, view_dims.tile_wid=
th);
> > > =
...
> > > > +			plane_info.height =3D plane_view_tile_rows(intel_fb, i, &view_d=
ims, y);
> > > >  =

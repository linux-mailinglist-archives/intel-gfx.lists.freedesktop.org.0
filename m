...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id 0F68F29CFFA
	for <lists+intel-gfx@lfdr.de>; Wed, 28 Oct 2020 14:04:50 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 09F536E50C;
	Wed, 28 Oct 2020 13:04:48 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga05.intel.com (mga05.intel.com [192.55.52.43])
 by gabe.freedesktop.org (Postfix) with ESMTPS id ECE6F6E0B8
 for <intel-gfx@lists.freedesktop.org>; Wed, 28 Oct 2020 13:04:46 +0000 (UTC)
IronPort-SDR: Nz2LRurrR6+h/ZGdBZPlLWxomlQVHnxnCB20hl6ImHy50OyzARcmBUjYw3ZL8DDa7/ghMRrIna
 GZEqPTs2aUGA==
X-IronPort-AV: E=McAfee;i="6000,8403,9787"; a="252950650"
X-IronPort-AV: E=Sophos;i="5.77,426,1596524400"; d="scan'208";a="252950650"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga001.fm.intel.com ([10.253.24.23])
 by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 28 Oct 2020 06:04:40 -0700
IronPort-SDR: PF1wwODxvQwCgnk8b2X7hfy25TT2qQ1HXXw0dugonbcMeMktavPuUkLQ0dgHJ7d305mgivtNxl
 Jbiv8SHyaayA==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.77,426,1596524400"; d="scan'208";a="424719958"
Received: from stinkbox.fi.intel.com (HELO stinkbox) ([10.237.72.174])
 by fmsmga001.fm.intel.com with SMTP; 28 Oct 2020 06:04:38 -0700
Received: by stinkbox (sSMTP sendmail emulation);
 Wed, 28 Oct 2020 15:04:37 +0200
Date: Wed, 28 Oct 2020 15:04:37 +0200
From: Ville =?iso-8859-1?Q?Syrj=E4l=E4?= <ville.syrjala@linux.intel.com>
To: Maarten Lankhorst <maarten.lankhorst@linux.intel.com>
Message-ID: <20201028130437.GW6112@intel.com>
...
In-Reply-To: <dec38bb5-c6a2-18dc-684c-92564e630451@linux.intel.com>
...
> >>>>>>
> >>>>>>  Make sure that when a plane is set in a bigjoiner mode, we will a=
dd
...
> >>>>>>
> >>>>>> Also fix the atomic intel plane iterator, so things watermarks sta=
rt
...
> >>>>>> - Make intel_atomic_crtc_state_for_each_plane_state() bigjoiner ca=
pable.
> >>>>>> - Add iter macro to intel_atomic_crtc_state_for_each_plane_state()=
 to
> >>>>>>   keep iteration working.
> >>>>>> Changes since v2:
> >>>>>> - Add icl_(un)set_bigjoiner_plane_links, to make it more clear whe=
re
> >>>>>>   links are made and broken.
> >>>>>>
> >>>>>> Signed-off-by: Maarten Lankhorst <maarten.lankhorst@linux.intel.co=
m>
...
> >>>>>>
> >>>>>> diff --git a/drivers/gpu/drm/i915/display/intel_atomic_plane.c b/d=
rivers/gpu/drm/i915/display/intel_atomic_plane.c
> >>>>>> index 3334ff253600..5df928f8f322 100644
> >>>>>> --- a/drivers/gpu/drm/i915/display/intel_atomic_plane.c
> >>>>>> +++ b/drivers/gpu/drm/i915/display/intel_atomic_plane.c
> >>>>>> @@ -246,12 +246,17 @@ static void intel_plane_clear_hw_state(struc=
...
plane_state,
> >>>>>> +void intel_plane_copy_uapi_to_hw_state(const struct intel_crtc_st=
ate *crtc_state,
...
> >>>>>> @@ -320,15 +325,36 @@ int intel_plane_atomic_check_with_state(cons=
t struct intel_crtc_state *old_crtc_
> >>>>>>  }
> >>>>>>  =

> >>>>>>  static struct intel_crtc *
> >>>>>> -get_crtc_from_states(const struct intel_plane_state *old_plane_st=
ate,
...
> >>>>>> +	struct intel_plane *plane =3D to_intel_plane(new_plane_state->ua=
pi.plane);
> >>>>>> +
...
> >>>>>> +			intel_atomic_get_new_plane_state(state, new_plane_state->bigjo=
iner_plane);
> >>>>>> +
> >>>>>> +		/* need to use uapi here, new_master_plane_state might not be c=
opied to hw yet */
...
> >>>>>> +			intel_atomic_get_old_plane_state(state, old_plane_state->bigjo=
iner_plane);
> >>>>>> +
...
> >>>>>> @@ -339,18 +365,33 @@ int intel_plane_atomic_check(struct intel_at=
omic_state *state,
...
> >>>>>> -	intel_plane_copy_uapi_to_hw_state(new_plane_state, new_plane_sta=
te);
> >>>>>> +	if (crtc)
...
> >>>>>>  						   new_crtc_state,
> >>>>>> diff --git a/drivers/gpu/drm/i915/display/intel_atomic_plane.h b/d=
rivers/gpu/drm/i915/display/intel_atomic_plane.h
...
> >>>>>> diff --git a/drivers/gpu/drm/i915/display/intel_display.c b/driver=
s/gpu/drm/i915/display/intel_display.c
> >>>>>> index c0715a3ea47b..579cccc1fd91 100644
> >>>>>> --- a/drivers/gpu/drm/i915/display/intel_display.c
> >>>>>> +++ b/drivers/gpu/drm/i915/display/intel_display.c
> >>>>>> @@ -3718,7 +3718,7 @@ intel_find_initial_plane_obj(struct intel_cr=
tc *intel_crtc,
...
> >>>>>> +	intel_plane_copy_uapi_to_hw_state(crtc_state, intel_state, intel=
_state);
> >>>>>>  =
...
> >>>>>> @@ -12801,26 +12801,180 @@ static bool check_single_encoder_clonin=
g(struct intel_atomic_state *state,
> >>>>>>  	return true;
> >>>>>>  }
> >>>>>>  =

> >>>>>> +static int icl_unset_bigjoiner_plane_links(struct intel_atomic_st=
ate *state,
> >>>>>> +					   struct intel_crtc_state *new_crtc_state)
> >>>>>> +{
> >>>>>> +	struct intel_crtc *crtc =3D to_intel_crtc(new_crtc_state->uapi.c=
rtc);
...
> >>>>>> +
> >>>>>> +		other_plane_state =3D intel_atomic_get_plane_state(state, other=
_plane);
...
> >>>>>
> >>>> Yes may be but honestly I leave this optimization/change to the orig=
inal
> >>>> author Maarten or you as a follow up
> >>> I don't want to see several hundred lines of totally uneccessary code
> >>> added. If it's buggy (which it may very well be because it's too big =
to
...
> There's good reason to link the planes. The reason is similar to linking =
Y and CbCr planes.
...
> You're completely correct that hardware programming doesn't need the link=
ing, and planes are
> in theory standalone.
> =

> But there's also atomic. The lifetime of plane_state on the bigjoiner sla=
ve plane must be
> linked to the lifetime of the master plane. Otherwise if you do a commit =
on the master plane,

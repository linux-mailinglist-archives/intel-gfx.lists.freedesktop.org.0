...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id A65F45F1786
	for <lists+intel-gfx@lfdr.de>; Sat,  1 Oct 2022 02:47:02 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id A3F2710EDEF;
	Sat,  1 Oct 2022 00:46:32 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga05.intel.com (mga05.intel.com [192.55.52.43])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 6F2EB10E49D;
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
 ...
 bh=FoXvi3befdkRPuXaOi7JB/TbCdKx6ficAzVM5L25DJI=;
 b=AOu9GbnM03bwQ3FUFBmfBXkSXMPRty2VHC+cwRcluUvoMLHe6SDmJCr9
 jD0gbwcfGX7R2QVLGZ+hnuQkIRRAFWoxP/rteo0ljz9VD1ytg8LxzlzR5
 cH50+Xm+gEYuiYqpaKsQba4KvSMBpqPOB6j1PkgVjJO+tb/nQxuiHd3xn
 vF6mJkwqCptC+WUNkYr1Q4kbH4nHi52duhUve6Wh58gZBqKgwCLN86Mz3
 VjabGQVx98Gh6pMz9rTGRz1rimRG4PBByfJY61xen0yeNw+5X6rMJVAVA
 9I98a+B6M04ZXgNKR7HnMX+QR3piW+/w2HjQhQO2zpAiuxk9QWgKvqR0X Q==;
X-IronPort-AV: E=McAfee;i="6500,9779,10486"; a="388607931"
X-IronPort-AV: E=Sophos;i="5.93,359,1654585200"; d="scan'208";a="388607931"
Received: from fmsmga008.fm.intel.com ([10.253.24.58])
 by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 30 Sep 2022 17:46:12 -0700
X-IronPort-AV: E=McAfee;i="6500,9779,10486"; a="685477644"
X-IronPort-AV: E=Sophos;i="5.93,359,1654585200"; d="scan'208";a="685477644"
Received: from mdroper-desk1.fm.intel.com ([10.1.27.134])
 by fmsmga008-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 30 Sep 2022 17:46:09 -0700
From: Matt Roper <matthew.d.roper@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Fri, 30 Sep 2022 17:45:47 -0700
Message-Id: <20221001004550.3031431-12-matthew.d.roper@intel.com>
...
Subject: [Intel-gfx] [PATCH v2 11/14] drm/i915/gt: Add MCR-specific
...
index 7bb878581d00..c5cd89aa13c9 100644
...
+				  LSC_L1_FLUSH_CTL_3D_DATAPORT_FLUSH_EVENTS_MASK);
 	...
 	if (IS_DG2_GRAPHICS_STEP(i915, G10, STEP_A0, STEP_B0)) {
@@ -2179,7 +2267,7 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2223,17 +2311,17 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 	if (IS_DG2_GRAPHICS_STEP(engine->i915, G10, STEP_A0, STEP_B0) ||
 	    IS_DG2_GRAPHICS_STEP(engine->i915, G11, STEP_A0, STEP_B0)) {
 		/* Wa_14012362059:dg2 */
...
@@ -2260,7 +2348,7 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+		wa_mcr_masked_en(wal, GEN8_ROW_CHICKEN2, GEN12_DISABLE_EARLY_READ);
 		...
 		/*
 		 * Wa_1407928979:tgl A*
@@ -2289,14 +2377,14 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+		wa_mcr_masked_en(wal, GEN9_ROW_CHICKEN4, GEN12_DISABLE_TDL_PUSH);
 	...
 	if (IS_DG1_GRAPHICS_STEP(i915, STEP_A0, STEP_B0) ||
@@ -2320,9 +2408,9 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2356,9 +2444,9 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 		 * Wa_1405733216:icl
...
+				GEN11_LQSC_CLEAN_EVICT_DISABLE);
 		...
 		/* Wa_1606682166:icl */
 		wa_write_or(wal,
@@ -2366,10 +2454,10 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
@@ -2427,30 +2515,30 @@ rcs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
...
+				     EVICTION_PERF_FIX_ENABLE, 0);
 	...
 	if (IS_HASWELL(i915)) {
@@ -2664,7 +2752,7 @@ ccs_engine_wa_init(struct intel_engine_cs *engine, struct i915_wa_list *wal)
 {
...
@@ -2690,8 +2778,8 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
 	}
...
@@ -2702,10 +2790,10 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
...
+			   true);
 	...
 	/*
@@ -2714,8 +2802,8 @@ add_render_compute_tuning_settings(struct drm_i915_private *i915,
 	 * platforms.
 	 */
...
+					THREAD_EX_ARB_MODE_RR_AFTER_DEP);
 ...
 /*
@@ -2741,30 +2829,30 @@ general_render_compute_wa_init(struct intel_engine_cs *engine, struct i915_wa_li
...
@@ -2773,19 +2861,19 @@ general_render_compute_wa_init(struct intel_engine_cs *engine, struct i915_wa_li
...
+		wa_mcr_write_or(wal, VEBX_MOD_CTRL, FORCE_MISS_FTLB);
 	...
 	if (IS_DG2(i915)) {
@@ -2793,7 +2881,7 @@ general_render_compute_wa_init(struct intel_engine_cs *engine, struct i915_wa_li

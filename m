...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 428F21C37C4
	for <lists+intel-gfx@lfdr.de>; Mon,  4 May 2020 13:13:03 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 96B1589FCC;
	Mon,  4 May 2020 11:13:01 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mga11.intel.com (mga11.intel.com [192.55.52.93])
 by gabe.freedesktop.org (Postfix) with ESMTPS id BB67A89FCC
 for <intel-gfx@lists.freedesktop.org>; Mon,  4 May 2020 11:12:59 +0000 (UTC)
IronPort-SDR: xAQspaeRyjxrWom88s0hiFm6Wqjhb8IrcgZBz27Vq3sl5SkfrOKprCen81EHu8qP37ZM8u7A6v
 E76q5/M4wZxw==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga007.jf.intel.com ([10.7.209.58])
 by fmsmga102.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 04 May 2020 04:12:59 -0700
IronPort-SDR: ejUbMajpVcQzC+pMWbj9sSNvS0N7kgP/oPdy+44RdMFDBNrlUzuXdnc1ZcehKtnMD2Focg+7QK
 g3IPL9SGlGUw==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,351,1583222400"; d="scan'208";a="248188636"
Received: from efilatov-mobl.ger.corp.intel.com (HELO
 delly.ger.corp.intel.com) ([10.252.56.163])
 by orsmga007.jf.intel.com with ESMTP; 04 May 2020 04:12:57 -0700
From: Lionel Landwerlin <lionel.g.landwerlin@intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Mon,  4 May 2020 14:12:48 +0300
Message-Id: <20200504111249.1367096-4-lionel.g.landwerlin@intel.com>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH v12 3/4] drm/i915/perf: prepare driver to
...
---
 drivers/gpu/drm/i915/i915_perf.c       | 565 +++++++++++++++----------
 drivers/gpu/drm/i915/i915_perf_types.h |  37 +-
 2 files changed, 361 insertions(+), 241 deletions(-)

diff --git a/drivers/gpu/drm/i915/i915_perf.c b/drivers/gpu/drm/i915/i915_perf.c
index 67257bf9768c..66d52ee4767b 100644
...
+			stream->oa_buffer.last_ctx_match = ctx_match;
 		...
 		/*
@@ -1197,136 +1223,174 @@ static int i915_oa_read(struct i915_perf_stream *stream,
...
+	return err;
 ...
 static void
@@ -1389,10 +1453,13 @@ static void i915_oa_stream_destroy(struct i915_perf_stream *stream)
...
@@ -1403,8 +1470,7 @@ static void i915_oa_stream_destroy(struct i915_perf_stream *stream)
...
@@ -1496,7 +1562,7 @@ static void gen8_init_oa_buffer(struct i915_perf_stream *stream)
...
@@ -1550,7 +1616,7 @@ static void gen12_init_oa_buffer(struct i915_perf_stream *stream)
 	 * reports we will forward to userspace while filtering for a single
 	 * context.
 	 */
-	stream->oa_buffer.last_ctx_id = INVALID_CTX_ID;
...
@@ -2273,11 +2339,10 @@ static int gen8_configure_context(struct i915_perf_stream *stream,
 	return err;
 }
...
+	int i;
 	...
 	struct flex regs_context[] = {
 		{
@@ -2298,7 +2363,7 @@ static int gen12_configure_oar_context(struct i915_perf_stream *stream,
...
+			RING_CONTEXT_CONTROL(stream->engine->mmio_base),
 			...
 			_MASKED_FIELD(GEN12_CTX_CTRL_OAR_CONTEXT_ENABLE,
 				      active ?
@@ -2307,18 +2372,28 @@ static int gen12_configure_oar_context(struct i915_perf_stream *stream,
 		},
 	};
...
+	return 0;
 ...
 /*
@@ -2585,11 +2660,9 @@ gen12_enable_metric_set(struct i915_perf_stream *stream,
...
@@ -2630,8 +2703,7 @@ static void gen12_disable_metric_set(struct i915_perf_stream *stream,
...
@@ -2640,8 +2712,7 @@ static void gen12_disable_metric_set(struct i915_perf_stream *stream,
...
@@ -2663,7 +2734,7 @@ static void gen7_oa_enable(struct i915_perf_stream *stream)
...
@@ -2880,7 +2951,7 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	}
...
@@ -2928,12 +2999,10 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	if (stream->periodic)
...
@@ -2970,21 +3039,14 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
...
-	stream->config_context->timeline = timeline;
 	...
 	if (ret)
@@ -3019,7 +3081,7 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 err_context_pin:
...
+err_context:
 ...
 err_oa_buf_alloc:
@@ -3032,8 +3094,7 @@ static int i915_oa_stream_init(struct i915_perf_stream *stream,
 	free_noa_wait(stream);
...
@@ -3226,8 +3287,12 @@ static void i915_perf_enable_locked(struct i915_perf_stream *stream)
...
+	}
 ...
 /**
@@ -3252,8 +3317,12 @@ static void i915_perf_disable_locked(struct i915_perf_stream *stream)
...
@@ -3271,7 +3340,7 @@ static long i915_perf_config_locked(struct i915_perf_stream *stream,
 		return -EINVAL;
...
+		int i;
 		...
 		if (!active) {
@@ -3279,28 +3348,29 @@ static long i915_perf_config_locked(struct i915_perf_stream *stream,
 			goto err_config;
 		}
...
@@ -3381,9 +3451,10 @@ static void i915_perf_destroy_locked(struct i915_perf_stream *stream)
...
@@ -3458,25 +3529,12 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
...
@@ -3497,17 +3555,16 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
...
@@ -3528,23 +3585,43 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
 	if (privileged_op &&
...
@@ -3579,12 +3656,11 @@ i915_perf_open_ioctl_locked(struct i915_perf *perf,
 err_flags:
 	if (stream->ops->destroy)
 		stream->ops->destroy(stream);
-err_alloc:
...
@@ -3616,7 +3692,7 @@ static int read_properties_unlocked(struct i915_perf *perf,
 {
...
@@ -3650,23 +3726,36 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+			props->n_ctx_handles = 1;
 		...
 		case DRM_I915_PERF_PROP_SAMPLE_OA:
 			if (value)
@@ -3675,7 +3764,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			break;
@@ -3683,12 +3773,14 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			break;
@@ -3696,7 +3788,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
@@ -3725,7 +3818,8 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
@@ -3741,13 +3835,14 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+				goto error;
 			...
 			props->has_sseu = true;
 			break;
@@ -3756,19 +3851,25 @@ static int read_properties_unlocked(struct i915_perf *perf,
...
+	return err;
 ...
 /**
@@ -3828,6 +3929,8 @@ int i915_perf_open_ioctl(struct drm_device *dev, void *data,
...
index c0a78166b1d9..45faf89f7be4 100644

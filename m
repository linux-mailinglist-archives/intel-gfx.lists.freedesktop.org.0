...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [IPv6:2610:10:20:722:a800:ff:fe36:1795])
	by mail.lfdr.de (Postfix) with ESMTPS id 6454D1BDC46
	for <lists+intel-gfx@lfdr.de>; Wed, 29 Apr 2020 14:33:26 +0200 (CEST)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id C2A106ECF6;
	Wed, 29 Apr 2020 12:33:23 +0000 (UTC)
X-Original-To: intel-gfx@lists.freedesktop.org
Delivered-To: intel-gfx@lists.freedesktop.org
Received: from mblankhorst.nl (mblankhorst.nl [141.105.120.124])
 by gabe.freedesktop.org (Postfix) with ESMTPS id B3F126EAB3
 for <intel-gfx@lists.freedesktop.org>; Wed, 29 Apr 2020 12:33:20 +0000 (UTC)
From: Maarten Lankhorst <maarten.lankhorst@linux.intel.com>
To: intel-gfx@lists.freedesktop.org
Date: Wed, 29 Apr 2020 14:33:04 +0200
Message-Id: <20200429123307.1449297-22-maarten.lankhorst@linux.intel.com>
...
MIME-Version: 1.0
Subject: [Intel-gfx] [PATCH 22/25] drm/i915: Move i915_vma_lock in the
...
index 21338c1980de..8c21cc46e0a7 100644
--- a/drivers/gpu/drm/i915/gt/selftest_rps.c
+++ b/drivers/gpu/drm/i915/gt/selftest_rps.c
@@ -84,20 +84,20 @@ create_spin_counter(struct intel_engine_cs *engine,
...
@@ -141,6 +141,14 @@ create_spin_counter(struct intel_engine_cs *engine,
 	*cancel = base + loop;
...
@@ -635,7 +643,6 @@ int live_rps_frequency_cs(void *arg)
 			goto err_vma;
 		}
...
-		i915_vma_lock(vma);
 		...
 		if (!err)
 			err = i915_vma_move_to_active(vma, rq, 0);
@@ -643,7 +650,6 @@ int live_rps_frequency_cs(void *arg)
...
-		i915_vma_unlock(vma);
 		...
 		if (err)
 			goto err_vma;
@@ -703,6 +709,7 @@ int live_rps_frequency_cs(void *arg)
...
@@ -777,7 +784,6 @@ int live_rps_frequency_srm(void *arg)
 			goto err_vma;
 		}
...
-		i915_vma_lock(vma);
 		...
 		if (!err)
 			err = i915_vma_move_to_active(vma, rq, 0);
@@ -785,7 +791,6 @@ int live_rps_frequency_srm(void *arg)
 			err = rq->engine->emit_bb_start(rq,
 							vma->node.start,
 							PAGE_SIZE, 0);
-		i915_vma_unlock(vma);
 		...
 		if (err)
 			goto err_vma;
@@ -844,6 +849,7 @@ int live_rps_frequency_srm(void *arg)
 		*cancel = MI_BATCH_BUFFER_END;
 		i915_gem_object_unpin_map(vma->obj);
 		i915_vma_unpin(vma);
+		i915_vma_unlock(vma);
...
index 3b319c0953cb..45258bc8d7ac 100644
